...
public:
    ...
    struct npc_mirror_imageAI : CasterAI
    {
        ...
        void IsSummonedBy(Unit* owner) override
        {
            ...
            // me->CastSpell(owner, 58838, true);

            // here mirror image casts on summoner spell (not present in client dbc) 49866
            // here should be auras (not present in client dbc): 35657, 35658, 35659, 35660 selfcasted by mirror images (stats related?)

            for (uint32 attackType = 0; attackType < MAX_ATTACK; attackType++)
            {
                WeaponAttackType attackTypeEnum = static_cast<WeaponAttackType>(attackType);
                me->SetBaseWeaponDamage(attackTypeEnum, MAXDAMAGE, owner->GetWeaponDamageRange(attackTypeEnum, MAXDAMAGE));
                me->SetBaseWeaponDamage(attackTypeEnum, MINDAMAGE, owner->GetWeaponDamageRange(attackTypeEnum, MINDAMAGE));
            }
        ...
        void EnterCombat(Unit* who) override
        {
            ...
            spell = SPELL_MAGE_FROSTBOLT;
            if (owner->HasGlyph(SPELL_MAGE_GLYPH))
            {
                switch (owner->GetSpecializationId(owner->GetActiveSpec()))
                {
                    case SPEC_MAGE_ARCANE:
                        spell = SPELL_MAGE_ARCANE_BLAST;
                        break;
                    case SPEC_MAGE_FIRE:
                        spell = SPELL_MAGE_FIREBALL;
                        break;
                    default: break;
                }
            }
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            if (victim)
            {
                ...
                {
                    // If not already casting, cast!
                    if (!me->HasUnitState(UNIT_STATE_CASTING))
                    {
                        me->CastSpell(victim, spell, false);
                        uint32 castTime = me->GetCurrentSpellCastTime(spell);
                    }
                }
                else
                {
            ...
            else
            {
                ...
                if (!target)
                {
                    ...
                    if (Unit* owner = me->GetOwner())
                    {
                        Unit* ownerVictim = nullptr;
                        if (Player* player = owner->ToPlayer())
                            ownerVictim = player->GetSelectedUnit();
                        else
                            ownerVictim = owner->getVictim();
	...
	struct npc_ebon_gargoyleAI : CasterAI
	{
		...
		void InitializeAI()
		{
			...
			if (Unit* owner = me->GetOwner())
			{
				...
				me->NearTeleportTo(x, y, z + 10, me->GetOrientation());
...
class npc_choose_faction : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
			if (!player->IsQuestRewarded(29799/*QUEST_THE_HEALING_OF_SHENZIN_SU*/))
				return true;
		...
		bool OnGossipSelect(Player* player, Creature* /*creature*/, uint32 /*sender*/, uint32 action)
		{
			...
			{
				if (player->HasAura(95247)) //Hack of Remove Ride Hot Air Balloon Spell for fix problem with use portal of pandaren race
					player->RemoveAura(95247);
...
class npc_capacitor_totem : public CreatureScript
{
    ...
    struct npc_capacitor_totemAI : public ScriptedAI
    {
		...
		void Reset()
		{
			if (Unit* owner = me->GetOwner())
			{
				if (owner->HasAura(55442)) // Glyph of Capacitor Totem
				{
					Timer = 3000;
					me->ApplyCastTimePercentMod(60, true);
				}
				else
					Timer = 5000;
			}
...
class npc_demoralizing_banner : public CreatureScript
{
    public:
        ...
        struct npc_demoralizing_bannerAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                //This banner must have 30% of the caster max health (Blizzlike)
                if (Unit* owner = me->GetOwner())
                {
                    me->SetMaxHealth(owner->CountPctFromMaxHealth(30));
                    me->SetHealth(owner->CountPctFromMaxHealth(30));
                }
            }
...
class npc_skull_banner : public CreatureScript
{
public:
    npc_skull_banner() : CreatureScript("npc_skull_banner") { }

    struct npc_skull_bannerAI : public ScriptedAI
    {
        npc_skull_bannerAI(Creature* creature) : ScriptedAI(creature) { }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_skull_bannerAI(creature);
    }
};
...
class npc_mocking_banner : public CreatureScript
{
public:
    npc_mocking_banner() : CreatureScript("npc_mocking_banner") { }

    struct npc_mocking_bannerAI : public ScriptedAI
    {
        npc_mocking_bannerAI(Creature* creature) : ScriptedAI(creature) { }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_mocking_bannerAI(creature);
    }
};
...
public:
    ...
    struct npc_frozen_orbAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                if (Unit* target = me->SelectNearbyTarget(0, 5.0f))
                    if (!me->HasAura(SPELL_MAGE_FROZEN_ORB_SELF_SNARE))
                        me->CastSpell(me, SPELL_MAGE_FROZEN_ORB_SELF_SNARE, true);

                if (frozenOrbTimer <= diff)
                {
                    if (Unit* target = me->SelectNearbyTarget(0, 10.0f))
                    {
                        ...
                        if (counter == 1)
                        {
                            owner->AddAura(SPELL_FINGERS_OF_FROST_VISUAL, owner);

                            if (AuraPtr auraFingerOfFrost = owner->GetAura(SPELL_FINGERS_OF_FROST_VISUAL))
                                if (auraFingerOfFrost->GetStackAmount() > 1)
                                    owner->AddAura(SPELL_FINGERS_OF_FROST, owner);
                        }
                    }

                    if (owner->HasSpellCooldown(SPELL_MAGE_FROZEN_ORB_DMG))
                        owner->RemoveSpellCooldown(SPELL_MAGE_FROZEN_ORB_DMG);

                    if (owner->GetGUID())
                        me->CastSpell(me, SPELL_MAGE_FROZEN_ORB_DMG, true, NULL, NULLAURA_EFFECT, owner->GetGUID());
    ...
    public:
        ...
        struct npc_wild_impAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
				...
				if ((me->getVictim() || pOwner->getAttackerForHelper() || (pOwner->GetSelectedUnit() && me->IsValidAttackTarget(pOwner->GetSelectedUnit()))))
                {					
					me->CastSpell(me->getVictim() ? me->getVictim() : (pOwner->GetSelectedUnit()) ? pOwner->GetSelectedUnit() : pOwner->getAttackerForHelper(), FIREBOLT, false);
                    charges--;

                    int32 demonicFury = 8;
                    pOwner->CastCustomSpell(pOwner, 104314, NULL, &demonicFury, NULL, true);

                    if (pOwner->HasAura(MoltenCore) && pOwner->getLevel() >= 69)
                        if (roll_chance_i(8))
                        {
                            pOwner->CastSpell(pOwner, MoltenCoreAura, true);

                            if (AuraPtr MoltenCore = pOwner->GetAura(MoltenCoreAura))
                            {
                                int32 stacks = MoltenCore->GetStackAmount();

                                if (stacks > 1)
                                    pOwner->CastSpell(pOwner, MoltenCoreVisual, true);
                            }
                        }
                }
...
enum eNpcWildMushroom
{
	SPELL_WILD_MUSHROOM_INVISIBILITY = 92661,
...
public:
	...
	struct npc_wild_mushroomAI : public ScriptedAI
	{
		...
		uint64 ownerGUID;
		uint32 CastTimer;
		uint32 m_min_check_timer;
		bool checkForGlyphOfEfflorescence;
		bool stealthed;
		//
		float posX;
		...
		void Reset()
		{
			m_min_check_timer = 300;
			me->SetReactState(REACT_PASSIVE);
			ownerGUID = 0;
			CastTimer = 6000;
			checkForGlyphOfEfflorescence = false;
			stealthed = false;
		}

		void JustDied(Unit* killer)
		{
			if (DynamicObject* dynObj = killer->GetDynObject(SPELL_DRUID_EFFLORESCENCE))
				dynObj->Remove();
		}

		void UpdateAI(const uint32 diff)
		{
			if (m_min_check_timer <= diff)
			{
				if (!checkForGlyphOfEfflorescence)
				{
					checkForGlyphOfEfflorescence = true;
					ownerGUID = me->GetUInt64Value(UNIT_FIELD_SUMMONEDBY);
					if (ownerGUID != 0)
					{
						if (Unit* unitOwner = ObjectAccessor::FindUnit(ownerGUID))
						{
							if (unitOwner->GetTypeId() == TYPEID_PLAYER)
							{
								if (Player* playerOwner = unitOwner->ToPlayer())
								{
									if (playerOwner->GetSpecializationId(playerOwner->GetActiveSpec()) == SPEC_DRUID_RESTORATION && playerOwner->HasGlyph(SPELL_DRUID_GLYPH_OF_EFFLORESCENCE))
									{
										if (!me->HasAura(SPELL_DRUID_EFFLORESCENCE))
										{
											me->AddAura(SPELL_DRUID_EFFLORESCENCE, me);
										}
									}
								}
							}
			...
			{
				m_min_check_timer -= diff;
			}

			// Invisibility
			if (CastTimer <= diff && !stealthed)
			{
				DoCast(me, SPELL_WILD_MUSHROOM_INVISIBILITY, true);
				me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
				stealthed = true;
			}
			else
			{
				...
				if (!stealthed)
					me->RemoveAura(SPELL_WILD_MUSHROOM_INVISIBILITY);
			}
		}
	};
...
public:
	...
	struct npc_fungal_growthAI : public Scripted_NoMovementAI
	{
		...
		void Reset()
		{
			...
			me->AddAura(FUNGAL_GROWTH_PERIODIC, me);			// Periodic Trigger spell : decrease speed
...
struct auraData
{
    auraData(uint32 id, int32 duration, uint8 stacks, uint64 casterGuid) : m_id(id), m_duration(duration), m_stacks(stacks), m_CasterGuid(casterGuid) {}
    uint32 m_id;
    int32 m_duration;
    uint8 m_stacks;
    uint64 m_CasterGuid;
                ...
                if (owner && owner->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    for (Unit::AuraApplicationMap::const_iterator itr = appliedAuras.begin(); itr != appliedAuras.end(); ++itr)
                    {
                        if (AuraPtr aura = itr->second->GetBase())
                        {
							...
							if (auraInfo->Id == 110310)
								continue;

                            auras.insert(new auraData(auraInfo->Id, aura->GetDuration(), aura->GetStackAmount(), aura->GetCasterGUID()));
                        }
                    }
                    ...
                    if (TempSummon* pastSelf = me->ToTempSummon())
                    {
                        if (Unit* m_owner = pastSelf->GetSummoner())
                        {
                            if (m_owner->ToPlayer())
                            {
                                ...
                                {
                                    AuraPtr aura = !m_owner->HasAura((*itr)->m_id, (*itr)->m_CasterGuid) ? m_owner->AddAura((*itr)->m_id, m_owner, (*itr)->m_CasterGuid) : m_owner->GetAura((*itr)->m_id, (*itr)->m_CasterGuid);
                                    if (aura)
                                    {
...
{
    SPELL_INITIALIZE_IMAGES = 102284,
    SPELL_GLYPH_OF_FIGHTING_POSE = 125872,
    SPELL_CLONE_CASTER = 102288,
    ...
    struct npc_transcendence_spiritAI : public Scripted_NoMovementAI
    {
        ...
        void Reset()
        {
            if (Unit* owner = me->GetOwner())
            {
                ...
                owner->AddAura(SPELL_VISUAL_SPIRIT_IMG, me);

                if (owner->HasAura(SPELL_GLYPH_OF_FIGHTING_POSE))
                    me->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_STATE_MONKOFFENSE_READYUNARMED);
                else
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (psychicHorrorTimer <= diff)
            {
                if (Unit* owner = me->GetOwner())
                {
                    ...
                    filterMask |= (EXCLUDE_ALLIES | ONLY_LOS | HAS_NOT_SPECIFIC_AURA);

                    me->FilterTargets(targetList, radius, filterMask, 1, NULL, DistanceOrderPred, 0, SPELL_PSYCHIC_HORROR);

                    for (auto itr : targetList)
                        me->CastSpell(itr, SPELL_PSYCHIC_HORROR, false, NULL, NULLAURA_EFFECT, owner->GetGUID());

                    psychicHorrorTimer =  200;
                }
            }
            else
...
class npc_custom_caster_guard : public CreatureScript
{
    public:
        ...
        struct npc_custom_caster_guardAI : CasterAI
        {
            ...
            {
				Player* warlock = me->GetOwner()->ToPlayer();
				if (me->GetEntry() == NPC_DOOM_GUARD &&  warlock->getClass() != CLASS_WARLOCK)
                ...
                if (uint32 event_id = events.ExecuteEvent())
                {
					switch (event_id){
						case EVENT_CAST_SPELL:
							Player* warlock = me->GetOwner()->ToPlayer(); 
							warlock->MonsterYell("Cast spell",0,0);
							if (warlock && warlock->getClass() == CLASS_WARLOCK)
							{		
								warlock->MonsterYell("Entrando a warlock", 0, 0);
								Unit* newTarget = warlock->GetSelectedUnit();
								if (newTarget && warlock->IsValidAttackTarget(newTarget)){
									me->SetTarget(newTarget->GetGUID());
									if (me->GetEntry() == NPC_DOOM_GUARD)
										me->GetMotionMaster()->MoveChase(newTarget, 30.0f);
									me->ToCreature()->CastSpell(newTarget, spellId, true);										
								}

							}
							else
							{
								warlock->MonsterYell("No entrando a warlock", 0, 0);
								if (me->GetEntry() == NPC_DOOM_GUARD)
								{
									UnitList targets;
...
public:
	...
	struct npc_force_of_natureAI : public ScriptedAI
	{
		...
		void InitializeAI()
		{
			...
			if (owner == NULL || owner->GetTypeId() != TYPEID_PLAYER)
				return;

			uint8 owner_level = owner->getLevel();
			me->SetLevel(owner_level);
		}
		...
		void IsSummonedBy(Unit* owner) override
		{
			...
			else if (me->GetEntry() == ENTRY_TREANT_RESTO)
			{
				owner->CastSpell(owner, 142421, true); //Cast Swiftmend 
				owner->CastSpell(owner, 142423, true); //Cast Efflorescence
			}
		}

		void CastStartSpell(Unit* owner, Unit* targetOwner)
		{
			if (me->GetEntry() == ENTRY_TREANT_RESTO)
			{
				if (targetOwner->IsFriendlyTo(owner))
				{
					owner->CastSpell(targetOwner, 142421, true); //Cast Swiftmend 
					owner->CastSpell(targetOwner, 142423, true); //Cast Efflorescence
				}
				else
				{
					owner->CastSpell(owner, 142421, true); //Cast Swiftmend 
			...
			else if (targetOwner->IsFriendlyTo(owner))
				return;

			switch (me->GetEntry())
			{
			case ENTRY_TREANT_GUARDIAN:
				me->CastSpell(targetOwner, 113830, true); // Taunt
				break;
			case ENTRY_TREANT_FERAL:
			case ENTRY_TREANT_BALANCE:
				me->CastSpell(targetOwner, 113770, true); // Root
				// on creatures 30seconds
				if (targetOwner->GetTypeId() != TYPEID_PLAYER)
				if (AuraPtr root = targetOwner->GetAura(113770, me->GetGUID()))
                    ...
                    if (!firstCast && me->getVictim())
                    {
                        switch (me->GetEntry())
                        {
                           ...
                           case 1863:
                               ...
                               {
                                   me->getVictim()->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE, 0, me->getVictim()->GetAura(32409)); // Shawow Word : Death Self Damage not be removed.
                                   me->getVictim()->RemoveAurasByType(SPELL_AURA_PERIODIC_DAMAGE_PERCENT);
                                   me->getVictim()->RemoveAurasByType(SPELL_AURA_PERIODIC_LEECH);
                               }
                               break;
                           case 17252:
...
struct CharacterDonationPointsLog
{
    uint64 guid;
    uint64 account;
    time_t transaction_date;
    uint16 currency_type;
    uint16 convertion_rate;
    uint16 total_points_converted;
    uint16 total_points_before_trans;
    uint16 total_points_after_trans;
    std::ostringstream comment;
};

class npc_currency_trader : public CreatureScript
{
public:

    npc_currency_trader() : CreatureScript("npc_currency_trader") {	}

    struct npc_currency_traderAI : public ScriptedAI
    {
        npc_currency_traderAI(Creature* creature) : ScriptedAI(creature) {}

        std::map<uint64, uint8> current_menu;
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_currency_traderAI(creature);
    }

    void PrepareMenu(uint8 aMenuID, Player* player, Creature* creature)
    {
        //
        player->PlayerTalkClass->ClearMenus();
        //
        CAST_AI(npc_currency_trader::npc_currency_traderAI, creature->AI())->current_menu[player->GetGUIDLow()] = aMenuID;
        //
        switch (aMenuID)
        {
        case 0:
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, "Trade Conquest Points", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, "Trade Honor Point", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, "Trade Justice Point", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_VENDOR, "Trade Valor Point", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 4);
            player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
        } break;
        case 1:
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_DOT, "Trade one", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_MONEY_BAG, "Trade all", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 2);
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_TALK, "Back to Trade Menu", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 3);
            player->SEND_GOSSIP_MENU(player->GetGossipTextId(creature), creature->GetGUID());
        } break;
        case 2:
        {
    ...
    {
        //
        auto _current_menu_store = CAST_AI(npc_currency_trader::npc_currency_traderAI, creature->AI())->current_menu;
        //
        auto _current_menu = _current_menu_store.find(player->GetGUIDLow());
        //
        switch (_current_menu != _current_menu_store.end() ? _current_menu->second : 0)
        {
        case 0: // Base Menu
        {
            //
            ...
            {
            case GOSSIP_ACTION_INFO_DEF + 1:
                PrepareMenu(1, player, creature);
                break;
            case GOSSIP_ACTION_INFO_DEF + 2:
                PrepareMenu(2, player, creature);
                break;
            case GOSSIP_ACTION_INFO_DEF + 3:
                PrepareMenu(3, player, creature);
                break;
            case GOSSIP_ACTION_INFO_DEF + 4:
                PrepareMenu(4, player, creature);
                break;
            default:
                break;
            }
        ...
        case 1: // Menu 1
        {
            ...
            {
            case GOSSIP_ACTION_INFO_DEF + 1:
                TradeCurrecy(player, creature, 390);
                break;
            case GOSSIP_ACTION_INFO_DEF + 2:
                TradeCurrecy(player, creature, 390, true);
                break;
            case GOSSIP_ACTION_INFO_DEF + 3: // Return to Base Menu
                PrepareMenu(0, player, creature); return false;
                break;
            default:
                break;
            }
        ...
        case 2: // Menu 2
        {
            ...
            {
            case GOSSIP_ACTION_INFO_DEF + 1:
                TradeCurrecy(player, creature, 392);
                break;
            case GOSSIP_ACTION_INFO_DEF + 2:
                TradeCurrecy(player, creature, 392, true);
                break;
            case GOSSIP_ACTION_INFO_DEF + 3:
                PrepareMenu(0, player, creature); return false;
                break;
        ...
        case 3: // Menu 3
        {
            ...
            {
            case GOSSIP_ACTION_INFO_DEF + 1:
                TradeCurrecy(player, creature, 395);
                break;
            case GOSSIP_ACTION_INFO_DEF + 2:
                TradeCurrecy(player, creature, 395, true);
                break;
            case GOSSIP_ACTION_INFO_DEF + 3:
                PrepareMenu(0, player, creature); return false;
                break;
        ...
        case 4: // Menu 4
        {
            ...
            {
            case GOSSIP_ACTION_INFO_DEF + 1:
                TradeCurrecy(player, creature, 396);
                break;
            case GOSSIP_ACTION_INFO_DEF + 2:
                TradeCurrecy(player, creature, 396, true);
                break;
            case GOSSIP_ACTION_INFO_DEF + 3:
                PrepareMenu(0, player, creature); return false;
                break;
    ...
    /* Trade */
    void TradeCurrecy(Player* player, Creature* creature, uint32 currencyType, bool all = false)
    {
        std::string currecyName = "";
        // Create the store object
        CharacterDonationPointsLog donation_entry;
        // Set the convertion rate
        switch (currencyType)
        {
        case 390:
            donation_entry.convertion_rate = sWorld->getIntConfig(CONFIG_CURRENCY_TYPE_1_TRADE_RATE);
            currecyName = "Conquest Point";
            break;
        case 392:
            donation_entry.convertion_rate = sWorld->getIntConfig(CONFIG_CURRENCY_TYPE_2_TRADE_RATE);
            currecyName = "Honor Point";
            break;
        case 395:
            donation_entry.convertion_rate = sWorld->getIntConfig(CONFIG_CURRENCY_TYPE_3_TRADE_RATE);
            currecyName = "Justice Point";
            break;
        case 396:
            donation_entry.convertion_rate = sWorld->getIntConfig(CONFIG_CURRENCY_TYPE_4_TRADE_RATE);
            currecyName = "Valor Point";
            break;
        default: return;
        }
        // Cero means currency trade is disabled
        if (donation_entry.convertion_rate == 0)
        {
            // Print an error
            return;
        }
        //
        donation_entry.total_points_converted = all ? player->GetCurrency(currencyType, true) / donation_entry.convertion_rate : 1;
        //  
        if (!player->HasCurrency(currencyType, donation_entry.total_points_converted * donation_entry.convertion_rate * 100))
        {
            // Print an error
            return;
        }
        // Get the account ID
        donation_entry.account = player->GetSession()->GetAccountId();
        // Get the player GUID
        donation_entry.guid = player->GetGUIDLow();
        // Get the total currency from the player
        QueryResult tradeData = CharacterDatabase.PQuery("SELECT total_points FROM character_donation_points WHERE account = '%u'", donation_entry.account);
        // 
        if (tradeData)
        {
            // Fetch
            Field* fields = tradeData->Fetch();
            // Begin Transaction
            donation_entry.total_points_before_trans = fields[0].GetUInt64();
        }
        else
        {
            donation_entry.total_points_before_trans = 0;
        }
        //
        if (donation_entry.total_points_converted == 0)
        {
            // Print an error
            return;
        }
        // Set the currency type
        donation_entry.currency_type = currencyType;
        //
        donation_entry.total_points_after_trans = donation_entry.total_points_before_trans + donation_entry.total_points_converted;
        // Set the time to the transaction
        donation_entry.transaction_date = time(NULL);
        // Required to create de comment
        tm time_simple = *localtime(&donation_entry.transaction_date);
        // Create the comment
        donation_entry.comment
            << "Account: " << donation_entry.account
            << " charater guid: " << player->GetGUIDLow()
            << " charater name: " << player->GetName()
            << " transaction date " << time_simple.tm_year + 1900 << "/" << time_simple.tm_mon << "/" << time_simple.tm_mday << " @ " << time_simple.tm_hour << ":" << time_simple.tm_min << ":" << time_simple.tm_sec
            << " has trade: " << donation_entry.total_points_converted  * donation_entry.convertion_rate << " " << currecyName << (donation_entry.total_points_converted  * donation_entry.convertion_rate > 1 ? "s" : "")
            << " total aquired: " << donation_entry.total_points_converted
            << " convertion rate: " << donation_entry.convertion_rate
            << " total points before: " << donation_entry.total_points_before_trans
            << " total points after: " << donation_entry.total_points_after_trans;
        //
        std::ostringstream ss, ss1;
        ss
            << "INSERT INTO character_donation_points_logs VALUES (NULL,"
            << donation_entry.guid << ","
            << donation_entry.account << ","
            << uint64(donation_entry.transaction_date) << ","
            << donation_entry.currency_type << ","
            << donation_entry.convertion_rate << ","
            << donation_entry.total_points_converted << ","
            << donation_entry.total_points_before_trans << ","
            << donation_entry.total_points_after_trans << ",\""
            << donation_entry.comment.str() << "\");\n";
        ...
        //
        if (donation_entry.total_points_before_trans != 0)
            ss1 << "UPDATE character_donation_points SET total_points=" << donation_entry.total_points_after_trans << " WHERE(account=" << donation_entry.account << ");";
        else
            ss1 << "INSERT INTO character_donation_points (account, total_points, comment) VALUES(" << donation_entry.account << ", " << donation_entry.total_points_after_trans << "," << "null);";
        // Create the trasaction to save the total ammount
        trans = CharacterDatabase.BeginTransaction();
        //
        trans->Append(ss1.str().c_str());
        // Commit the transaction
        CharacterDatabase.CommitTransaction(trans);
        // Discount the currency
        player->ModifyCurrency(donation_entry.currency_type, -(donation_entry.total_points_converted * donation_entry.convertion_rate * 100), true, true);
        // Create a transaction to save the currency
        trans = CharacterDatabase.BeginTransaction();
        // Prepare the transaction
        player->ForcedSaveCurrency(trans);
        // Commit the transaction
        CharacterDatabase.CommitTransaction(trans);
    }
};
...
void AddSC_npcs_special()
{
    ...
    new npc_skull_banner();
    new npc_mocking_banner();
    new npc_frozen_orb();
