...
#define EMOTE_ACHIEVE_STARTED "ACHIEVE RACE STARTED"
#define EMOTE_ACHIEVE_FAILED "ACHIEVE RACE ENDEND"
    ...
    struct npc_glintrok_scout_2AI : public ScriptedAI
    {
        ...
        void MoveInLineOfSight(Unit* pWho)
        {
			...
			if (pWho->GetTypeId() == TYPEID_PLAYER && pWho->GetDistance(me) < 25.0f)
            {
				...
				if (GetDifficulty() == DUNGEON_DIFFICULTY_HEROIC)
				{
					...
					{
						instance->SetData(DATA_ACHIEVE, IN_PROGRESS);
						instance->DoSendNotifyToInstance(EMOTE_ACHIEVE_STARTED);
        ...
        void UpdateAI(const uint32 diff)
        {
            if (!UpdateVictim())
				...
				{
					Timer = 300000;
					me->GetInstanceScript()->SetData(DATA_ACHIEVE, FAIL);
					me->GetInstanceScript()->DoSendNotifyToInstance(EMOTE_ACHIEVE_FAILED);
