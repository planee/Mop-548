...
enum Spells
{
    ...
    SPELL_BREW_MISI_DARK    = 128258, // Visual Launch of summon first wave spawn
    SPELL_BREW_MISI_MEDIUM  = 128256, // Visual Launch of summon second wave spawn
    SPELL_BREW_MISI_WHEAT   = 128254, // Visual Launch of summon third wave spawn
};
...
class npc_uncle_gao : public CreatureScript
{
    public :
        ...
        struct npc_uncle_gao_AI : public ScriptedAI
        {
            ...
            {
                if (!introStarted && me->FindNearestPlayer(25.0f, true))
                    ...
                    switch(eventId)
                    {
                        ...
                        case EVENT_UNCLE_GAO_INTRO_2:
                            if (Creature* bunny = me->FindNearestCreature(NPC_CAULDRON_BUNNY, 10.0f, true))
                            {
                                for (uint8 i = 0; i < 2; i++)
                                {
                                    bunny->CastSpell(AddWavesPositions[i], SPELL_BREW_MISI_DARK, true);
                                    me->SummonCreature(RandomStoutAbility == ABILITY_BLOAT ? NPC_BLOATED_BREW_ALEMENTAL : NPC_STOUT_BREW_ALEMENTAL, AddWavesPositions[i], TEMPSUMMON_CORPSE_DESPAWN);
                                }
                            }                                
                            break;
                        ...
                        case EVENT_UNCLE_GAO_INTRO_5:
                            if (Creature* bunny = me->FindNearestCreature(NPC_CAULDRON_BUNNY, 10.0f, true))
                            {
                                ...
                                {
                                    bunny->CastSpell(AddWavesPositions[i], SPELL_BREW_MISI_MEDIUM, true);
                                    me->SummonCreature(RandomAleAbility == ABILITY_BUBBLE_SHIELD ? NPC_BUBBLING_BREW_ALEMENTAL : NPC_YEASTY_BREW_ALEMENTAL, AddWavesPositions[i], TEMPSUMMON_CORPSE_DESPAWN);
                                }
                            }
                            break;
                        ...
                        case EVENT_UNCLE_GAO_INTRO_8:
                            if (Creature* bunny = me->FindNearestCreature(NPC_CAULDRON_BUNNY, 10.0f, true))
                            {
                                ...
                                {
                                    bunny->CastSpell(AddWavesPositions[i], SPELL_BREW_MISI_WHEAT, true);
                                    me->SummonCreature(RandomAleAbility == ABILITY_WALL_OF_SUDS ? NPC_SUDSY_BREW_ALEMENTAL : NPC_FIZZY_BREW_ALEMENTAL, AddWavesPositions[i], TEMPSUMMON_CORPSE_DESPAWN);
                                }
                            }
                            break;
    ...
    public :
        ...
        struct boss_yan_zhu_the_uncasked_AI : public BossAI
        {
            ...
            void JustSummoned(Creature* summon)
            {
                ...
                if (summon->GetEntry() != NPC_BLOATED_STALKER)
                summon->setActive(true);

                if (me->isInCombat() && summon->GetEntry() != NPC_BLOATED_STALKER)
                ...
                while(uint32 eventId = events.ExecuteEvent())
                {
                    switch(eventId)
                    {
                        ...
                        case EVENT_SET_BUBBLE_SHIELD_STACKS:
                            ...
                            SpawnInCircle(4.0f, 8, NPC_BUBBLE_SHIELD, TEMPSUMMON_MANUAL_DESPAWN);
        ...
        void SpawnInCircle(float row, uint8 summonNumber, uint32 unitEntry, TempSummonType summonType = TEMPSUMMON_MANUAL_DESPAWN, uint32 despawnTimer = 30000)
        {
            ...
            for (uint8 i = 0 ; i < summonNumber ; i++)
            {
                ...
                if (summonType = TEMPSUMMON_MANUAL_DESPAWN)
                    me->SummonCreature(unitEntry, x, y, me->GetPositionZ() + 5.0f, 0, summonType);
                else
                    me->SummonCreature(unitEntry, x, y, me->GetPositionZ() + 5.0f, 0, summonType, despawnTimer);
            }
        }
...
class npc_fizzy_bubble_yanzhu : public CreatureScript
{
    public:
        ...
        struct npc_fizzy_bubble_yanzhuAI : public PassiveAI
        {
            ...
            {
                creature->SetUnitMovementFlags(MOVEMENTFLAG_HOVER);
...
class spell_yanzhu_bloat : public SpellScriptLoader
{
    public:
        ...
        class spell_yanzhu_bloat_AuraScript : public AuraScript
        {
                ...
                if (GetCaster() && GetTarget())
                {
                    if (Unit* caster = GetCaster())
                    {
                        if (Unit* target = GetTarget())
                        {
                            ...
                            for (uint8 i = 0; i < 2; i++)
                            {
                                if (bloatStalker = caster->SummonCreature(NPC_BLOATED_STALKER, target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), 0, TEMPSUMMON_MANUAL_DESPAWN))
                                {
                                    if (AuraPtr bloat = caster->AddAura(SPELL_BLOATED, bloatStalker))
                                    {
            ...
            void AfterVehicleApply(constAuraEffectPtr aurEff, AuraEffectHandleModes mode)
            {
                if (GetCaster() && GetTarget())
                {
                        if (Unit* target = GetTarget())
                        {
                            ...
                            GetCreatureListWithEntryInGrid(creatures, target, NPC_BLOATED_STALKER, 100.0f);
                            ...
                            for (std::list<Creature*>::iterator iter = creatures.begin(); iter != creatures.end(); ++iter)
                            {
                                ...
                                {  
                                    ...
                                    (*iter)->DespawnOrUnsummon(31000);
                                    (*iter)->EnterVehicle(target); // Enter player.
            ...
            void Register()
            {                
                OnEffectApply += AuraEffectApplyFn(spell_yanzhu_bloat_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_SCALE, AURA_EFFECT_HANDLE_REAL);
                AfterEffectApply += AuraEffectApplyFn(spell_yanzhu_bloat_AuraScript::AfterVehicleApply, EFFECT_1, SPELL_AURA_SET_VEHICLE_ID, AURA_EFFECT_HANDLE_REAL);
            }
        };
...
class spell_yanzhu_carbonation : public SpellScriptLoader
{
    public:
        ...
        class spell_yanzhu_carbonation_AuraScript : public AuraScript
        {
            ...
            {
                if (Unit* caster = GetCaster())
                        caster->SummonCreature(NPC_FIZZY_BUBBLE, caster->GetPositionX() + frand(-13.0f, 13.0f), caster->GetPositionY() + frand(-13.0f, 13.0f), caster->GetPositionZ() + 2.0f,  caster->GetOrientation(), TEMPSUMMON_MANUAL_DESPAWN);
            }
            ...
            void OnRemove(constAuraEffectPtr aurEff, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    if (caster->HasAura(SPELL_ROOT))
                        caster->RemoveAura(SPELL_ROOT);

                    std::list<Creature*> creatures;
                    GetCreatureListWithEntryInGrid(creatures, caster, NPC_FIZZY_BUBBLE, 100.0f);
            ...
            void Register()
            {
                ...
                OnEffectRemove += AuraEffectRemoveFn(spell_yanzhu_carbonation_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL, AURA_EFFECT_HANDLE_REAL);
            }
        };
        ...
        class spell_yanzhu_carbonation_damage_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
				...
				if (map && map->IsDungeon())
				{
					...
					for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
						if (Player* player = itr->getSource())
							if (!(player->HasAura(SPELL_FIZZY_BUBBLE)))
								targets.push_back(player);
				}
            }
