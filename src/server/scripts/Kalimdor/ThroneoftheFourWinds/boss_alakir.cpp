...
enum ScriptTextAnshal
{
	SAY_AGGRO				 = 0,
	SAY_WIND_BURST           = 1,
	SAY_SUMMON_STORMLING     = 2,
	SAY_STORMLING_PHASE		 = 3,
	SAY_WEATHER              = 4,
	SAY_KILL                 = 5,
	SAY_DEATH				 = 6,
};
...
enum Spells
{
	...
	SPELL_ICE_STORM_DPS                 = 87469,
	SPELL_ICE_STORM_RAIN                = 88239,
	...
	SPELL_WEATHER_HEAVY_RAIN            = 82651, 
};
...
public:
    ...
    struct boss_alakirAI : public BossAI
    {
        ...
        void Reset()
        {
			...
			instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_ACID_RAIN); // Remove just on Reset
			instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_WEATHER_HEAVY_RAIN);
			DespawnCreatures(NPC_STORMLING);
			DespawnCreatures(NPC_SQUILL_1);
			DespawnCreatures(NPC_SQUILL_2);
			DespawnCreatures(NPC_ICE_STORM);
			DespawnCreatures(NPC_ICE_STORM_RAIN);
			DespawnCreatures(NPC_STORMLING_PRE_EFFECT);
        }
        ...
        void JustDied(Unit* Killer)
        {
			...
			if (instance)
			{
				...
				//Achievement Throne of the Four Winds 10, 25, 10H, 25H
				if (map->GetDifficulty() == RAID_DIFFICULTY_10MAN_NORMAL)
				{
					instance->DoCompleteAchievement(4851);
				}
				...
				{
					instance->DoCompleteAchievement(4851);
					//Achievement Al'Akir Heroic kills (Throne of the Four Winds)
					instance->DoCompleteAchievement(5577);
				}
				...
				if (Creature* boss = me->FindNearestCreature(NPC_ALAKIR, 500.0f, true))
				if (boss->HasAura(SPELL_FEEDBACK))
					instance->DoCompleteAchievement(5305);
			}
			summons.DespawnAll();
        }
        ...
        void DamageTaken(Unit* /*damageDealer*/, uint32& damage)
        {
            ...
            if (!HealthAbovePct(25) && phase == PHASE_STORMLING)
            {
                ...
                me->RemoveAurasDueToSpell(SPELL_ACID_RAIN);
				me->SetFloatValue(UNIT_FIELD_COMBAT_REACH, 135.0f);
				me->SetFloatValue(UNIT_FIELD_BOUNDING_RADIUS, 135.0f);
		...
		void ChangeWeather()
		{
			...
			if (!PlayerList.isEmpty())
			{
				...
				{
					Player *player = i->getSource();
					if (!player->HasAura(SPELL_WEATHER_HEAVY_RAIN))
						player->AddAura(SPELL_WEATHER_HEAVY_RAIN, player);
				}
			}
		}
                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_STATIC_SHOCK:
							if (me->IsWithinMeleeRange(me->getVictim()))
							{
								...
								events.RescheduleEvent(EVENT_STATIC_SHOCK, 5000);
							}
                            events.RescheduleEvent(EVENT_STATIC_SHOCK, 5000);
                            break;
                        ...
                        case EVENT_ICE_STORM:
                            if (Unit *target = SelectTarget(SELECT_TARGET_RANDOM, 0, 24.0f, true))
                            {
								...
								me->SummonCreature(NPC_ICE_STORM_RAIN, SquallLinePos[1], TEMPSUMMON_TIMED_DESPAWN, 15000); 
							}
...
public:
    ...
    struct npc_ice_stormAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			me->CastSpell(me, SPELL_ICE_STORM_DPS, true);
...
public:
    ...
    struct npc_stormlingAI : public ScriptedAI
    {
        ...
        {
			if (Creature* boss = me->FindNearestCreature(NPC_ALAKIR, 500.0f, true))
				boss->AddAura(SPELL_FEEDBACK, boss);
        }
    ...
    public:
        ...
        class spell_lightning_strike_effect_SpellScript : public SpellScript
        {
            ...
            void HandleAfterCast()
            {
                ...
                WoWSource::AnyPlayerInObjectRangeCheck checker(GetCaster(), 20.0f);
				WoWSource::PlayerListSearcher<WoWSource::AnyPlayerInObjectRangeCheck> searcher(GetCaster(), players, checker);
...
void AddSC_boss_alakir()
{
    ...
    new npc_stormling();
    //new spell_electrocute(); Fix this when the hook work again
