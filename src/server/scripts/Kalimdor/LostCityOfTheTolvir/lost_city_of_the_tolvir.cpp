		...
		void UpdateAI(const uint32 diff)
		{
			...
			for (std::list<SlipstreamPlayer>::iterator itr = lPlayers.begin(); itr != lPlayers.end();)
			{
				if ((*itr).uiPersonalTimer <= diff)
				{
					...
					{
						if (Creature* Landing = player->FindNearestCreature(48097, 100.0f))
						{
							player->NearTeleportTo(Landing->GetPositionX(), Landing->GetPositionY(), Landing->GetPositionZ(), Landing->GetOrientation());
						}
					}
