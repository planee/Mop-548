	...
	struct boss_lockmawAI : public ScriptedAI
	{
		...
		void JustDied(Unit* /*Killer*/)
		{
			...
			if (instance)
			{
				...
				//No es necesario comprobar si esta en heroico pues este Augh solo tiene Spawn en heroico 
				//y si no lo encuentra no entra en el if
				if (Creature* augh = me->FindNearestCreature(49045, 300.0f))
				{
					me->loot.clear(); //para que el loot lo de Augh
	...
	struct boss_aughAI : public ScriptedAI
	{
		...
		void DoAction(const int32 action)
		{
			if (action == ACTION_LOCKMAW_IS_DONE)
			{
				...
				//Los Textos de Augh mientras camina
		...
		{
			//Una vez que alcance su posicion se vuelve atacable
		...
		void UpdateAI(const uint32 diff)
		{
			if (Intro)
			{
				//el intro este no pincha, lo deje por si acaso pero con los textos de all
