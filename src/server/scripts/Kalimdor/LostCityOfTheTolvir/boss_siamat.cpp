...
public:
	...
	struct boss_siamatAI : public ScriptedAI
	{
		...
		void EnterCombat(Unit* /*who*/)
		{
			...
			events.ScheduleEvent(EVENT_CALL_OF_SKY, 3000);
			...
			events.ScheduleEvent(EVENT_STORM_BOLT_DW, 8000, 0, PHASE_DEFLECTING_WINDS);
		...
		{
			//Para invocar los 3 sirvientes, cancela el anterior pues se ejecutara inmediatamente y programa un nuevo evento
		...
		void JustDied(Unit* /*killer*/)
		{
			events.Reset();
			lSummons.DespawnAll();
			DespawnCreatures(44713);//para quitar las nubes que quedan
			//esto esta bien en el server de testeo pero lo agregue como prueba asi q ignorar
			if (instance)
				if (IsHeroic())
				{
					instance->DoCompleteAchievement(5066);
					instance->DoCompleteAchievement(4848);
		...
		void UpdateAI(const uint32 diff)
		{
			...
			if (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_STATIC_SHOCK:
				{
					...
					++uiStaticShockId;
					events.ScheduleEvent(EVENT_STATIC_SHOCK, 45000, 0, PHASE_DEFLECTING_WINDS);
				...
				case EVENT_CALL_OF_SKY:
				{
					...
					events.ScheduleEvent(EVENT_CALL_OF_SKY, 30000, 0);
				}
				break;
				...
				{
					if (Creature* cloud = me->FindNearestCreature(44713, 100.0f))
					{
						me->CastSpell(cloud, SPELL_ABSORB_STORMS, false);
						cloud->DespawnOrUnsummon(2000);
					}
		...
		void DamageTaken(Unit*, uint32 &damage)
		{
			...
			if (damage >= me->GetHealth())
			{
				...
				if (!LightningCharge)
				{
					...
					me->AttackStop();
					events.Reset();
					events.ScheduleEvent(EVENT_SERVANT_DIED, 1000);
				}
			}
		}
