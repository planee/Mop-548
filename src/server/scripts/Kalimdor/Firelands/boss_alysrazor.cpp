...
Position const AlysrazorSpawnPos = { 49.160f, -342.090f, 64.9282f, 2.591995f };
...
class mob_alysrazor_strangle_aly_gauntlet : public CreatureScript
{
public:
	mob_alysrazor_strangle_aly_gauntlet() : CreatureScript("mob_alysrazor_strangle_aly_gauntlet") { }

	struct mob_alysrazor_strangle_aly_gauntletAI : public ScriptedAI
	{
		mob_alysrazor_strangle_aly_gauntletAI(Creature* creature) : ScriptedAI(creature), _isGauntletInProgress(false)
		{
		...
		void EnterCombat(Unit* who) 
		{
			if (who->GetTypeId() == TYPEID_PLAYER && !_isGauntletInProgress)
				{
					_isGauntletInProgress = true;
					DoAction(ACTION_START_GAUNTLET);
					me->SetReactState(REACT_PASSIVE);
					me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NON_ATTACKABLE);
				}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			if (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_TRANSFORM_HAWK:
					...
					break;
				case EVENT_FLY_UP:
					me->SummonCreature(NPC_ALYSRAZOR, AlysrazorSpawnPos);
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new mob_alysrazor_strangle_aly_gauntletAI(creature);
	}
};
...
void AddSC_boss_alysrazor()
{
	...
	/* Event Starter */
	new mob_alysrazor_strangle_aly_gauntlet();
