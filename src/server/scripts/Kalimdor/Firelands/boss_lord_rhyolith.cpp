    ...
    public:
        ...
        struct boss_lord_rhyolithAI : public BossAI
        {
            ...
            void EnterCombat(Unit* attacker)
            {
				...
				if (Controller)
                {
					GuidController = Controller->GetGUID();
				...
				if (RightFoot)
					GuidRightFoot = RightFoot->GetGUID();

				if (LeftFoot)
					GuidLeftFoot = LeftFoot->GetGUID();
            ...
            {
				Creature* LeftFoot = Unit::GetCreature(*me, GuidLeftFoot);
				Creature* RightFoot = Unit::GetCreature(*me, GuidRightFoot);
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (me->HealthBelowPct(25) && phase == 0)
                {
                    ...
                    if (Creature* pRhyolith = me->SummonCreature(NPC_RHYOLITH_2, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ()))
                    {
						...
						if (Creature* controller = me->FindNearestCreature(NPC_RHYOLITH, 150.0f, true))
							controller->DespawnOrUnsummon();
						if (Creature* rightFoot = me->FindNearestCreature(NPC_RIGHT_FOOT, 150.0f, true))
							rightFoot->DespawnOrUnsummon();
						if (Creature* leftFoot = me->FindNearestCreature(NPC_LEFT_FOOT, 150.0f, true))
					...
					Creature* LeftFoot = Unit::GetCreature(*me, GuidLeftFoot);
					Creature* RightFoot = Unit::GetCreature(*me, GuidRightFoot);
					Creature* Controller = Unit::GetCreature(*me, GuidController);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
							if (Creature* Controller = Unit::GetCreature(*me, GuidController))
							{
								if (me->GetDistance2d(Controller) <= 0.5f)
                                {
                                    me->StopMoving();
							...
							if (Controller && (LeftFoot && LeftFoot->IsAIEnabled) && (RightFoot && RightFoot->IsAIEnabled))
                            {
        ...
        private:
			uint64 GuidController;
			uint64 GuidRightFoot;
			uint64 GuidLeftFoot;
    ...
    public:
        ...
        struct npc_lord_rhyolith_rhyolithAI : public ScriptedAI
        {
            ...
            void Reset()
            {
				if (pInstance)
				{
					if (pInstance->GetBossState(DATA_RHYOLITH) == DONE)
						me->DespawnOrUnsummon();
				}
            ...
            void EnterCombat(Unit* attacker)
            {
				...
				if (pInstance)
				{
					pInstance->SetBossState(DATA_RHYOLITH, IN_PROGRESS);
				}
