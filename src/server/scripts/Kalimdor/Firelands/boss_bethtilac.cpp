...
enum Spells
{
	...
	SPELL_CONSUME = 99332, // phase 2 also
	SPELL_SMOLDERING_DEVASTATION = 99052,

	// Phase 2
	SPELL_FRENZY = 99497,
	SPELL_THE_WIDOWS_KISS = 99506, //99476
	...
	SPELL_CONSUME_DRONE = 100634,
	SPELL_BURNING_ACID_DRONE = 99934,
	SPELL_FIXATE_H = 99559,
	...
	struct boss_bethtilacAI : public ScriptedAI
	{
		...
		void Reset()
		{
			if (_moveup)
			{
				...
				me->GetMotionMaster()->MoveJump(downPos, 37.0f, 0.0f);
				_moveup = false;
				return;
			}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_ENERGY_DRAIN:
					if (me->GetPower(POWER_ENERGY) == 0)
					{
						...
						if (Devastion == 3)
						{
							...
							events.ScheduleEvent(EVENT_PHASE_LOWER, 3000);
							break;
						}
					}
					else
				...
				case EVENT_PHASE_LOWER:
					...
					events.ScheduleEvent(EVENT_BETHILAC_CONSUME, 20000);
...
{
public:
	npc_bethtilac_cinderweb_spinner() :
		CreatureScript("npc_bethtilac_cinderweb_spinner")
	{
	}

	struct npc_bethtilac_cinderweb_spinnerAI : public ScriptedAI
	{
		npc_bethtilac_cinderweb_spinnerAI(Creature* creature) :
			ScriptedAI(creature)
		{
		}
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_bethtilac_cinderweb_spinnerAI(creature);
	}
};
...
{
public:
	npc_bethtilac_cinderweb_drone() :
		CreatureScript("npc_bethtilac_cinderweb_drone")
	{
	}

	struct npc_bethtilac_cinderweb_droneAI : public ScriptedAI
	{
		npc_bethtilac_cinderweb_droneAI(Creature* creature) :
			ScriptedAI(creature)
		{
		}
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_DRONE_BURNING_ACID:
					if (me->GetPower(POWER_ENERGY) > 0)
					{
						...
						events.ScheduleEvent(EVENT_DRAIN_ENERGY, 300);
					break;
				case EVENT_BOILING_SPLATTER:
					if (me->GetPower(POWER_ENERGY) > 0)
					{
						DoCastAOE(SPELL_BOILING_SPLATTER);
						events.ScheduleEvent(EVENT_BOILING_SPLATTER, urand(25000, 35000));
						me->SetPower(POWER_ENERGY, me->GetPower(POWER_ENERGY) - 15);
					}
					else
						events.ScheduleEvent(EVENT_DRAIN_ENERGY, 300);
					break;
				case EVENT_CONSUME:
					if (Creature* spiderling = me->FindNearestCreature(NPC_CINDERWEB_SPIDERLING, 5.0f, true))
					{
						me->AddAura(SPELL_CONSUME_DRONE, me);
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_bethtilac_cinderweb_droneAI(creature);
	}
};
...
{
public:
	npc_bethtilac_cinderweb_spiderling() :
		CreatureScript("npc_bethtilac_cinderweb_spiderling")
	{
	}

	struct npc_bethtilac_cinderweb_spiderlingAI : public ScriptedAI
	{
		npc_bethtilac_cinderweb_spiderlingAI(Creature* creature) :
			ScriptedAI(creature)
		{
		}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_CHECK_PLAYER_SEEPING_VENOM:
					...
					{
						if (AuraPtr aura = target->GetAura(SPELL_SEEPING_VENOM))
						{
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_bethtilac_cinderweb_spiderlingAI(creature);
	}
};
...
{
public:
	npc_bethtilac_engorged_broodling() :
		CreatureScript("npc_bethtilac_engorged_broodling")
	{
	}

	struct npc_bethtilac_engorged_broodlingAI : public ScriptedAI
	{
		npc_bethtilac_engorged_broodlingAI(Creature* creature) :
			ScriptedAI(creature)
		{
		}
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_bethtilac_engorged_broodlingAI(creature);
	}
};
...
public:
	...
	class spell_bethtilac_smoldering_devastation_SpellScript : public SpellScript
	{
		...
		void FilterTargets(std::list<WorldObject*>& targets)
		{
			...
			targets.remove_if(PlayerPositionCheck(false));
		}
...
public:
	...
	class spell_bethtilac_burning_acid_SpellScript : public SpellScript
	{
		...
		void FilterTargets(std::list<WorldObject*>& targets)
		{
			...
			targets.remove_if(PlayerPositionCheck(true));
		}
