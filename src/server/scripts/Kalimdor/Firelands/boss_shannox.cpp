        ...
        struct boss_shannoxAI : public BossAI
        {
            ...
            void Reset()
            {
				...
				if (GetRiplimb())  // Prevents Crashes
				{
					...
					GetRiplimb()->AI()->Reset();
					GetRiplimb()->GetMotionMaster()->MoveFollow(me, walkRiplimbDistance, walkRiplimbAngle);
				}
				else
				{
				...
				if (GetRageface())  // Prevents Crashes
				{
					...
					GetRageface()->GetMotionMaster()->MoveFollow(me, walkRagefaceDistance, walkRagefaceAngle);
				}
				else
				{
            ...
            void UpdateAI(const uint32 diff)
            {
				...
				if (((GetRiplimb() && GetRiplimb()->GetDistance2d(me) >= maxDistanceBetweenShannoxAndDogs
					...
					&& GetRageface()->isAlive()) && (!me->HasAura(SPELL_SEPARATION_ANXIETY)))
    ...
    public:
        ...
        struct npc_shannox_riplimbAI : public ScriptedAI
        {
            ...
            void Reset()
            {
				...
				frenzy = false; // Is needed, that Frenzy is not casted twice on Rageface

				if (pInstance)
				{
					pInstance->SetData(DATA_SHANNOX, NOT_STARTED);
					pInstance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, me); // Remove Frame
				}
            }
            ...
            void UpdateAI(const uint32 diff)
            {
				...
				if (GetShannox())
				{
					...
					{
						frenzy = true;
						DoCast(me, SPELL_FRENZIED_DEVOTION);
					}

					if (GetShannox()->GetDistance(me) >= maxDistanceBetweenShannoxAndDogs
						&& !me->HasAura(SPELL_SEPARATION_ANXIETY)) //TODO Sniff right Distance
						DoCast(me, SPELL_SEPARATION_ANXIETY);
				}
        ...
        struct npc_shannox_ragefaceAI : public ScriptedAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
				...
				if (GetShannox())
				{
					...
					if (GetShannox()->GetDistance(me) >= maxDistanceBetweenShannoxAndDogs
						&& !me->HasAura(SPELL_SEPARATION_ANXIETY)) //
					{
						DoCast(me, SPELL_SEPARATION_ANXIETY);
					}
