...
Position const ShannoxSpawnPos = { -49.59899f, -59.491199f, 57.687099f, 3.330400f };
    ...
    public:
        ...
        struct instance_firelands_InstanceMapScript : public InstanceScript
        {
			...
			void OnUnitDeath(Unit* unit)
			{
				...
				{
					// This will make sure that the counter doesnt move if an event is in progress (mobs)
					if (GetBossState(DATA_ALYSRAZOR) != IN_PROGRESS && GetBossState(DATA_BETHTILAC) != IN_PROGRESS)
						shannoxCounter++;

					if (isShannoxSpawned == false && shannoxCounter == 25)
					{
						isShannoxSpawned = true;
						/* Getting the dogs to move */
						if (Creature* shannox = instance->SummonCreature(NPC_SHANNOX, ShannoxSpawnPos))
						{
							if (Creature* riplimb = shannox->FindNearestCreature(NPC_RIPLIMB, 5000.0f, true))
							{
								riplimb->GetMotionMaster()->MoveFollow(shannox, walkRiplimbDistance, walkRiplimbAngle);
							}
								
							if (Creature* rageface = shannox->FindNearestCreature(NPC_RAGEFACE, 5000.0f, true))
							{
								rageface->GetMotionMaster()->MoveFollow(shannox, walkRagefaceDistance, walkRagefaceAngle);
							}
						}
					}
				}
            ...
            private:
                ...
                uint32 uiTimer;
                bool bEvent;
				bool isShannoxSpawned;
				uint32 shannoxCounter;
