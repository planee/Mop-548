...
enum Spells
{
	...
	SPELL_ARCANE_BARRAGE_MOB    = 81013,
};
...
enum Events
{
    ...
    EVENT_ARCANE_BARRAGE    = 6,
	EVENT_ARCANE_BURST      = 7
};
...
enum Adds
{
	...
	NPC_SPATIAL_FLUX            = 39612, // spawnmask=0
...
public:
	...
	{
		npc_spatial_anomalyAI(Creature* pCreature) : ScriptedAI(pCreature)
		{
			me->ApplySpellImmune(0, IMMUNITY_EFFECT, SPELL_EFFECT_KNOCK_BACK, true);
			me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_GRIP, true);
			me->ApplySpellImmune(0, IMMUNITY_MECHANIC, MECHANIC_STUN, true);
		...
		void EnterCombat(Unit* who)
		{
			events.ScheduleEvent(EVENT_ARCANE_BURST, urand(14000, 14500));
		}
...
class npc_flux_animator : public CreatureScript
{
public:
	npc_flux_animator() : CreatureScript("npc_flux_animator") {}

	struct npc_flux_animatorAI : public ScriptedAI
	{
		npc_flux_animatorAI(Creature *c) : ScriptedAI(c) {}
		...
		void UpdateAI(const uint32 diff)
		{
			...
			if (barrage <= diff)
			{
				if (Unit *pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 100, true))
					DoCast(pTarget, SPELL_ARCANE_BARRAGE_MOB);
				barrage = 8000;
			}
			else barrage -= diff;
	...
	CreatureAI* GetAI(Creature* pCreature) const
	{
		return new npc_flux_animatorAI(pCreature);
	}
