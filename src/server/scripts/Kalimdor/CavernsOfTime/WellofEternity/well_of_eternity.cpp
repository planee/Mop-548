    ...
    public:
        ...
        struct npc_well_of_eternity_legion_demonAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* who)
            {
				...
				events.ScheduleEvent(EVENT_CRUSHING_LEAP, urand(15000, 20000));
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CRUSHING_LEAP:
                           	DoCastAOE(SPELL_CRUSHING_LEAP_DMG);
							events.ScheduleEvent(EVENT_CRUSHING_LEAP, urand(10000, 15000));
                            break;
                        default:
                            break;
                    }
    ...
    public:
        ...
        struct npc_well_of_eternity_illidan_2AI : public ScriptedAI
        {
            npc_well_of_eternity_illidan_2AI(Creature* pCreature) : ScriptedAI(pCreature)
            {
                ...
                doomguards = 0;
                pInstance = me->GetInstanceScript();
				SetEquipmentSlots(false, 30208, 30209, 0);
            }
            ...
            void MoveInLineOfSight(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_MALFURION_INTRO_1, 4000);
            }
            ...
            void DoAction(const int32 action)
            {
                ...
                else if (action == ACTION_MANNOROTH_AGGRO)
                {
					...
					me->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
					me->AI()->DoAction(ACTION_MANNOROTH_COMBAT);
                }
                else if (action == ACTION_MANNOROTH_RESET)
                {
                    ...
                    events.Reset();
					//events.CancelEvent(ACTION_MANNOROTH_COMBAT);
                }
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MALFURION_INTRO_1:
                            ...
                            events.ScheduleEvent(EVENT_MALFURION_INTRO_2, 6000);
                            break;
                        case EVENT_MALFURION_INTRO_2:
                            if (Creature* pMalfurion = me->FindNearestCreature(NPC_MALFURION, 100.0f))
                                pMalfurion->AI()->Talk(SAY_MALFURION_INTRO_2);
                            events.ScheduleEvent(EVENT_ILLIDAN_INTRO_2_2, 5000);
                            break;
                        case EVENT_ILLIDAN_INTRO_2_2:
                            Talk(SAY_ILLIDAN_2_INTRO_2);
                            events.ScheduleEvent(EVENT_TYRANDE_INTRO_1, 5000);
                            break;
                        case EVENT_TYRANDE_INTRO_1:
                            if (Creature* pTyrande = me->FindNearestCreature(NPC_TYRANDE, 100.0f))
                                pTyrande->AI()->Talk(SAY_TYRANDE_INTRO_1);
                            events.ScheduleEvent(EVENT_ILLIDAN_INTRO_2_3, 7000);
                            break;
                        case EVENT_ILLIDAN_INTRO_2_3:
                            Talk(SAY_ILLIDAN_2_INTRO_3); 
                            events.ScheduleEvent(EVENT_TYRANDE_INTRO_2, 3000);
                            break;
                        ...
                        case EVENT_ILLIDAN_MOVE_2_9:
							...
							events.ScheduleEvent(ACTION_MANNOROTH_COMBAT, urand(8000, 12000));
							break;
						case ACTION_MANNOROTH_COMBAT:
							if (Creature* pBoss = me->FindNearestCreature(NPC_MANNOROTH, 100.0f))
							{
								DoCast(pBoss, SPELL_DARKLANCE);
								me->AddAura(SPELL_AURA_OF_IMMOLATION, me);
								DoCast(pBoss, SPELL_DEMON_RUSH);
								events.ScheduleEvent(ACTION_MANNOROTH_COMBAT, urand(8000, 12000));
								break;
							}
                        default:
