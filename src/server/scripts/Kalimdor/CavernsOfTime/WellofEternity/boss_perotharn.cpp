        ...
        struct boss_perotharnAI : public BossAI
        {
            ...
            void UpdateAI(const uint32 diff)
            {
				...
				if (me->HealthBelowPct(20) && !bEnraged)
				{
					bEnraged = true;
					Talk(SAY_RAGE);
					DoCast(me, SPELL_ENDLESS_RAGE, true);
						events.ScheduleEvent(EVENT_CORRUPTING_TOUCH, urand(18000, 20000));
						events.ScheduleEvent(EVENT_FEL_FLAMES, urand(21000, 25000));
...

                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CORRUPTING_TOUCH:
                            DoCastVictim(SPELL_CORRUPTING_TOUCH);
                            events.ScheduleEvent(EVENT_CORRUPTING_TOUCH, urand(29000, 31000)); 
                            break;
                        case EVENT_FEL_FLAMES:
							if (Unit* pTarget = SelectTarget(SELECT_TARGET_RANDOM, 0, 0.0f, true))
							{
								DoCast(pTarget, SPELL_FEL_FLAMES, true);
								me->SendPlaySpellVisual(23004, pTarget, 1.0f, false, true);
								
							}
                            events.ScheduleEvent(EVENT_FEL_FLAMES, urand(31000, 35000));
                            break;
                        ...
                        case EVENT_EASY_PREY:
                            ...
                            events.ScheduleEvent(EVENT_CORRUPTING_TOUCH, 15000);
                            events.ScheduleEvent(EVENT_FEL_FLAMES, urand(10000, 12000));
                        ...
                        case EVENT_END_HUNT:
                            ...
                            Talk(SAY_ACHIEVE);
                            bAchieve = true;
                            me->AddAura(SPELL_ENFEEBLED, me);
                            events.ScheduleEvent(EVENT_CORRUPTING_TOUCH, 23000);
