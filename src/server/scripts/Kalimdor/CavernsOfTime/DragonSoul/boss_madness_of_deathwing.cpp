                        ...
                        case EVENT_FINISH:
                        {
                                             ...
                                             case RAID_DIFFICULTY_10MAN_NORMAL:
                                                 ...
                                                 if (!trall->FindNearestGameObject(GO_ELEMENTIUM_FRAGMENT_10N, 500.0f))
                                                     trall->SummonGameObject(GO_ELEMENTIUM_FRAGMENT_10N, -12100.5f, 12173.6f, -2.73425f, 5.09636f, 0, 0, 0, 0, 7 * DAY);                                              
                                                 break;
                                             case RAID_DIFFICULTY_25MAN_NORMAL:
                                                 if (Creature * trall = me->FindNearestCreature(NPC_THRALL_3, 500.0f, true))
                                                 if (!trall->FindNearestGameObject(GO_ELEMENTIUM_FRAGMENT_25N, 500.0f))
                                                     trall->SummonGameObject(GO_ELEMENTIUM_FRAGMENT_25N, -12100.5f, 12173.6f, -2.73425f, 5.09636f, 0, 0, 0, 0, 7 * DAY);
                                                 break;
                                             case RAID_DIFFICULTY_10MAN_HEROIC:
                                                 if (Creature * trall = me->FindNearestCreature(NPC_THRALL_3, 500.0f, true))
                                                 if (!trall->FindNearestGameObject(GO_ELEMENTIUM_FRAGMENT_10H, 500.0f))
                                                     trall->SummonGameObject(GO_ELEMENTIUM_FRAGMENT_10H, -12100.5f, 12173.6f, -2.73425f, 5.09636f, 0, 0, 0, 0, 7 * DAY);
                                                 break;
                                             case RAID_DIFFICULTY_25MAN_HEROIC:
                                                 if (Creature * trall = me->FindNearestCreature(NPC_THRALL_3, 500.0f, true))
                                                 if (!trall->FindNearestGameObject(GO_ELEMENTIUM_FRAGMENT_25H, 500.0f))
                                                     trall->SummonGameObject(GO_ELEMENTIUM_FRAGMENT_25H, -12100.5f, 12173.6f, -2.73425f, 5.09636f, 0, 0, 0, 0, 7 * DAY);
                                                 break;
        ...
        struct npc_madness_of_deathwing_limb_tentacleAI : public Scripted_NoMovementAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                if (pInstance && m_current_platform)
                {
                    ...
                    if (Creature* pAspect = ObjectAccessor::GetCreature(*me, pInstance->GetData64(DATA_NOZDORMU_DRAGON)))                    
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_UPDATE_HEALTH:
                            if (AuraPtr aur = me->GetAura(SPELL_BURNING_BLOOD))
                            {
    ...
    public:
        ...
        struct npc_madness_of_deathwing_blistering_tentacleAI : public Scripted_NoMovementAI
        {
            ...
            void Reset()
            {
                ...
                me->SetFloatValue(UNIT_FIELD_COMBAT_REACH, 16.0f);
    ...
    public:
        ...
        struct npc_madness_of_deathwing_regenerative_bloodAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                me->SetInt32Value(UNIT_FIELD_MAXPOWER1, 90);
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_UPDATE_HEALTH:                  
                            if (me->GetUInt32Value(UNIT_FIELD_POWER1) < 90)
                            {
                            ...
                            else if (me->GetUInt32Value(UNIT_FIELD_POWER1) >= 90)
                            {
                                me->SetPower(POWER_ENERGY, 0);
                                me->SetInt32Value(UNIT_FIELD_POWER1, 0);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_UPDATE_HEALTH:
                        {
                            ...
                            if (AuraEffectPtr aurEff = me->GetAuraEffect(SPELL_CORRUPTED_BLOOD, EFFECT_1))
                            {
...
public:
    ...
    class spell_madness_of_deathwing_burning_blood_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            if (AuraPtr aur = GetCaster()->GetAura(SPELL_BURNING_BLOOD))
            {
...
public:
    ...
    class spell_madness_of_deathwing_degenerative_bite_AuraScript : public AuraScript
    {
        ...
        bool OnCheckProc(ProcEventInfo& eventInfo)
        {
            ...
            {
               if(AuraPtr aur = target->GetAura(SPELL_DEGENERATIVE_BITE_DPS))
               {
...
public:
    ...
    class spell_madness_of_deathwing_tetanus_AuraScript : public AuraScript
    {
        ...
        bool OnCheckProc(ProcEventInfo& eventInfo)
        {
            if (Unit* target = eventInfo.GetActionTarget())
            {
                if (AuraPtr aur = target->GetAura(SPELL_ELEMENTIUM_TERROR_TETANUS_DPS))
                {
                    aur->ModStackAmount(1);
                    return false;
                }
