...
public:
    ...
    struct boss_keristraszaAI : public BossAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if (instance->GetBossState(DATA_ANOMALUS) == DONE && instance->GetBossState(DATA_ORMOROK) == DONE && instance->GetBossState(DATA_GRAND_MAGUS_TELESTRA) == DONE)
			{
				if (IsHeroic())
				{
					instance->DoCompleteAchievement(490);
					instance->DoModifyPlayerCurrenciesIfLevel(395, 5000, 80);
				}
				else
				{
					instance->DoCompleteAchievement(478);
				}
			}
        }
