/*
* Copyright (C) 2008-2017 RoG WoW <http://wow.rog.snet/>
*
	...
	struct boss_halionAI : public generic_halionAI
	{
		...
		void JustDied(Unit* /*killer*/)
		{
			...
			if (instance)
			{
				...
				// Giving the Achievement
				switch (map->GetDifficulty())
				{
					case RAID_DIFFICULTY_10MAN_NORMAL:
						instance->DoCompleteAchievement(4817);
						break;
					case RAID_DIFFICULTY_10MAN_HEROIC:
						instance->DoCompleteAchievement(4818);
						break;
					case RAID_DIFFICULTY_25MAN_NORMAL:
						instance->DoCompleteAchievement(4815);
						break;
					case RAID_DIFFICULTY_25MAN_HEROIC:
						instance->DoCompleteAchievement(4816);
						break;
	...
	struct npc_halion_controllerAI : public ScriptedAI
	{
		...
		void DoAction(int32 const action)
		{
			...
			case ACTION_MONITOR_CORPOREALITY:
			{
				...
				//DoCast(me, SPELL_SUMMON_EXIT_PORTALS); This Spell is Useless in Pandaria
				// Instead we will select a few members of the instance group and Teleport them into the realm
				...
				if (!PlayerList.isEmpty())
				{
					...
					{
						// Teleporting out of Twilight Realm Players that are 10 yards away from the boss
						if (me->GetDistance2d(i->getSource()) > 10.0f)
						{
							if (Player* pPlayer = i->getSource())
								pPlayer->CastCustomSpell(pPlayer, SPELL_LEAVE_TWILIGHT_REALM, NULL, NULL, NULL, true);
						}
					}
				}
