    ...
    struct boss_sartharionAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            if (instance)
            {
				...
				bool AliveDrakes = true;
				if (!pTenebron->isAlive() && !pShadron->isAlive() && !pVesperon->isAlive())
					AliveDrakes = false;

				Map::PlayerList const &PlayerList = map->GetPlayers();

				uint32 drake_counter = 0;
				if ((pTenebron->isAlive() && !pShadron->isAlive() && !pVesperon->isAlive()) ||
					!pTenebron->isAlive() && pShadron->isAlive() && !pVesperon->isAlive() ||
					!pTenebron->isAlive() && !pShadron->isAlive() && pVesperon->isAlive())
					drake_counter = 1;
				else if ((pTenebron->isAlive() && pShadron->isAlive() && !pVesperon->isAlive()) ||
					(pTenebron->isAlive() && !pShadron->isAlive() && pVesperon->isAlive()) ||
					!pTenebron->isAlive() && pShadron->isAlive() && pVesperon->isAlive())
					drake_counter = 2;

				// Normal Mode
				AchievementEntry const* ach_2049 = sAchievementStore.LookupEntry(2049);
				AchievementEntry const* ach_2050 = sAchievementStore.LookupEntry(2050);
				AchievementEntry const* ach_2051 = sAchievementStore.LookupEntry(2051);
				
				// Heroic Mode
				AchievementEntry const* ach_2052 = sAchievementStore.LookupEntry(2052);
				AchievementEntry const* ach_2053 = sAchievementStore.LookupEntry(2053);
				AchievementEntry const* ach_2054 = sAchievementStore.LookupEntry(2054);

				switch (map->GetDifficulty())
				{
					// 10 Players Normal
				case RAID_DIFFICULTY_10MAN_NORMAL:
					instance->DoCompleteAchievement(1876);
					
					if (PlayerList.getSize() < 9 && !AliveDrakes)
						instance->DoCompleteAchievement(624);
					// Adding Loot For Drake Mount
					if (AliveDrakes)
						me->AddLootMode(2);
					// Twilight Assist Achievement Chain
					for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
					{
						if (Player* pPlayer = i->getSource())
						{
							if (!pPlayer->GetAchievementMgr().HasAchieved(2049) && drake_counter == 1)
								pPlayer->CompletedAchievement(ach_2049);
							else if (pPlayer->GetAchievementMgr().HasAchieved(2049) && drake_counter == 2)
								pPlayer->CompletedAchievement(ach_2050);
							else if (pPlayer->GetAchievementMgr().HasAchieved(2050) && AliveDrakes)
								pPlayer->CompletedAchievement(ach_2051);
						}
					}
					break;
				...
				case RAID_DIFFICULTY_25MAN_NORMAL:
					if (PlayerList.getSize() < 21 && !AliveDrakes)
						instance->DoCompleteAchievement(1877);
					// Checking For Feat of Strength (Obsidian Slayer)
					QueryResult result = CharacterDatabase.PQuery("SELECT * FROM character_achievement WHERE achievement = 456");
					// No Achievements were found Realm-First
					if (!result)
					{
						instance->DoCompleteAchievement(456);
					}
					// Twilight Assist Achievement Chain
					for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
					{
						if (Player* pPlayer = i->getSource())
						{
							if (!pPlayer->GetAchievementMgr().HasAchieved(2052) && drake_counter == 1)
								pPlayer->CompletedAchievement(ach_2052);
							else if (pPlayer->GetAchievementMgr().HasAchieved(2052) && drake_counter == 2)
								pPlayer->CompletedAchievement(ach_2053);
							else if (pPlayer->GetAchievementMgr().HasAchieved(2053) && AliveDrakes)
								pPlayer->CompletedAchievement(ach_2054);
						}
