    ...
    public:
        ...
        struct boss_devourer_of_soulsAI : public BossAI
        {
            ...
            void EnterCombat(Unit* /*who*/)
            {
                ...
                events.ScheduleEvent(EVENT_PHANTOM_BLAST, 60000);
            ...
            void DamageTaken(Unit* /*pDoneBy*/, uint32 &uiDamage)
            {
                if (mirroredSoulTarget && me->HasAura(SPELL_MIRRORED_SOUL))
                {
                    if (Player* player = Unit::GetPlayer(*me, mirroredSoulTarget))
                    {
                        ...
                        {
                            int32 mirrorDamage = (uiDamage* 95)/100;
            ...
            void JustDied(Unit* /*killer*/)
            {
				...
				if (instance)
				{
					...
					instance->DoCompleteAchievement(ACHIEV_FORGE_OF_SOULS_N);
					if (IsHeroic())
					{
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_MIRRORED_SOUL:
                            ...
                            events.ScheduleEvent(EVENT_MIRRORED_SOUL, urand(10000, 15000));
                            break;
                        case EVENT_WELL_OF_SOULS:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                DoCast(target, SPELL_WELL_OF_SOULS);
                            events.ScheduleEvent(EVENT_WELL_OF_SOULS, 8000);
                            break;
