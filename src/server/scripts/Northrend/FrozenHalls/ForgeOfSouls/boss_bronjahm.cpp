    ...
    public:
        ...
        struct boss_bronjahmAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                events.ScheduleEvent(EVENT_MAGIC_BANE, urand(6000, 10000));
                events.ScheduleEvent(EVENT_CORRUPT_SOUL, urand(15000, 25000), 0, PHASE_1);
           ...
           void JustReachedHome()
           {
            ...
            void DamageTaken(Unit* /*attacker*/, uint32& /*damage*/)
            {
                if (events.GetPhaseMask() & (1 << PHASE_1) && !HealthAbovePct(30))
                {
                    ...
                    events.ScheduleEvent(EVENT_FEAR, urand(5000, 7000), 0, PHASE_2);
            ...
            {
				if (summon->GetEntry() == CREATURE_CORRUPTED_SOUL_FRAGMENT)
				{
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_MAGIC_BANE:
                            ...
                            events.ScheduleEvent(EVENT_MAGIC_BANE, urand(6000, 12000));
                            break;
            ...
            void MovementInform(uint32 type, uint32 id)
            {
                if (instance)
                {
                    if (TempSummon* summ = me->ToTempSummon())
                    {
                        ...
                        {
                            if (me->GetDistance2d(bronjahm->GetPositionX(), bronjahm->GetPositionY()) <= 5.0f) 
                            {
