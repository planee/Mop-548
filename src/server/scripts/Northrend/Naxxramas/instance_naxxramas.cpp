...
enum eEnums
{
	...
	GO_FINAL_PORTAL			= 181229, // Final Portal Trigger
        ...
        void OnGameObjectCreate(GameObject* go)
        {
            ...
            switch (go->GetEntry())
            {
                ...
                case GO_KELTHUZAD_TRIGGER:
                    ...
                    break;
				case GO_FINAL_PORTAL:
					// Behaviour for Sapphiron Portal
					eyePortalRamp = go->GetGUID();
					if (GetBossState(BOSS_MAEXXNA) == DONE && GetBossState(BOSS_LOATHEB) == DONE && 
						GetBossState(BOSS_THADDIUS) == DONE && GetBossState(BOSS_HORSEMEN) == DONE)
...
class at_naxxramas_final_portal : public AreaTriggerScript
{
public:
	at_naxxramas_final_portal() : AreaTriggerScript("at_naxxramas_final_portal") {}

	bool OnTrigger(Player* player, AreaTriggerEntry const* trigger)
	{
		InstanceScript* instance = player->GetInstanceScript();
		if (instance->GetBossState(BOSS_MAEXXNA) == DONE && instance->GetBossState(BOSS_LOATHEB) == DONE &&
			instance->GetBossState(BOSS_THADDIUS) == DONE && instance->GetBossState(BOSS_HORSEMEN) == DONE)
		{
			player->TeleportTo(533, float(3498.28), float(-5349.9), float(144.96), float(1.31));
			return true;
		}
		else
			return false;
