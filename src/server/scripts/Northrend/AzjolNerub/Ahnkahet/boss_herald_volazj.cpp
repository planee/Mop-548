    ...
    struct boss_volazjAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if (instance)
			{
				...
				// Verifying that Previous Encounters are done
				if (instance->GetBossState(DATA_AMANITAR_EVENT) == DONE &&
					instance->GetBossState(DATA_JEDOGA_SHADOWSEEKER_EVENT) == DONE &&
					instance->GetBossState(DATA_PRINCE_TALDARAM_EVENT) == DONE)
				{
					if (!IsHeroic())
					{
						instance->DoCompleteAchievement(481);
					}
					else
					{
						instance->DoCompleteAchievement(492);
