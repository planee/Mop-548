        ...
        struct boss_anub_arakAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
				...
				if (_instance)
				{
					...
					if (!IsHeroic())
					{
						if (_instance->GetBossState(DATA_KRIKTHIR_THE_GATEWATCHER_EVENT) == DONE && _instance->GetBossState(DATA_HADRONOX_EVENT) == DONE)
							_instance->DoCompleteAchievement(480);
					}
					else
					{
						if (_instance->GetBossState(DATA_KRIKTHIR_THE_GATEWATCHER_EVENT) == DONE && _instance->GetBossState(DATA_HADRONOX_EVENT) == DONE)
						{
							_instance->DoCompleteAchievement(491);
							_instance->DoModifyPlayerCurrenciesIfLevel(395, 5000, 80);
						}
					}
				}
