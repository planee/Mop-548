    ...
    public:
        ...
        struct boss_mechano_lord_capacitusAI : public BossAI
        {
            ...
            void JustDied(Unit* /*victim*/) 
            {
                _JustDied();
                Talk(YELL_DEATH);
				/*Remove all Buff when Die*/
				std::list<Player*>_PlayerList;
				GetPlayerListInGrid(_PlayerList, me, 60.0f);
				for (std::list<Player*>::iterator iter = _PlayerList.begin(); iter != _PlayerList.end(); ++iter)
					(*iter)->RemoveAura(SPELL_POSITIVE_CHARGE_STACK);
            }
            ...
            void UpdateAI(const uint32 diff) 
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
							Position pos = me->GetRandomNearPosition(pos, 5.0f);
