...
class instance_zulgurub : public InstanceMapScript
{
    public:
        ...
        struct instance_zulgurub_InstanceMapScript : public InstanceScript
        {
			...
			{
				// Cast Yoga Flame all Tiki Torch
				if (torchTimer <= diff)
				{
					std::list<Creature*> TikiTorch;
					
					if (Creature* venoxis = instance->GetCreature(venoxisGUID)){
						GetCreatureListWithEntryInGrid(TikiTorch, venoxis, 52419, 500.0f);
						if (TikiTorch.empty())
							return;
					}					
					for (std::list<Creature*>::iterator iter = TikiTorch.begin(); iter != TikiTorch.end(); ++iter)
						(*iter)->CastSpell(*iter, 97000, false);
					torchTimer = 6000;
				}
				else
					torchTimer -= diff;
