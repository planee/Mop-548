...
enum KingSpells
{
    ...
    SPELL_SHOOT_FALSTAD   = 84833,
    SPELL_STORM_HAMMER    = 84828,

    //Moira Thaurissan
    SPELL_HEAL            = 84822, 
    SPELL_MIND_BLAST      = 84825,
    SPELL_RENEW           = 84824,
};
...
class boss_muradin_broncebeard : public CreatureScript
{
public:
    boss_muradin_broncebeard() : CreatureScript("boss_muradin_broncebeard") { }

    struct boss_muradin_broncebeardAI : public ScriptedAI
    {    
        boss_muradin_broncebeardAI(Creature* creature) : ScriptedAI(creature) { }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new boss_muradin_broncebeardAI(creature);
    }
};
...
public:
    ...
    {
        boss_moira_thaurissanAI(Creature* creature) : ScriptedAI(creature) { }

        void Reset() { events.Reset(); }

        void EnterCombat(Unit* /*who*/)
        {
            events.ScheduleEvent(EVENT_MIND_BLAST, 3000);
            events.ScheduleEvent(EVENT_HEAL, 8000);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_HEAL:
                    DoCast(SPELL_HEAL);
                    events.ScheduleEvent(EVENT_HEAL, urand(8 * IN_MILLISECONDS, 10 * IN_MILLISECONDS));
                    break;
                case EVENT_RENEW:
                    DoCast(SPELL_RENEW);
                    events.ScheduleEvent(EVENT_RENEW, urand(20 * IN_MILLISECONDS, 22 * IN_MILLISECONDS));
                    break;
                case EVENT_MIND_BLAST:
                    DoCast(SelectTarget(SELECT_TARGET_TOPAGGRO, 0), SPELL_MIND_BLAST);
                    events.ScheduleEvent(EVENT_MIND_BLAST, urand(3 * IN_MILLISECONDS, 5 * IN_MILLISECONDS));
                    break;
                }
            }
            DoMeleeAttackIfReady();
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new boss_moira_thaurissanAI(creature);
    }
};
