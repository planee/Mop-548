    ...
    public:
        ...
        struct boss_arcanotronAI : public BossAI
        {
            ...
            void JustDied(Unit* who)
            {
				...
				instance->SetBossState(DATA_OMNOTRON, DONE);
...
//ID - 79629 Power Generator - Triggred by spell : (79628) Power Generator

class boss_arcanotron_spell_warl_power_generator : public SpellScriptLoader
{
public:
	boss_arcanotron_spell_warl_power_generator() : SpellScriptLoader("boss_arcanotron_spell_warl_power_generator") { }

	class spell_war_power_generator_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_war_power_generator_SpellScript);
		
		void HandleEnergize(SpellEffIndex /*effIndex*/){
			Unit* _target = GetHitUnit();
			if (_target){								
				if (_target->GetEntry() == 42166 || _target->GetEntry() == 42179 || _target->GetEntry() == 42186 || _target->GetEntry() == 42178 || _target->GetEntry() == 42180){
		...
		void Register()
		{				
			OnEffectHitTarget += SpellEffectFn(spell_war_power_generator_SpellScript::HandleEnergize, EFFECT_1, SPELL_EFFECT_ENERGIZE);
	...
	SpellScript* GetSpellScript() const
	{
		return new spell_war_power_generator_SpellScript();
	}
};
...
void AddSC_boss_omnotron_defence_system()
{
	...
	new boss_arcanotron_spell_warl_power_generator();
}