        ...
        struct boss_maloriakAI : public BossAI
        {
                ...
                if (type == POINT_MOTION_TYPE)
                {
                    switch (id)
                    {
                        ...
                        case POINT_DARK:							
							...
							if (irand(1, 2) == 1)
								events.ScheduleEvent(EVENT_FIRE_PHASE, 90000);
							else
								events.ScheduleEvent(EVENT_FROST_PHASE, 90000);
                            break;
                    }
                }
                if (id == EVENT_JUMP)
            ...
            {				
                switch (spellInfo->Id)
                {
                    case SPELL_DRINK_RED_BOTTLE:
						...
						me->GetMotionMaster()->MoveChase(me->SelectNearestPlayer(100.0f));
						break;
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CONSUMING_FLAMES:							
							if (Unit* target = SelectAreaTargetRandom(40.0f))
                        ...
                        { 							
							if (Unit* targetflashfreeze = SelectAreaTargetRandom(40.0f))							
                        ...
                        {
							if (Unit* targetabsolutezero = SelectAreaTargetRandom(40.0f))
                                DoCast(targetabsolutezero, SPELL_ABSOLUTE_ZERO);
                            events.ScheduleEvent(EVENT_ABSOLUTE_ZERO, urand(15000, 25000));
                            break;
                        }
                        ...
                        case EVENT_CONTINUE:
                            me->SetReactState(REACT_AGGRESSIVE);
                            me->GetMotionMaster()->MoveChase(me->SelectNearestPlayer(50.0f));
                            break;
				...
				if (events.Empty()){
					switch (state){
						case 1:
							events.ScheduleEvent(EVENT_OPENING, 1000);							
							break;
						case 2:
							events.ScheduleEvent(EVENT_FIRE_PHASE, 1000);							
							break;
						case 3:
							events.ScheduleEvent(EVENT_FROST_PHASE, 1000);							
							break;
						case 4:
							events.ScheduleEvent(EVENT_GREEN_PHASE, 1000);							
							break;
						case 5:
							events.ScheduleEvent(EVENT_DARK_PHASE, 1000);							
							break;						
					}
				}
            }
            ...
            private:
                ...
                {	
                    ...
                    {	
						...
						events.ScheduleEvent(EVENT_CONSUMING_FLAMES, 8000);
						events.ScheduleEvent(EVENT_RELEASE_ABERRATIONS, 11000); 
						events.ScheduleEvent(EVENT_REMEDY, 14000);						
						if (bFrost)
							events.ScheduleEvent(EVENT_GREEN_PHASE, 18000);
						else
							events.ScheduleEvent(EVENT_FROST_PHASE, 18000);
                    }
                    else
                    {		
						bFire = false;
                ...
                {	
                    ...
                    if (on)
                    {
						...
						events.ScheduleEvent(EVENT_ARCANE_STORM, 8000);
						events.ScheduleEvent(EVENT_FLASH_FREEZE, 14000);
                        events.ScheduleEvent(EVENT_RELEASE_ABERRATIONS, 17000); 
						if (bFire)
							events.ScheduleEvent(EVENT_GREEN_PHASE, 20000);						
						else						
							events.ScheduleEvent(EVENT_FIRE_PHASE, 20000);
                ...
                {	
                    ...
                    {		
						...
						events.ScheduleEvent(EVENT_OPENING, 9000);						
                    }
                    else
                    {						
                        events.CancelEvent(EVENT_ARCANE_STORM);
				...
				void EventOpening(bool on)
				{			
					state = 1;
					if (on)
					{
						if (IsHeroic())
							events.ScheduleEvent(EVENT_DARK_PHASE, 20000);
						else{							
							if (irand(1, 2) == 1)
								events.ScheduleEvent(EVENT_FIRE_PHASE, 20000);
							else
								events.ScheduleEvent(EVENT_FROST_PHASE, 20000);
						}						
                ...
                {	
                    ...
                    events.ScheduleEvent(EVENT_MAGMA_JETS, 4000);
                    events.ScheduleEvent(EVENT_ACID_NOVA, urand(5000, 6000));
                }
...
class aura_maloriak_flash_freeze : public SpellScriptLoader
{
    public:
		aura_maloriak_flash_freeze() : SpellScriptLoader("aura_maloriak_flash_freeze") { }

        class aura_maloriak_flash_freeze_AuraScript : public AuraScript
        {
			PrepareAuraScript(aura_maloriak_flash_freeze_AuraScript);
            ...
            void Register()
            {
				OnEffectApply += AuraEffectApplyFn(aura_maloriak_flash_freeze_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_STUN, AURA_EFFECT_HANDLE_REAL);
            }
        };

        AuraScript* GetAuraScript() const
        {
			return new aura_maloriak_flash_freeze_AuraScript();
        }
};
...
public:
	...
	class spell_maloriak_flash_freeze_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_maloriak_flash_freeze_SpellScript);
		

		void HandleEffectHit()
		{			
			Creature* _caster = GetCaster()->ToCreature();
			if (!_caster)
				return;
			Position pos;
			Unit* _target = GetHitUnit();
			_target->GetPosition(&pos);
			TempSummon* summon = _caster->SummonCreature(NPC_FLASH_FREEZE, pos);
			if (summon)
				summon->AI()->SetGUID(_target->GetGUID(), DATA_TRAPPED_PLAYER);
		}
	

		void Register()
		{			
			AfterHit += SpellHitFn(spell_maloriak_flash_freeze_SpellScript::HandleEffectHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_maloriak_flash_freeze_SpellScript();
	}
};

//Spell maloriak bitting chill - 77760
class spell_maloriak_bitting_chill : public SpellScriptLoader
{
public:
	spell_maloriak_bitting_chill() : SpellScriptLoader("spell_maloriak_bitting_chill") { }

	class spell_maloriak_bitting_chill_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_maloriak_bitting_chill_SpellScript);
		...
		void FilterTargets(std::list<WorldObject*>& targets)
		{
			...
			auto ranPlayer = irand(1,10);
			int _count = 1;
			std::list<Unit*> _targets;
			_caster->GetAttackableUnitListInRange(_targets, 30.0f);			
		...
		void Register()
		{
			OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_maloriak_bitting_chill_SpellScript::FilterTargets, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_maloriak_bitting_chill_SpellScript();
	}
};
...
void AddSC_boss_maloriak()
{
    ...
    new npc_vile_swill();
    new aura_maloriak_flash_freeze();
	...
	new spell_maloriak_bitting_chill();
