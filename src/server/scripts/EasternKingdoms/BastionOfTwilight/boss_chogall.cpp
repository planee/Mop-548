...
enum Spells
{
    ...
    SPELL_CHO_BLAST                             = 82518,
    SPELL_GALL_BLAST                            = 82523,
...
enum Events
{
    ...
    EVENT_SHADOW_ORDERS         = 3,
    EVENT_FLAME_ORDERS          = 4,
    ...
    public:
        ...
        struct boss_chogallAI : public BossAI
        {
            ...
            void Reset() 
            {
                ...
                events.Reset();
                summons.DespawnAll();
                DespawnCreatures(NPC_CORRUPTION);
                ...
                bPhase2 = false;
                summoner = false;
                instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_CHO_BLAST);
                instance->DoRemoveAurasDueToSpellOnPlayers(SPELL_GALL_BLAST);
            }
            ...
            void EnterCombat(Unit* who)
            {
                ...
                events.ScheduleEvent(EVENT_FLAME_ORDERS, 5000); // uses normal spell and adds trigger aura. 10 sec this->flaming.
                events.ScheduleEvent(EVENT_SHADOW_ORDERS, 25000); // uses normal spell and adds trigger aura. 9 sec this->empowered.
                events.ScheduleEvent(EVENT_BERSERK, 10 * MINUTE * IN_MILLISECONDS);
                events.ScheduleEvent(EVENT_CORRUPTING_ADHERENT, 92000);
            ...
            void JustDied(Unit* killer)
            {
                ...
                if (instance)
                {
                    ...
                    //Achievement Bastion of Twilight
                        instance->DoCompleteAchievement(4850);                    
                }
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                {
                    bPhase2 = true;
                    if (me->HasAura(SPELL_FLAME_ORDERS_PERIODIC_TRIGGER) && me->HasAura(SPELL_SHADOW_ORDERS_PERIODIC_TRIGGER)) // Remove P1 Orders triggers.
                    {
                        me->RemoveAurasDueToSpell(SPELL_FLAME_ORDERS_PERIODIC_TRIGGER);
                        me->RemoveAurasDueToSpell(SPELL_SHADOW_ORDERS_PERIODIC_TRIGGER);
                    }
                    ...
                    Talk(SAY_CONSUME);
                    //DoCast(me, SPELL_CONSUME_BLOOD_OF_THE_OLD_GOD); //That Present Problem, need a Script...
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_CONVERSION:
                        Talk(SAY_CONVERSION);
                        me->CastCustomSpell(SPELL_CONVERSION, SPELLVALUE_MAX_TARGETS, RAID_MODE(2, 4, 3, 6), 0, false);
                        events.ScheduleEvent(EVENT_CONVERSION, urand(35000, 37000)); 
                        break;
                    case EVENT_FLAME_ORDERS:
                        if (!summoner)
                            summoner = true;
                        DoCast(me, SPELL_FLAME_ORDERS_1); // just once.
                        DoCast(me, SPELL_FLAME_ORDERS_PERIODIC_TRIGGER); // every 50 sec.
                        break;
                    case EVENT_SHADOW_ORDERS:
                        DoCast(me, SPELL_SHADOW_ORDERS_1); // just once.
                        DoCast(me, SPELL_SHADOW_ORDERS_PERIODIC_TRIGGER);  // every 50 sec.
                        break;
                    ...
                    case EVENT_DARKENED_CREATIONS:
                        Talk(SAY_CREATIONS);
                        me->CastCustomSpell(SPELL_DARKENED_CREATIONS, SPELLVALUE_MAX_TARGETS, RAID_MODE(4, 10, 4, 10), 0, false);                       
...
class npc_chogall_fire_portal : public CreatureScript
{
    public:
        ...
        struct npc_chogall_fire_portalAI : public Scripted_NoMovementAI
        {
            ...
            void Reset() 
            {
                me->CastSpell(me, SPELL_VIS_FLAMEP, true);
            }
...
class npc_chogall_shadow_portal : public CreatureScript
{
    public:
        ...
        struct npc_chogall_shadow_portalAI : public Scripted_NoMovementAI
        {
            ...
            void Reset() 
            {
                me->CastSpell(me, SPELL_VIS_SHADOWP, true);
            }
...
class npc_chogall_fire_elemental : public CreatureScript
{
    public:
        ...
        struct npc_chogall_fire_elementalAI : public ScriptedAI
        {
            ...
            EventMap events;
            Creature* pChogall = me->FindNearestCreature(NPC_CHOGALL, 100.0f);
            bool bNear;
                ...
                if (pChogall)
                {
                    if (IsHeroic())
                    {
                        if (me->GetDistance(pChogall) < 1.0f && !bNear)
                        {
                            ...
                            pChogall->CastSpell(pChogall, SPELL_ABSORB_FIRE, true);
                            me->DespawnOrUnsummon();
                            return;
                        }
                    }
                    ...
                    {
                        if (me->GetDistance(pChogall) < 5.0f && !bNear)
                        {
                            bNear = true;
                            me->GetMotionMaster()->Clear();
                            pChogall->CastSpell(pChogall, SPELL_ABSORB_FIRE, true);
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (pChogall)
                {
                    if (me->GetDistance(pChogall) < 1.0f && !bNear)
                    {
                        bNear = true;
                        me->GetMotionMaster()->Clear();
                        pChogall->CastSpell(pChogall, SPELL_ABSORB_SHADOW, true);
                        me->DespawnOrUnsummon();
                        return;
                    }
                }
...
class npc_chogall_darkened_creation : public CreatureScript
{
    public:
        ...
        struct npc_chogall_darkened_creationAI : public Scripted_NoMovementAI
        {
            ...
            {
                pInstance = pCreature->GetInstanceScript();
                pCreature->CastSpell(pCreature, SPELL_TENTACLE_VIS, true);
            }
...
class npc_chogall_blood_of_the_old_god : public CreatureScript
{
    public:
        ...
        struct npc_chogall_blood_of_the_old_godAI : public ScriptedAI
        {
            ...
            {
                me->SetSpeed(MOVE_RUN, 0.55f);
                me->SetSpeed(MOVE_WALK, 0.55f);
                pInstance = pCreature->GetInstanceScript();
            }
        ...
        class spell_chogall_conversion_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                if (GetCaster()->getVictim() != GetHitUnit())
                {
                    ...
                    GetHitUnit()->CastSpell(GetHitUnit(), SPELL_WORSHIPPING_LINKED, true);
                }
    ...
    public:
        ...
        class spell_chogall_summon_corrupting_adherent_SpellScript : public SpellScript
        {
            ...
            void HandleScript(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    GetCaster()->CastSpell(GetCaster(),  SPELL_SUMMON_CORRUPTING_ADHERENT_1, true);
                    GetCaster()->CastSpell(GetCaster(),  SPELL_SUMMON_CORRUPTING_ADHERENT_2, true);
                }
                else
    ...
    public:
        ...
        class spell_chogall_worshipping_AuraScript : public AuraScript
        {
            ...
            void OnApply(constAuraEffectPtr aurEff, AuraEffectHandleModes /*mode*/)
            {
                if (!GetTarget())
                    return;
                if (Creature* chogal = GetTarget()->FindNearestCreature(NPC_CHOGALL, 100.0f, true))
                {
                    if (GetTarget()->GetTypeId() == TYPEID_PLAYER)
                    {
                        TargetFaction = GetTarget()->getFaction();
                        GetTarget()->ToPlayer()->SetClientControl(GetTarget(), 0);
                        GetTarget()->setFaction(chogal->getFaction());
                    }
                }
            }
            ...
            void OnRemove(constAuraEffectPtr aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                if (GetTarget()->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    GetTarget()->setFaction(TargetFaction);
                }                   
            }
            ...
            void OnPeriodic(constAuraEffectPtr /*aurEff*/)
            {
                if (!GetUnitOwner())
                    return;
                if (Creature* chogal = GetUnitOwner()->FindNearestCreature(NPC_CHOGALL, 100.0f, true))
                {
                    if (GetUnitOwner()->GetTypeId() == TYPEID_PLAYER)
                    {
                        if (chogal->GetTypeId() != TYPEID_PLAYER)
                        GetUnitOwner()->CastSpell(chogal, SPELL_TWISTED_DEVOTION, true);
                    }
                }                
            }
            ...
            void Register()
            {
                ...
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_chogall_worshipping_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
            }
        };
