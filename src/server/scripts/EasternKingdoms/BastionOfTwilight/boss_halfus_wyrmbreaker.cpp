...
enum Adds
{
    NPC_STORM_RIDER     = 44652,
    NPC_NETHER_SCION    = 44645,
	NPC_SLATE_DRAKE     = 44650,
            ...
            void Reset()
            {
                ...
                else
                {
                    ...
                    case 1:
						...
						slatedrake->RemoveAura(SPELL_UNRESPONSIVE_DRAKE);
                        for (uint8 i = 0; i < 8; i++)
                            whelps[i]->CastSpell(whelps[i], SPELL_UNRESPONSIVE_WHELP, true);
                        proto->CastSpell(proto, SPELL_DANCING_FLAMES, true);
                        DoCast(me, SPELL_SHADOW_WARPED); 
...
class npc_halfus_dragon : public CreatureScript
{
    public:
        ...
        struct npc_halfus_dragonAI : public ScriptedAI 
        {
			...
			void JustDied(Unit* killer)
			{
				if (Creature* Halfus = me->FindNearestCreature(NPC_HALFUS_WYRMBREAKER, 500.0f, true))
					Halfus->AddAura(SPELL_DRAGON_VENGEANCE, Halfus);
			}
        };
};
