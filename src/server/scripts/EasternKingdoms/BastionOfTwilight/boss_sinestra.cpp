...
const Position addsPos[13] = 
{
    ...
    {-997.45f, -796.70f, 446.01f, 0.0f},        // 11 Stalker near Calen
    {-976.52f, -776.35f, 458.58f, 0.0f},        // 12 Stalker near Sinestra
//    {-989.62f, -788.95f, 449.97f, 0.0f},      // 13 Stalker between
};
...
class boss_sinestra : public CreatureScript
{
    public:
        ...
        struct boss_sinestraAI : public Scripted_NoMovementAI
        {
            ...
            {
                summons.DespawnAll();
                DespawnCreatures(NPC_TWILIGHT_ESSENCE);
                events.Reset();
                ...
                me->SetFloatValue(UNIT_FIELD_BOUNDING_RADIUS, 20.0f);
                me->SetFloatValue(UNIT_FIELD_COMBAT_REACH, 20.0f);
            ...
            void MoveInLineOfSight(Unit* who)
            {
                ...
                if (!PlayerList.isEmpty())
                {
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        ...
                        if (!introDone && player->IsWithinDistInMap(me, 40.0f))
                        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                pInstance->DoCompleteAchievement(5121); //Achievement Dragonslayer
                GameObject* Chest = killer->SummonGameObject(GO_CACHE_OF_THE_BROODMOTHER, -965.402f, -746.666f, 438.594f, 4.1264f, 0, 0, 0, 0, 90000000);
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                else if (me->GetPower(POWER_MANA) < 1 && phase == 1)
                {
                    ...
                    if (Creature* pStalker = me->FindNearestCreature(NPC_BARRIER_COSMETIC_STALKER, 100.0f))
                    {
                        ...
                        pStalker->GetMotionMaster()->MovePoint(0, addsPos[12]);
                    }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUMMON_WHELP:
                            ...
                            {
                                for (int i = 0; i < 5; i++)
                                    me->SummonCreature(NPC_TWILIGHT_WHELP, WhelpsLocations[i].GetPositionX(),
                                    WhelpsLocations[i].GetPositionY(),
                                    WhelpsLocations[i].GetPositionZ(),
                                    WhelpsLocations[i].GetOrientation(), TEMPSUMMON_CORPSE_DESPAWN, 1000);
                                Side = true;
                            }
                            else
                            {
                                for (int i = 0; i < 5; i++)
                                    me->SummonCreature(NPC_TWILIGHT_WHELP, WhelpsLocations2[i].GetPositionX(),
                                    WhelpsLocations2[i].GetPositionY(),
                                    WhelpsLocations2[i].GetPositionZ(),
                                    WhelpsLocations2[i].GetOrientation(), TEMPSUMMON_CORPSE_DESPAWN, 1000);
                                Side = false;
                            }
                        ...
                        case EVENT_WRACK:
                            ...
                            for (std::list<Unit*>::iterator itr = targets.begin(); itr != targets.end();
                                ++itr)
                                me->CastSpell((*itr), SPELL_WRACK_10H, true);
                        ...
                        case EVENT_CONTINUE_PHASE_2:
                        {
                            ...
                            {
                                pStalker->GetMotionMaster()->MovementExpired(false);
                                pStalker->GetMotionMaster()->MovePoint(0, addsPos[11]);
                            }
                            phase = 1;
                            break;
                        }
                        case EVENT_SUMMON_CALLER:
                            if (Creature* pCaller = me->SummonCreature(NPC_TWILIGHT_SPITECALLER, addsPos[9], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000))
                        ...
                        {
                            uint8 i = urand(0, 1);
                            if (Creature* pDrake = me->SummonCreature(NPC_TWILIGHT_DRAKE, addsPos[5+i], TEMPSUMMON_CORPSE_TIMED_DESPAWN, 1000))
...
class npc_sinestra_twilight_whelp : public CreatureScript
{
    public:
        ...
        struct npc_sinestra_twilight_whelpAI : public ScriptedAI
        {
            ...
            {
                events.ScheduleEvent(EVENT_TWILIGHT_SPIT, 4000);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                {
                    events.ScheduleEvent(EVENT_RESURRECT, 3000);
                    Revived = true;
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_TWILIGHT_SPIT:                        
                            if (AuraPtr aur = me->getVictim()->GetAura(SPELL_TWILIGHT_SPIT_10H))
    ...
    public:
        ...
        struct npc_sinestra_twilight_essenceAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                ...
                events.ScheduleEvent(EVENT_ESSENCE_GROW, 15000);
            }
...
class npc_sinestra_barrier_stalker : public CreatureScript
{
    public:
        ...
        struct npc_sinestra_barrier_stalkerAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                me->NearTeleportTo(addsPos[11].GetPositionX(), addsPos[11].GetPositionY(), addsPos[11].GetPositionZ(), addsPos[11].GetOrientation());
            }
    ...
    public:
        ...
        struct npc_sinestra_twilight_spitecallerAI : public ScriptedAI 
        {
            ...
            void EnterCombat(Unit* /*who*/)
            {
                ...
                events.ScheduleEvent(EVENT_UNLEASH_ESSENCE, 10000);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if ((me->HasUnitState(UNIT_STATE_STUNNED) || me->HasUnitState(UNIT_STATE_ROOT)) && !indomitable)
                {
                    indomitable = true;
                    events.ScheduleEvent(EVENT_INDOMITABLE, 3000);
                }

                if (indomitable && me->HasAura(SPELL_INDOMITABLE_DUMMY))
                    indomitable = false;
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INDOMITABLE:
                                DoCast(me, SPELL_INDOMITABLE_DUMMY);                                                  
                            break;
                        case EVENT_UNLEASH_ESSENCE:
                            DoCastAOE(SPELL_UNLEASH_ESSENCE_10H);
                            events.ScheduleEvent(EVENT_UNLEASH_ESSENCE, urand(19000, 25000));
                            break;
                    }
                }
        ...
        class spell_sinestra_wrack_AuraScript : public AuraScript
        {
            ...
            void OnUpdate(AuraEffectPtr aurEff)
            {
                ...
                if (aurEff->GetTickNumber() == 1)
                    return;
                int32 dmg = int32(aurEff->GetAmount() * 2.0f);
            ...
            void Register()
            {                
                //OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_sinestra_wrack_AuraScript::OnUpdate, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);  Fix this when the hook work again
