	...
	struct instance_throne_of_the_tides_InstanceMapScript : public InstanceScript
	{
		...
		void SetData(uint32 type, uint32 data)
		{
			...
			case DATA_NEPTULON_EVENT:
				m_uiEvents[2] = data;
				if (data == DONE)
					if (Creature* neptulon = instance->GetCreature(uiNeptulonGUID))
					{
						if (GameObject* Chest = neptulon->SummonGameObject(instance->IsHeroic() ? GO_NEPTULON_CACHE_H : GO_NEPTULON_CACHE, -145.697f, 985.088f, 230.406f, 6.137f, 0, 0, 0, 0, 90000000))
		...
		bool SetBossState(uint32 type, EncounterState state)
		{
			...
			case DATA_LADY_NAZJAR:
				...
				{
					if (GameObject* nazjardoor = instance->GetGameObject(uiLadyNazjarDoorGUID))
						nazjardoor->SetPhaseMask(1, true);
				}
				if (state == DONE)
				{
					if (GameObject* nazjardoor = instance->GetGameObject(uiLadyNazjarDoorGUID))
						nazjardoor->SetPhaseMask(2, true);
				}
				break;
			case DATA_COMMANDER_ULTHOK:
				if (state == DONE)
