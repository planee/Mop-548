    ...
    struct boss_entropiusAI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
			...
			if (instance)
			{
				...
				instance->DoModifyPlayerCurrenciesIfLevel(395 /* Justice Points */, 7500 /* 75 Justice Points */, 70 /* Max Allowed Lv to Gain Currency */);
			}
        }
...
public:
    ...
    struct boss_muruAI : public Scripted_NoMovementAI
    {
        ...
        void Reset()
        {
			...
			{
				hp_setted = true;
				me->SetMaxHealth(me->GetMaxHealth() * 0.75f);
				me->SetHealth(me->GetMaxHealth() * 0.75f);

				// Reduce damage, custome
				const CreatureTemplate* cinfo = me->GetCreatureTemplate();
				if (cinfo)
				{
					me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, cinfo->mindmg * 0.75f);
					me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->maxdmg * 0.75f);
				}
...
public:
    ...
    struct npc_dark_fiendAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			{
				hp_setted = true;
				me->SetMaxHealth(me->GetMaxHealth() * 0.5f);
    ...
    struct npc_void_sentinelAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			if (!hp_setted)
			{
				...
				if (cinfo)
				{
					me->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, cinfo->mindmg * 0.5f);
					me->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->maxdmg * 0.5f);
				}
				me->UpdateDamagePhysical(BASE_ATTACK);
			}
        }
...
class spell_negative_energy_periodic : public SpellScriptLoader
{
public:
	spell_negative_energy_periodic() : SpellScriptLoader("spell_negative_energy_periodic") { }

	class spell_negative_energy_periodic_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_negative_energy_periodic_AuraScript);

		void PeriodicDamage(AuraEffectPtr aurEff)
		{
			aurEff->SetAmount(irand(1000, 1500));
		}

		void Register()
		{
			OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_negative_energy_periodic_AuraScript::PeriodicDamage, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
		}
	...
	{
		return new spell_negative_energy_periodic_AuraScript();
	}
};
...
void AddSC_boss_muru()
{
    ...
    new npc_blackhole();
	new spell_negative_energy_periodic();
}
