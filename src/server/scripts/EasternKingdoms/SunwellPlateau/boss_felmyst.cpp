...
public:
    ...
    struct boss_felmystAI : public ScriptedAI
    {
        ...
        void Reset()
        {
			...
			{
				hp_setted = true;
				me->SetMaxHealth(me->GetMaxHealth() * 0.6f);
				me->SetHealth(me->GetMaxHealth() * 0.6f);
			}
        }
        ...
        void SpellHit(Unit* caster, const SpellInfo* spell)
        {
            ...
            if (spell->Id == SPELL_FOG_INFORM)
            {
                ...
                {
                    summon->SetMaxHealth(caster->GetMaxHealth() * 0.5f);
                    summon->SetHealth(caster->GetMaxHealth() * 0.5f);
                    summon->CastSpell(summon, SPELL_FOG_CHARM, true);
                    summon->CastSpell(summon, SPELL_FOG_CHARM2, true);
					// Reduce damage, custome
					const CreatureTemplate* cinfo = summon->ToCreature()->GetCreatureTemplate();
					if (cinfo)
					{
						summon->SetBaseWeaponDamage(BASE_ATTACK, MINDAMAGE, cinfo->mindmg * 0.5f);
						summon->SetBaseWeaponDamage(BASE_ATTACK, MAXDAMAGE, cinfo->maxdmg * 0.5f);
					}
            ...
            if (phase == PHASE_FLIGHT)
            {
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_SUMMON_FOG:
                        ...
                        events.ScheduleEvent(EVENT_SUMMON_FOG, 15000);
                        break;
                }
            }
        }
