...
public:
	...
	struct boss_janalaiAI : public BossAI
	{
		...
		void Reset()
		{
			DespawnCreatures(NPC_AMANISHI_HATCHER1);
			DespawnCreatures(NPC_AMANISHI_HATCHER2);
			DespawnCreatures(NPC_HATCHLING);
			if (instance)
			...
			me->GetGameObjectListWithEntryInGrid(doorList, GO_JANALAI_FIRE_DOOR, 50.0f);
			if (!doorList.empty())
			{
				...
				case EVENT_FLAME_BREATH:
					...
					if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
					{
						...
						for (uint8 i = 0; i < 7; i++)
						{
							...
							//me->CastSpell(x, y, z, SPELL_FLAME_BREATH_1, true);
						}
						DoCast(target, SPELL_FLAME_BREATH);
						DoCast(target, SPELL_FLAME_BREATH_1, true);
						me->StopMoving();
