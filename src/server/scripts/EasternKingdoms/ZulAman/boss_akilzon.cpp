		...
		void UpdateAI(const uint32 diff)
		{
			...
			while (uint32 eventId = events.ExecuteEvent())
			{
				...
				case EVENT_STATIC_DISRUPTION:
					...
					events.ScheduleEvent(EVENT_SUMMON_KIDNAPPER, 8000);
					break;
				...
				case EVENT_SUMMON_KIDNAPPER:
					...
					events.RescheduleEvent(EVENT_SUMMON_KIDNAPPER, urand(18000, 20000));
					break;
...
public:
	...
	struct npc_amani_kidnapperAI : public ScriptedAI
	{
		...
		{
		target->CastSpell(me, SPELL_PLUCKED, true);
		...
		void UpdateAI(const uint32 diff)
		{
			if (arrived && numb > 0)
			{
				if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
				{
					...
					target->CastSpell(me, SPELL_MOUNT, true);
					me->AddAura(SPELL_PLUCKED, me->GetPlayer(*me, TargetGUID));
					me->GetMotionMaster()->MoveRandom(30.0f);
					arrived = false;
				}
			}
