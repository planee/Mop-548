...
enum PaladinSpells
{
	...
	PALADIN_SPELL_SEAL_OF_JUSTICE_DEBUFF         = 20170,
    ...
    PALADIN_SPELL_LUMINOUS_CHARGER_VISUAL        = 126666,
    PALADIN_SPELL_GLYPH_OF_THE_LUMINOUS_CHARGER  = 89401,
	...
	PALADIN_SPELL_GLYPH_OF_HOLY_WRATH            = 54923,
	PALADIN_SPELL_GLYPH_OF_FALLING_AVENGER       = 115931,
	PALADIN_SPELL_GLYPH_OF_DOUBLE_JEOPARDY       = 54922,
	PALADIN_SPELL_GLYPH_OF_DOUBLE_JEOPARDY_AURA  = 121027,
    GLYPH_OF_DEVINE_SHIELD                       = 146956,
    SPELL_PAL_GLYPH_OF_PILLAR_OF_LIGHT_EFFECT    = 148064
};
	        ...
	        void HandleOnHit()
	        {
	            if (Player* _player = GetCaster()->ToPlayer())
	            {
	                if (Unit* unitTarget = GetHitUnit())
	                {
						...
						if (AuraPtr proc = _player->GetAura(PALADIN_SPELL_SHIELD_OF_THE_RIGHTEOUS_PROC, _player->GetGUID()))
	                    {
        ...
        class spell_pal_tower_of_radiance_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                    if (Unit* target = GetHitUnit())
                    {
						if (_player->HasAura(PALADIN_SPELL_SELFLESS_HEALER) && GetSpellInfo()->Id == 82326)
                            _player->ModifyPower(POWER_HOLY_POWER, 1);
...
public:
    ...
    class spell_pal_sacred_shield_absorb_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(constAuraEffectPtr , int32 & amount, bool & )
        {
			...
			if (GetCaster())
			{
				//Protects the target with a shield of Holy Light for 6 seconds.The shield absorbs up to $m1 + 0.819 * HOLY_SP damage every $t1 sec.
				amount = int32(30 + GetCaster()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_HOLY) * 0.819f);
			}
        }
...
public:
    ...
    class spell_pal_sacred_shield_absorb_holy_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(constAuraEffectPtr , int32 & amount, bool & )
        {
			if (GetCaster())
			{
				// Protects the target with a shield of Holy Light for 30 seconds. The shield absorbs up to 342 + 1.17 * HOLY_SP damage every 6 sec.
            ...
            void HandleBeforeHit()
            {
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (_player->HasAura(GLYPH_OF_DEVINE_SHIELD))
                    {
                        ...
                        heal = _player->SpellHealingBonusTaken(_player, GetSpellInfo(), heal, HEAL);
...
public:
    ...
    class spell_pal_divine_purpose_proc_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pal_divine_purpose_proc_AuraScript);
        ...
        void OnProc(constAuraEffectPtr aurEff, ProcEventInfo& eventInfo)
        {
            if (Unit* caster = eventInfo.GetActor())
            {
                ...
                {
                    if (!player->HasAura(PALADIN_SPELL_DIVINE_PURPOSE))
                        player->CastWithDelay(100, player, PALADIN_SPELL_DIVINE_PURPOSE, true);
                }
            }
        }
        ...
        void Register()
        {
            OnEffectProc += AuraEffectProcFn(spell_pal_divine_purpose_proc_AuraScript::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_pal_divine_purpose_proc_AuraScript();
    }
};
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* unitTarget = GetHitUnit())
                    {
						...
						if (_player->HasAura(PALADIN_SPELL_SEAL_OF_JUSTICE))
							_player->CastSpell(unitTarget, PALADIN_SPELL_SEAL_OF_JUSTICE_DEBUFF, true);

						if (_player->HasAura(PALADIN_SPELL_GLYPH_OF_DOUBLE_JEOPARDY))
						{
							if (AuraPtr aura = _player->GetAura(PALADIN_SPELL_GLYPH_OF_DOUBLE_JEOPARDY_AURA))
							{
								if (aura->GetCasterGUID() != unitTarget->GetGUID())
								{
									int32 damege = int32(GetHitDamage() * 1.2f);
									SetHitDamage(damege);
								}
							}
						}
...
// Glyph of Luminous Charger - 98401
class spell_pal_glyph_of_luminous_charger : public SpellScriptLoader
{
public:
    spell_pal_glyph_of_luminous_charger() : SpellScriptLoader("spell_pal_glyph_of_luminous_charger") { }

    class spell_pal_glyph_of_luminous_charger_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pal_glyph_of_luminous_charger_AuraScript);

        void OnApply(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Player* _player = GetTarget()->ToPlayer())
            {
                if (_player->HasAura(PALADIN_SPELL_GLYPH_OF_THE_LUMINOUS_CHARGER))
                    _player->AddAura(PALADIN_SPELL_LUMINOUS_CHARGER_VISUAL, _player);
            }
        }
        ...
        void OnRemove(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Player* _player = GetTarget()->ToPlayer())
            {
                if (_player->HasAura(PALADIN_SPELL_LUMINOUS_CHARGER_VISUAL))
                    _player->RemoveAura(PALADIN_SPELL_LUMINOUS_CHARGER_VISUAL);
            }
        }

        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_pal_glyph_of_luminous_charger_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOUNTED, AURA_EFFECT_HANDLE_REAL);
            OnEffectRemove += AuraEffectRemoveFn(spell_pal_glyph_of_luminous_charger_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOUNTED, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_pal_glyph_of_luminous_charger_AuraScript();
    }
};
...
public:
	...
	class spell_pal_word_of_glory_heal_SpellScript : public SpellScript
	{
		...
		void HandleOnHit()
		{
			if (Player* _player = GetCaster()->ToPlayer())
			{
				if (Unit* unitTarget = GetHitUnit())
				{
					...
					if (unitTarget != _player && _player->HasAura(GLYPH_OF_PROTECTOR_OF_THE_INNOCENT))
					{
		...
		{
			OnHit += SpellHitFn(spell_pal_word_of_glory_heal_SpellScript::HandleOnHit);
		}
	};
...
/*######
# spell_holy_wrath_stun - 119072
#######*/

class spell_holy_wrath_stun : public SpellScriptLoader
{
public:
	spell_holy_wrath_stun() : SpellScriptLoader("spell_holy_wrath_stun") { }

	class spell_holy_wrath_stun_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_holy_wrath_stun_SpellScript)
		...
		void HandleDummy(SpellEffIndex effIndex)
		{
			...
			if (GetHitUnit()->GetTypeId() == TYPEID_PLAYER || !(GetHitUnit()->GetCreatureTypeMask() & CREATURE_TYPEMASK_DEMON_OR_UNDEAD))
			{
				// Check the Glyph PALADIN_SPELL_GLYPH_OF_HOLY_WRATH AND THE TARGET IS AN ELEMENTAL OR DRAGONKIN
				if (GetCaster()->HasAura(PALADIN_SPELL_GLYPH_OF_HOLY_WRATH) && 
					(GetHitUnit()->GetCreatureType() == CREATURE_TYPE_ELEMENTAL || 
					GetHitUnit()->GetCreatureType() == CREATURE_TYPE_DRAGONKIN))
					return;
				PreventHitAura();
			}
		}
		...
		void Register()
		{
			OnEffectHitTarget += SpellEffectFn(spell_holy_wrath_stun_SpellScript::HandleDummy, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_holy_wrath_stun_SpellScript();
	}
};
...
public:
	...
	class spell_pal_glyph_of_avenging_wrath_SpellScript : public SpellScript
	{
		...
		void HandleOnHit()
		{
			if (Unit* caster = GetCaster())
			{
				if (caster->HasAura(54927)) // Glyph of Avenging Wrath
				    caster->CastSpell(caster, 115547, true);
			}
		}
...
/*######
## spell_pal_glyph_of_double_jeopardy - 54922
######*/
...
public:
	...
	class spell_pal_glyph_of_double_jeopardy_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_pal_glyph_of_double_jeopardy_AuraScript);
		...
		bool OnCheckProc(ProcEventInfo& eventInfo)
		{
			if (Unit* caster = GetCaster())
			{
				if (DamageInfo* damageInfo = eventInfo.GetDamageInfo())
				{
					...
					{
						if (AuraPtr aura = caster->GetAura(PALADIN_SPELL_GLYPH_OF_DOUBLE_JEOPARDY))
						{
							for (int i = 0; i < MAX_SPELL_EFFECTS; i++)
							{
								if (AuraEffectPtr auraEff = aura->GetEffect(i))
								{
									if (auraEff->IsAffectingSpell(spellInfo))
									{
										return true;
									}
								}
		...
		void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
		{
			...
			if (Unit* caster = GetCaster())
			{
				...
				{
					caster->CastSpell(caster, PALADIN_SPELL_GLYPH_OF_DOUBLE_JEOPARDY_AURA, true, NULL, NULLAURA_EFFECT, target->GetGUID());
				}
			}
		}
		...
		void Register()
		{
			DoCheckProc += AuraCheckProcFn(spell_pal_glyph_of_double_jeopardy_AuraScript::OnCheckProc);
			OnEffectProc += AuraEffectProcFn(spell_pal_glyph_of_double_jeopardy_AuraScript::OnProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
	...
	AuraScript* GetAuraScript() const
	{
		return new spell_pal_glyph_of_double_jeopardy_AuraScript();
	}
};
...
//Guardian of Ancient King - 86698 - 86669 - 86659
...
public:
	...
	class spell_pal_guardian_of_ancient_kings_SpellScript : public SpellScript
	{
		...
		void Register()
		{
			OnCheckCast += SpellCheckCastFn(spell_pal_guardian_of_ancient_kings_SpellScript::CheckTarget);
		}
	};
...
// This proc is moved from "Unit.cpp" to "spell_paladin.cpp". Please avoid make procs (hacks) in Unit :) !
...
public:
    ...
    public:
        spell_pal_glyph_of_pillar_of_light_AuraScript()	{}

        enum iSpells {
            LUZ_SAGRADA             = 635,
            IMPOSICION_DE_MANOS     = 633,
            PALABRA_DE_GLORIA       = 130551,
            LLAMA_ETERNA            = 114163,
            PRISMA_SAGRADO          = 114871,
            LUZ_DEL_ALBA            = 85222,
            EJECUCION_POSPUESTA     = 114917,
            DESTELLO_DE_LUZ         = 19750,
            LUZ_DIVINA              = 82326,
            CHOQUE_SAGRADO          = 20473,
            CHOQUE_SAGRADO_CHAINED  = 25914,
            RADIANCIA_SAGRADA       = 82327
        };
        ...
        void HandleProc(constAuraEffectPtr aurEff, ProcEventInfo& eventInfo)
        {
            ...
            //This proc only when target is not the player itself
            if (caster == target)
                return;

            //This proc only with this spells
            if ((spellID == iSpells::LUZ_SAGRADA) || (spellID == iSpells::IMPOSICION_DE_MANOS) || (spellID == iSpells::PALABRA_DE_GLORIA) ||
                (spellID == iSpells::LLAMA_ETERNA) || (spellID == iSpells::PRISMA_SAGRADO) || (spellID == iSpells::LUZ_DEL_ALBA) ||
                (spellID == iSpells::EJECUCION_POSPUESTA) || (spellID == iSpells::DESTELLO_DE_LUZ) || (spellID == iSpells::LUZ_DIVINA) ||
                (spellID == iSpells::CHOQUE_SAGRADO) || (spellID == iSpells::CHOQUE_SAGRADO) || (spellID == iSpells::RADIANCIA_SAGRADA) ||
                //I don't find this spell in SpellWork but some spell in dbc exist in client (readed from trinity)
                (spellID == iSpells::CHOQUE_SAGRADO_CHAINED) || (spellID == 86452))
            {
                caster->CastSpell(target, SPELL_PAL_GLYPH_OF_PILLAR_OF_LIGHT_EFFECT, true);
            }
        }

        void Register()
        {
            OnEffectProc += AuraEffectProcFn(spell_pal_glyph_of_pillar_of_light_AuraScript::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
        }
...
void AddSC_paladin_spell_scripts()
{
    ...
    new spell_pal_glyph_of_luminous_charger();
