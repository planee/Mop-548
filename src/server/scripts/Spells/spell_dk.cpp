 ...
 * Scriptnames of files in this file should be prefixed with "spell_dk_". test
 */
...
enum DeathKnightSpells
{
    ...
    DK_SPELL_GLYPH_OF_DEATHS_EMBRACE            = 58677,
	...
	DK_SPELL_GLYPH_OF_STRANGULATE               = 58618,
    DK_SPELL_GLYPH_OF_UNHOLY_FRENZY             = 58616,
	...
	DK_SPELL_CRIMSONSCOURGE						= 81136,
    DK_SPELL_BLOOD_TAP                          = 45529,
	DK_SPELL_BLOODPLAGUE						= 55078
};
        ...
        class spell_dk_death_and_decay_AuraScript : public AuraScript
        {
            ...
            void OnRemove(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    {
                        if (target->HasAura(DK_SPELL_DEATH_AND_DECAY_DECREASE_SPEED))
                            target->RemoveAura(DK_SPELL_DEATH_AND_DECAY_DECREASE_SPEED);
                    }
                }
            ...
            void Register()
            {
                OnEffectRemove += AuraEffectApplyFn(spell_dk_death_and_decay_AuraScript::OnRemove, EFFECT_2, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
            }
        };
                ...
                if (Unit* caster = GetCaster())
                {
                    if (DynamicObject* dynObj = caster->GetDynObject(DK_SPELL_DESECRATED_GROUND))
                       if (caster->GetDistance(dynObj) <= 8.0f)
                           ...
                           caster->RemoveAura(DK_SPELL_DESECRATED_GROUND_IMMUNE);
                }
            }
...
public:
    ...
    class spell_dk_glyph_of_deaths_embrace_AuraScript : public AuraScript
    {
        ...
        void OnProc(constAuraEffectPtr aurEff, ProcEventInfo& procInfo)
        {
            ...
            if (Unit* caster = procInfo.GetActor())
            {
            ...
            void HandleOnHit()
            {
                if (Unit* caster = GetCaster())
                {
                    if (Player* player = caster->ToPlayer())
                    {
                        if (Unit* target = GetHitUnit())
                        {
                            if (GetSpellInfo()->Id == SPELL_DK_DEATH_COIL && player->HasGlyph(DK_SPELL_GLYPH_OF_DEATHS_EMBRACE) && target->HasUnitTypeMask(UNIT_MASK_MINION) && player->IsValidAssistTarget(target))
                                return;

                            if (player->HasSpell(DK_SPELL_BLOOD_TAP))
                            {
    ...
    public:
        ...
        class spell_dk_anti_magic_shell_self_AuraScript : public AuraScript
        {
            ...
            void Trigger(AuraEffectPtr aurEff, DamageInfo & /*dmgInfo*/, uint32 & absorbAmount)
            {
                ...
                // We gain 1 Runic Power for each 1000 damage absorbed
                int32 bp = int32(absorbAmount / 100);
                ...
                if (Unit* caster = GetCaster())
                {
                    if (leftAbsorb > 0 && caster->HasAura(DK_GLYPH_OF_REGENERATIVE_MAGIC))
                    {
                        ...
                        if (removeMode == AURA_REMOVE_BY_EXPIRE)
                        {
							...
							int32 cooldownMod = (int32)CalculatePct(40.0f, modifier) * IN_MILLISECONDS;
						...
						if (caster->GetAreaId() != 4281)
						{
							if (Map* map = caster->GetMap())
							{
								if (map->IsDungeon() || map->IsRaid())
								{
									...
									areaTrigger = sObjectMgr->GetGoBackTrigger(map->GetId());
									if (areaTrigger)
									{
										player->m_recallMap = areaTrigger->target_mapId;
										player->m_recallX = areaTrigger->target_X;
										player->m_recallY = areaTrigger->target_Y;
										player->m_recallZ = areaTrigger->target_Z;
										player->m_recallO = areaTrigger->target_Orientation + M_PI;
									}
								}
								else
								{
							...
							player->TeleportTo(0, 2359.41f, -5662.084f, 382.259f, 0.60f);
						}
						else
						{
...
//				else
//				{
//					uint32 basepoints = GetHitDamage();
//					damage += damage + caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.514f * basepoints;
//					caster->CastCustomSpell(target, SPELL_DK_DEATH_COIL_DAMAGE, &damage, NULL, NULL, true, 0, NULLAURA_EFFECT, caster->GetGUID());
//				}
//			}
//		}
//
...
//		void Register()
//		{
//			OnCheckCast += SpellCheckCastFn(spell_dk_death_coil_damage_SpellScript::CheckCast);
...
// 47633 - Death Coil heal
class spell_dk_shadow_infusion_by_heal : public SpellScriptLoader
{
public:
    spell_dk_shadow_infusion_by_heal() : SpellScriptLoader("spell_dk_shadow_infusion_by_heal") { }

    class sspell_dk_shadow_infusion_by_heal_SpellScript : public SpellScript
    {
        PrepareSpellScript(sspell_dk_shadow_infusion_by_heal_SpellScript);
        ...
        void HandleOnHit()
        {
			if (Player* _player = GetCaster()->ToPlayer())
			{
				...
				{
					if (_player->HasAura(DK_SPELL_DARK_INFUSION_STACKS))
					{
						int32 stacks = 0;

						if (AuraPtr DarkInfusion = _player->GetAura(DK_SPELL_DARK_INFUSION_STACKS))
						{
							stacks = DarkInfusion->GetStackAmount();
							if (stacks >= 4)
								_player->AddAura(SPELL_DK_DARK_TRANS_DRIVER, _player);
						}
						if (AuraPtr ShadowInfusion = _player->GetAura(DK_SPELL_DARK_INFUSION_STACKS))
							ShadowInfusion->ModStackAmount(+1);
						if (AuraPtr ShadowInfusion = pet->GetAura(DK_SPELL_DARK_INFUSION_STACKS))
							ShadowInfusion->ModStackAmount(+1);
					}
					else
					{
						_player->CastSpell(_player, DK_SPELL_DARK_INFUSION_STACKS);
						pet->CastSpell(pet, DK_SPELL_DARK_INFUSION_STACKS);
					}
				}
			}
        }
        ...
        void Register()
        {
            OnHit += SpellHitFn(sspell_dk_shadow_infusion_by_heal_SpellScript::HandleOnHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new sspell_dk_shadow_infusion_by_heal_SpellScript();
    }
};
        ...
        class spell_impl : public SpellScript
        {
            ...
            void HandleOnHit()
            {
				...
				if (target->IsImmunedToSpell(GetSpellInfo()))				{
					...
					GetCaster()->AddAura(DK_SPELL_ASPHYXIATE2, target);
				}
...
public:
	...
	class spell_dk_strangulate_SpellScript : public SpellScript
	{
		...
		void HandleOnHit()
		{
			...
			if (player->HasAura(DK_SPELL_GLYPH_OF_STRANGULATE))
			{
				if (Interrupt == true)
				{
					if (AuraPtr Strangulate = target->GetAura(DK_SPELL_STRANGULATE))
					{
						Strangulate->SetMaxDuration(Strangulate->GetMaxDuration() + 2000);
						Strangulate->RefreshDuration();
					}
				}
			}
		}
		...
		void Register()
		{
			OnHit += SpellHitFn(spell_dk_strangulate_SpellScript::HandleOnHit);
...
public:
    ...
    class spell_dk_unholy_frenzy_AuraScript : public AuraScript
    {
        ...
        void OnTick(constAuraEffectPtr aurEff)
        {
			if (Unit* _unit = GetCaster())
			{
				if (_unit->GetTypeId() != TYPEID_PLAYER)
					return;
				if (Player* _player = _unit->ToPlayer())
				{
					if (!_player->HasAura(DK_SPELL_GLYPH_OF_UNHOLY_FRENZY))
					{
						// Drain 2% of max health every 3 second
						int32 basepoints = _player->CountPctFromMaxHealth(aurEff->GetAmount());

						_player->DealDamage(_player, basepoints, NULL, NODAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
						_player->SendSpellNonMeleeDamageLog(_player, GetSpellInfo()->Id, basepoints, SPELL_SCHOOL_MASK_NORMAL, 0, 0, false, 0, false);
					}
				}
			}
        }
        ...
        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_dk_unholy_frenzy_AuraScript::OnTick, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
        }
    };
...
public:
	...
	class spell_dk_army_of_the_dead_SpellScript : public SpellScript
	{
		...
		void Register()
		{
			OnCheckCast += SpellCheckCastFn(spell_dk_army_of_the_dead_SpellScript::CheckTarget);
		}
	};
...
//////////////////////////////
//ID - 81136 Crimson Scourge//
//////////////////////////////
class spell_dk_crimsonScourge_effect : public SpellScriptLoader
{
public:
	spell_dk_crimsonScourge_effect() : SpellScriptLoader("spell_dk_crimsonScourge_effect") { }

	class spell_dk_crimsonScourge_effect_AuraScript : public AuraScript
	{
		PrepareAuraScript(spell_dk_crimsonScourge_effect_AuraScript);

		void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
		{
			if (Player* caster = GetCaster()->ToPlayer()){				
				Unit* target = caster->getVictim();
				if (target && !target->HasAura(DK_SPELL_BLOODPLAGUE)){					
					PreventDefaultAction();
				}
			}
		}
		...
		void Register()
		{			
			OnEffectProc += AuraEffectProcFn(spell_dk_crimsonScourge_effect_AuraScript::OnProc, EFFECT_1, SPELL_AURA_PROC_TRIGGER_SPELL);
	...
	AuraScript* GetAuraScript() const
	{
		return new spell_dk_crimsonScourge_effect_AuraScript();
	}
};
...
void AddSC_deathknight_spell_scripts()
{
	...
	//new spell_dk_death_coil_heal();   /*Esto es solo momentanio hasta que se pueda poner en Sepll_dk*/
	//new spell_dk_death_coil_damage();
	//new spell_dk_death_coil();
    new spell_dk_shadow_infusion_by_heal();
	...
	new spell_dk_crimsonScourge_effect();
}
