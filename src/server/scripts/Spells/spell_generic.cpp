...
class spell_gen_change_spec : public SpellScriptLoader
{
public:
	spell_gen_change_spec() : SpellScriptLoader("spell_gen_change_spec") {}

	class spell_gen_change_spec_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_gen_change_spec_SpellScript);
		...
		void Register()
		{
			OnCheckCast += SpellCheckCastFn(spell_gen_change_spec_SpellScript::CheckCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_gen_change_spec_SpellScript();
	}
};
...
//99556 -Spell teleport to MoltenFront
class spell_druid_teleport_to_moltenfront : public SpellScriptLoader
{
public:
	spell_druid_teleport_to_moltenfront() : SpellScriptLoader("spell_druid_teleport_to_moltenfront") {}

	class spell_druid_teleport_to_moltenfront_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_druid_teleport_to_moltenfront_SpellScript);
		...
		void HandleAfterCast()
		{
			...
			if (_npc)
			{
				if (Player* _player = _npc->ToPlayer())
				{
					_player->ToPlayer()->TeleportTo(861, 1034, 368, 39, 0);
				}
			}
		...
		void Register()
		{
			AfterCast += SpellCastFn(spell_druid_teleport_to_moltenfront_SpellScript::HandleAfterCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_druid_teleport_to_moltenfront_SpellScript();
	}
};


class spell_gen_jewelcrafting_quest_stardust2 : public SpellScriptLoader
{
public:
	spell_gen_jewelcrafting_quest_stardust2() : SpellScriptLoader("spell_gen_jewelcrafting_quest_stardust2") {}

	class spell_gen_jewelcrafting_quest_stardust2_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_gen_jewelcrafting_quest_stardust2_SpellScript);
// 66118 Leeching Swarm
		bool Validate(SpellInfo const* /*spellEntry*/)
		{
			if (!sSpellMgr->GetSpellInfo(73667))
				return false;
			return true;
		}
		...
		void HandleOnCast()
		{
			Unit* _unit = GetCaster();
			Unit* _target = GetExplTargetUnit();
			if (_unit && _target)
			{
				if (Player* player = _unit->ToPlayer())
				{
					if (_target->GetCreatureType() == CREATURE_TYPE_HUMANOID)
					{
						player->KilledMonsterCredit(39237);
					}
				}
			}
		...
		void Register()
		{
			OnCast += SpellCastFn(spell_gen_jewelcrafting_quest_stardust2_SpellScript::HandleOnCast);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_gen_jewelcrafting_quest_stardust2_SpellScript();
	}
};
...
public:
	...
	private:
		...
		void SetBonusValueForEffect(SpellEffIndex effIndex, int32 value, constAuraEffectPtr aurEff)
		{
		...
		void CalculateAmount(constAuraEffectPtr aurEff, int32& amount, bool& /*canBeRecalculated*/)
		{
			if (GetCaster()->HasAura(SPELL_MIXOLOGY) && GetCaster()->HasSpell(GetSpellInfo()->Effects[EFFECT_0].TriggerSpell))
			{
				...
				case SPELL_ELIXIR_OF_GIANTH_GROWTH:
					...
					break;
				default:
					sLog->outDebug(LOG_FILTER_GENERAL, "SPELL", "SpellId %d couldn't be processed in spell_gen_mixology_bonus", GetId());
					break;
				}
				amount += bonus;
			}
...
class spell_gen_battle_fatigue : public SpellScriptLoader
{
    public:
		...
		class spell_gen_battle_fatigue_SpellScript : public SpellScript
		{
			PrepareSpellScript(spell_gen_battle_fatigue_SpellScript);
			...
			SpellCastResult CheckCast()
			{
				...
				if (caster->GetMap()->IsDungeon() || caster->GetMap()->IsRaid() || !caster->IsPvP())
					return SPELL_FAILED_DONT_REPORT;
			...
			void Register()
			{
				OnCheckCast += SpellCheckCastFn(spell_gen_battle_fatigue_SpellScript::CheckCast);
			}
		};

		SpellScript* GetSpellScript() const
		{
			return new spell_gen_battle_fatigue_SpellScript();
		}
...
// Synapse Springs Estadistic - 96228, 96229, 96230
class spell_gen_synapse_springs_estadistic : public SpellScriptLoader
{
public:
	spell_gen_synapse_springs_estadistic() : SpellScriptLoader("spell_gen_synapse_springs_estadistic") { }

	class spell_gen_synapse_springs_estadistic_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_gen_synapse_springs_estadistic_SpellScript);
		...
		void HandleOnHit()
		{
			...
			{
				int32 basepoints0 = 0;
				uint8 playerLevel = GetCaster()->ToPlayer()->getLevel();
				if (playerLevel <= 80)
					basepoints0 = 192;
				else if (playerLevel > 80 && playerLevel <= 85)
					basepoints0 = 480;
				else if (playerLevel > 85 && playerLevel <= 90)
					basepoints0 = 1920;
				if (AuraPtr atributte = _player->GetAura(SYNAPSE_SPRINGS_STRENGTH))
				{
					atributte->GetEffect(0)->ChangeAmount(basepoints0);
				}
				if (AuraPtr atributte = _player->GetAura(SYNAPSE_SPRINGS_AGILITY))
				{
					atributte->GetEffect(0)->ChangeAmount(basepoints0);
				}
				if (AuraPtr atributte = _player->GetAura(SYNAPSE_SPRINGS_INTELLECT))
				{
					atributte->GetEffect(0)->ChangeAmount(basepoints0);
				}
			}
		...
		void Register()
		{
			OnHit += SpellHitFn(spell_gen_synapse_springs_estadistic_SpellScript::HandleOnHit);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_gen_synapse_springs_estadistic_SpellScript();
	}
};
    ...
    class script_impl final : public AuraScript
    {
		...
		void HandleEffectPeriodic(constAuraEffectPtr aurEff)
        {
            ...
            if (!target->HasAura(spellsByStack[stackAmount]))
                target->CastSpell(target, spellsByStack[stackAmount], true, NULL, aurEff);
        }
...
class spell_gen_brewfest_ram_fatigue_tracker final : public SpellScriptLoader
{
    class script_impl final : public AuraScript
    {
		...
		void HandleEffectPeriodic(constAuraEffectPtr eff)
        {
            Player* const caster = GetCaster()->ToPlayer();
            if (!caster)
                return;
            ...
            AuraPtr fatigueAura = caster->GetAura(SPELL_FATIGUE_TRACKER);
            if (!fatigueAura)
            {
                if (m_data.fatigue > 0)
...
class spell_gen_brewfest_ram_exhausted final : public SpellScriptLoader
{
    class script_impl final : public AuraScript
    {
		...
		void HandleEffectRemove(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (AuraPtr aura = target->GetAura(SPELL_SPEED_TRACKER))
                aura->SetStackAmount(1);
            if (AuraPtr aura = target->GetAura(SPELL_FATIGUE_TRACKER))
...
class spell_brewfest_ram_race_increase_duration final : public SpellScriptLoader
{
    class script_impl final : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            ...
            if (AuraPtr aur = target->GetAura(SPELL_RAM_AURA))
            {
...
class spell_gen_orb_of_power final : public SpellScriptLoader
{
    class script_impl final : public AuraScript
    {
		...
		void OnRemove(constAuraEffectPtr aurEff, AuraEffectHandleModes mode){
			if (auto eff0 = GetEffect(EFFECT_0))
				eff0->SetAmount((std::max(100, int32(eff0->GetAmount()  - 5))) * -1);

			if (auto eff1 = GetEffect(EFFECT_1))
				eff1->SetAmount((eff1->GetAmount() + 30) * -1);

			if (auto eff2 = GetEffect(EFFECT_2))
				eff2->SetAmount((std::min(100, int32(eff2->GetAmount() + 10))) * -1);
		}

        void Register() final
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(script_impl::HandleEffectPeriodic, EFFECT_3, SPELL_AURA_PERIODIC_DUMMY);
			OnEffectRemove += AuraEffectRemoveFn(script_impl::OnRemove, EFFECT_3, SPELL_AURA_PERIODIC_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };
    ...
    public:
        ...
        class spell_gen_zandalari_warding_AuraScript : public AuraScript
        {
			...
			void CalculateAmount(constAuraEffectPtr /*aurEff*/, int32 & amount, bool& /*canBeRecalculated*/)
            {
                if (Unit* caster = GetCaster())
                {
                    if (AuraPtr stackAura = caster->GetAura(138967))
...
class spell_gen_alchemists_flask : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_alchemists_flask_SpellScript : public SpellScript
        {
			...
			bool Validate(SpellInfo const* /*spellEntry*/)
			{
				if (!sSpellMgr->GetSpellInfo(ALCHEMISTS_STRENGTH) || !sSpellMgr->GetSpellInfo(ALCHEMISTS_AGILITY) || !sSpellMgr->GetSpellInfo(ALCHEMISTS_INTELLECT))
					return false;
				return true;
			}
			...
			void HandleOnHit()
			{
				...
				if (Player* _player = GetCaster()->ToPlayer())
				{
					if (_player->HasAura(ALCHEMISTS_STRENGTH) || _player->HasAura(ALCHEMISTS_AGILITY) || _player->HasAura(ALCHEMISTS_INTELLECT))
					{
						_player->RemoveAura(ALCHEMISTS_STRENGTH);
						_player->RemoveAura(ALCHEMISTS_AGILITY);
						_player->RemoveAura(ALCHEMISTS_INTELLECT);
					}
					int32 basepoints0 = 0;
					uint8 playerLevel = GetCaster()->ToPlayer()->getLevel();
					if (playerLevel <= 60)
					...
					{
					case STAT_STRENGTH:
						_player->CastCustomSpell(_player, ALCHEMISTS_STRENGTH, &basepoints0, NULL, NULL, true);
						break;
					case STAT_AGILITY:
						_player->CastCustomSpell(_player, ALCHEMISTS_AGILITY, &basepoints0, NULL, NULL, true);
						break;
					case STAT_INTELLECT:
						_player->CastCustomSpell(_player, ALCHEMISTS_INTELLECT, &basepoints0, NULL, NULL, true);
						break;
					}
				}
			}
...
// Mouthwatering Brew and Its Uses - 105583 
class spell_gen_mouthwatering_brew : public SpellScriptLoader
{
    public:
        spell_gen_mouthwatering_brew() : SpellScriptLoader("spell_gen_mouthwatering_brew") { }

        class spell_gen_mouthwatering_brew_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_mouthwatering_brew_SpellScript);
            ...
            void HandleOnHit()
            {
                ...
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (_player->GetQuestStatus(29889) == QUEST_STATUS_INCOMPLETE)//Quest Borrowed Brew
                        _player->KilledMonsterCredit(56260);
                }
            }

            void Register()
            {
                OnHit += SpellHitFn(spell_gen_mouthwatering_brew_SpellScript::HandleOnHit);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gen_mouthwatering_brew_SpellScript();
        }
};

enum SpellEremita
{
    SPELL_TRIMMING          = 123303, 
    SPELL_TREE_FINISHED     = 123307,
    SPELL_PAINTING          = 123346,
    SPELL_PAINTED_FINISHED  = 123924,
    SPELL_FEEDING           = 123945,
    SPELL_FEED_FINISHED     = 123946,
};

// Called by Trimming, Painting, Feeding - 123303, 123346, 123945
class spell_gen_a_visit_to_lorewalker_cho : public SpellScriptLoader
{
public:
    spell_gen_a_visit_to_lorewalker_cho() : SpellScriptLoader("spell_gen_a_visit_to_lorewalker_cho") { }

    class spell_gen_a_visit_to_lorewalker_cho_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_a_visit_to_lorewalker_cho_AuraScript);
            ...
            if (Unit* caster = GetCaster())
            {
                ...
                if (Unit* target = GetTarget())
                {
                    ...
                    if (removeMode == AURA_REMOVE_BY_EXPIRE)
                    {
                        ...
                        {
                        case SPELL_TRIMMING:
                            if (caster->ToPlayer()->GetQuestStatus(31130) == QUEST_STATUS_INCOMPLETE) //Quest A Visit with Lorewalker Cho
                            caster->CastSpell(caster, SPELL_TREE_FINISHED);
                            break;
                        case SPELL_PAINTING:
                            if (caster->ToPlayer()->GetQuestStatus(31130) == QUEST_STATUS_INCOMPLETE) //Quest A Visit with Lorewalker Cho
                            caster->CastSpell(caster, SPELL_PAINTED_FINISHED);
                            break;
                        case SPELL_FEEDING:
                            if (caster->ToPlayer()->GetQuestStatus(31130) == QUEST_STATUS_INCOMPLETE) //Quest A Visit with Lorewalker Cho
                            caster->CastSpell(caster, SPELL_FEED_FINISHED);
                            break;
                        }
                    }
                }
        ...
        void Register()
        {
            AfterEffectRemove += AuraEffectRemoveFn(spell_gen_a_visit_to_lorewalker_cho_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_STUN, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_gen_a_visit_to_lorewalker_cho_AuraScript();
    }
};
...
{
    //Spell
    SPELL_GUILD_BATTLE_STANDARD = 90216,
    //Creature
    NPC_GUILD_BATTLE_STANDARD_A5  = 48115,
    NPC_GUILD_BATTLE_STANDARD_A10 = 48633,
    NPC_GUILD_BATTLE_STANDARD_A15 = 48634,
    NPC_GUILD_BATTLE_STANDARD_H5  = 48636,
    NPC_GUILD_BATTLE_STANDARD_H10 = 48637,
    NPC_GUILD_BATTLE_STANDARD_H15 = 48638,
};

// called by guild battle standard 89481 
class spell_gen_guild_battle_standard : public SpellScriptLoader
{
public:
    spell_gen_guild_battle_standard() : SpellScriptLoader("spell_gen_guild_battle_standard") { }

    class spell_gen_guild_battle_standard_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_guild_battle_standard_AuraScript);
        ...
        void OnPeriodic(constAuraEffectPtr /*aurEff*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                {
                case NPC_GUILD_BATTLE_STANDARD_A5:
                case NPC_GUILD_BATTLE_STANDARD_H5:
                    bp = 5;
                    break;
                case NPC_GUILD_BATTLE_STANDARD_A10:
                case NPC_GUILD_BATTLE_STANDARD_H10:
                    bp = 10;
                    break;
                case NPC_GUILD_BATTLE_STANDARD_A15:
                case NPC_GUILD_BATTLE_STANDARD_H15:
                    bp = 15;
                    break;
                }
                if (bp)
                caster->CastCustomSpell(caster, SPELL_GUILD_BATTLE_STANDARD, &bp, &bp, &bp, 0, 0, false);
            }
        }

        void Register()
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_gen_guild_battle_standard_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_gen_guild_battle_standard_AuraScript();
    }
};

// called by guild battle standard 90216 
class spell_gen_filter_target_guild_battle_standard : public SpellScriptLoader
{
public:
    spell_gen_filter_target_guild_battle_standard() : SpellScriptLoader("spell_gen_filter_target_guild_battle_standard") { }

    class GuildTargetSelector
    {
    public:
        explicit GuildTargetSelector(Unit* owner) : _caster(owner) {}
        ...
        bool operator()(WorldObject* object) const
        {
            ...
            if (!object->ToPlayer()->GetGuild())
                return true;

            if (!_caster->ToPlayer()->GetGuild())
                return true;

            if (_caster->ToPlayer() == object->ToPlayer())
                return false;

            if (Guild* guildtarget = object->ToPlayer()->GetGuild())
            if (Guild* guildowner = _caster->ToPlayer()->GetGuild())
            if (guildtarget != guildowner)
                return true;
    ...
    class spell_gen_filter_target_guild_battle_standard_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_filter_target_guild_battle_standard_SpellScript);
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            if (Unit* caster = GetCaster())
            {
                if (caster->ToTempSummon())
                if (Unit* Owner = caster->ToTempSummon()->GetOwner())
                    targets.remove_if(GuildTargetSelector(Owner));
            }
        }
        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_filter_target_guild_battle_standard_SpellScript::FilterTargets, EFFECT_0, TARGET_UNIT_SRC_AREA_ALLY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_filter_target_guild_battle_standard_SpellScript::FilterTargets, EFFECT_1, TARGET_UNIT_SRC_AREA_ALLY);
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_filter_target_guild_battle_standard_SpellScript::FilterTargets, EFFECT_2, TARGET_UNIT_SRC_AREA_ALLY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_filter_target_guild_battle_standard_SpellScript();
    }   
};
...
public:
    ...
    class  spell_gen_golbin_glider_AuraScript : public AuraScript
    {
        ...
        void OnUpdate(const uint32 /*diff*/)
        {
            ...
            AuraPtr aura = player->GetAura(SpellInfo->Id);
            if (!aura)
                return;
            ...
            if (AuraPtr aura = player->GetAura(GetSpellInfo()->Id))
...
// Powder Round 130994  
class spell_gen_powder_round_target : public SpellScriptLoader
{
public:
    spell_gen_powder_round_target() : SpellScriptLoader("spell_gen_powder_round_target") { }

    enum PowderRoundTarget
    {
        //Creature affected by this spell
        NPC_KRIKTHIK_LIMBPINCER_1 = 61960,
        NPC_KRIKTHIK_LIMBPINCER_2 = 61754,
        NPC_KRIKTHIK_DRONE        = 61181,
        NPC_THUNDER_HOLD_SOLDIER  = 66291,
    };

    class spell_gen_powder_round_target_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_powder_round_target_SpellScript);
        ...
        void CorrectTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(NpcCheck());

        }

        void Register()
        {
            OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_gen_powder_round_target_SpellScript::CorrectTargets, EFFECT_0, TARGET_UNIT_DEST_AREA_ENTRY);
        }
    };
    ...
    public:
        bool operator()(const WorldObject* target)
        {
            ...
            {
                if (target->GetEntry() == NPC_KRIKTHIK_LIMBPINCER_1)
                    return false;
                if (target->GetEntry() == NPC_KRIKTHIK_LIMBPINCER_2)
                    return false;
                if (target->GetEntry() == NPC_KRIKTHIK_DRONE)
                    return false;
                if (target->GetEntry() == NPC_THUNDER_HOLD_SOLDIER)
                    return false;
            }
            return true;
        }
    ...
    SpellScript* GetSpellScript() const
    {
        return new spell_gen_powder_round_target_SpellScript();
    }
};
...
void AddSC_generic_spell_scripts()
{
	...
	new spell_gen_synapse_springs_estadistic();
   ...
   // new spell_gen_zandalari_warding();
	...
	//new spell_druid_teleport_to_moltenfront();
	new spell_gen_jewelcrafting_quest_stardust2();
    ...
    new spell_gen_mouthwatering_brew();
    new spell_gen_a_visit_to_lorewalker_cho();
    new spell_gen_guild_battle_standard();
    new spell_gen_filter_target_guild_battle_standard();
    new spell_gen_golbin_glider();
    new spell_gen_powder_round_target();
}
