...
/* Scrtip for combining two item and crate a key  By RoG_Mayor*/
class spell_item_jagged_red_crystal : public SpellScriptLoader
{
public:
    spell_item_jagged_red_crystal() : SpellScriptLoader("spell_item_jagged_red_crystal") { }

    class spell_item_jagged_red_crystal_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_jagged_red_crystal_SpellScript)

        void HandleOnHit()
        {
            if (Player* target = GetHitPlayer())
            {
                target->AddItem(30438, 1);
            }
        }

        void Register()
        {
            OnHit += SpellHitFn(spell_item_jagged_red_crystal_SpellScript::HandleOnHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_item_jagged_red_crystal_SpellScript();
    }
};
...
{
    typedef std::array<std::array<uint32, Dimension>, Size> Spell;

public:
    spell_item_readiness_generic(const char* scriptName, Spell const& spells) : SpellScriptLoader(scriptName), _spells(spells) { }
    ...
    {
    public:
        spell_item_readiness_generic_AuraScript(Spell const& spells)
        ...
        void Check(constAuraEffectPtr aurEff)
        {
            if (!GetCaster() || !GetCaster()->ToPlayer())
                return;
...
public:
    ...
    class spell_item_drinks_AuraScript : public AuraScript
    {
        ...
        void CorrectAmounts(constAuraEffectPtr aurEff, AuraEffectHandleModes mode)
        {
            Unit* l_Caster = GetCaster();
            if (!l_Caster)
                return;

            AuraPtr l_Aura = GetAura();
            AuraEffectPtr l_AuraEffect = l_Aura->GetEffect(SpellEffIndex::EFFECT_0);
            AuraEffectPtr l_DummyEffect = l_Aura->GetEffect(SpellEffIndex::EFFECT_1);
        ...
        void OnPeriodic(constAuraEffectPtr aurEff)
        {
            ...
            if (Unit* l_Target = GetUnitOwner())
            {
                if (Player* l_Player = l_Target->ToPlayer())
                    l_Target->CastSpell(l_Target, BladesOfRenatakiSpells::BLADES, true, l_Player->GetItemByGuid(aurEff->GetBase()->GetCastItemGUID()), nullptr, l_Target->GetGUID());
            }
        }
...
public:
    ...
    class spell_item_spark_of_zandalar_AuraScript : public AuraScript
    {
        ...
        void CheckStack(constAuraEffectPtr aurEff, AuraEffectHandleModes mode)
        {
            if (Unit* l_Owner = GetUnitOwner())
            {
                AuraPtr l_Aura = aurEff->GetBase();
                if (l_Aura->GetStackAmount() >= 10)
                {
                    if (AuraPtr l_ZandalariWarrior = l_Owner->GetAura(SparkOfZandalar::ZANDALARI_WARRIOR))
...
public:
    ...
    class spell_aura_reorigination_AuraScript : public AuraScript
    {
        ...
        void DoubleRating(constAuraEffectPtr /*aurEff*/, int32 & amount, bool& /*canBeRecalculated*/)
        {
            if (!GetUnitOwner()) ///< Can that even be null?
                return;
            ...
            {
                amount = l_Player->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + Rating);
            }
        }

        template<CombatRating Rating>
        void ZeroRating(constAuraEffectPtr /*aurEff*/, int32 & amount, bool& /*canBeRecalculated*/)
        {
            if (!GetUnitOwner()) ///< Can that even be null?
                return;
            ...
            if (Player* l_Player = GetUnitOwner()->ToPlayer())
            {
                amount = -static_cast<int32>(l_Player->GetUInt32Value(PLAYER_FIELD_COMBAT_RATING_1 + Rating));
            }
        }
    ...
    class spell_item_zandalari_warding_AuraScript : public AuraScript
    {
        ...
        void CalculateAmount(constAuraEffectPtr /*aurEff*/, int32 & amount, bool& /*canBeRecalculated*/)
        {
            if (Unit* l_Caster = GetCaster())
            {
                ...
                if (AuraPtr l_ZandalariBlessing = l_Caster->GetAura(eSpells::BlessingOfZuldazar))
                {
                    l_BlessingCounts += l_ZandalariBlessing->GetStackAmount();
                    l_ZandalariBlessing->Remove();
                }

                amount *= l_BlessingCounts;
            }
        }

        void Register() override
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_item_zandalari_warding_AuraScript::CalculateAmount, EFFECT_0, SPELL_AURA_SCHOOL_ABSORB);
        }
    };
    ...
    class spell_taunt_flag_targeting_SpellScript : public SpellScript
    {
        ...
        SpellCastResult CheckCast()
        {
            ...
            if (Player* l_Caster = GetCaster()->ToPlayer())
            {
                ...
                WoWSource::AllWorldObjectsInRange l_Check(l_Caster, l_SearchDist);
                WoWSource::WorldObjectListSearcher<WoWSource::AllWorldObjectsInRange> l_Searcher(l_Caster, l_Targets, l_Check);
...
public:
    ...
    class spell_item_fusion_fire_core_AuraScript : public AuraScript
    {
        ...
        void HandleProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            if (DamageInfo* l_DamageInfos = eventInfo.GetDamageInfo())
            {
...
public:
    ...
    class spell_item_juggernault_focusing_crystal_AuraScript : public AuraScript
    {
        ...
        void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            if (DamageInfo* l_DamageInfo = eventInfo.GetDamageInfo())
            {
...
public:
    ...
    class spell_item_multistrike_AuraScript : public AuraScript
    {
        ...
        void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            if (DamageInfo* l_DmgInfo = eventInfo.GetDamageInfo())
            {
...
public:
    ...
    class spell_item_multistrike_heal_AuraScript : public AuraScript
    {
        ...
        void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            if (HealInfo* l_HealInfos = eventInfo.GetHealInfo())
            {
...
// 99246 99245 - Druid of the Flames
class spell_item_fandral_flamescythe : public SpellScriptLoader
{
public:
    spell_item_fandral_flamescythe() : SpellScriptLoader("spell_item_fandral_flamescythe") { }

    class spell_item_fandral_flamescythe_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_fandral_flamescythe_AuraScript);
        ...
        void OnRemove(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                caster->ToPlayer()->RemoveSpellCooldown(99245, true);
                caster->ToPlayer()->ReduceSpellCooldown(99246, true);
            }
        ...
        void Register() override
        {
            AfterEffectRemove += AuraEffectRemoveFn(spell_item_fandral_flamescythe_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_item_fandral_flamescythe_AuraScript();
    }
};

enum FoodHability
{
    SPELL_FOOD_STRENGTH = 87545,
    SPELL_FOOD_AGILITY = 87546,
    SPELL_FOOD_INTELLECT = 87547,
    SPELL_DRINK = 87959,
};

//Fortune Cookie 87604
class spell_item_fortune_cookie : public SpellScriptLoader
{
public:
    spell_item_fortune_cookie() : SpellScriptLoader("spell_item_fortune_cookie") { }

    class spell_item_fortune_cookie_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_fortune_cookie_SpellScript);
        ...
        void HandleOnHit()
        {
            ...
            {
                _player->CastSpell(_player, SPELL_DRINK, true);

                if (_player->HasAura(SPELL_FOOD_STRENGTH) || _player->HasAura(SPELL_FOOD_AGILITY) || _player->HasAura(SPELL_FOOD_INTELLECT))
                {
                    _player->RemoveAura(SPELL_FOOD_STRENGTH);
                    _player->RemoveAura(SPELL_FOOD_AGILITY);
                    _player->RemoveAura(SPELL_FOOD_INTELLECT);
                }
            }
        }
        ...
        void Register()
        {
            OnHit += SpellHitFn(spell_item_fortune_cookie_SpellScript::HandleOnHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_item_fortune_cookie_SpellScript();
    }

    class spell_item_fortune_cookie_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_fortune_cookie_AuraScript);
        ...
        void OnPeriodic(constAuraEffectPtr aurEff)
        {
            ...
            if (Player* _player = GetCaster()->ToPlayer())
            {
                ...
                {
                case STAT_STRENGTH:
                    _player->CastSpell(_player, SPELL_FOOD_STRENGTH, true);
                    break;
                case STAT_AGILITY:
                    _player->CastSpell(_player, SPELL_FOOD_AGILITY, true);
                    break;
                case STAT_INTELLECT:
                    _player->CastSpell(_player, SPELL_FOOD_INTELLECT, true);
                    break;
                }
            }
        }
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_item_fortune_cookie_AuraScript::OnPeriodic, EFFECT_1, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_item_fortune_cookie_AuraScript();
    }
};

// Release Puppies - 127233
class spell_item_release_puppies : public SpellScriptLoader
{
public:
    spell_item_release_puppies() : SpellScriptLoader("spell_item_release_puppies") { }

    class spell_item_release_puppies_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_release_puppies_SpellScript);
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_item_release_puppies_SpellScript::CheckTarget);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_item_release_puppies_SpellScript();
    }
};
enum AnglerFishingRaft
{
    SPELL_RAFTLING = 115841,
};

//Called by Anglers Fishing Raft 124036
class spell_item_anglers_fishing_raft : public SpellScriptLoader
{
public:
    spell_item_anglers_fishing_raft() : SpellScriptLoader("spell_item_anglers_fishing_raft") { }

    class spell_item_anglers_fishing_raft_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_anglers_fishing_raft_AuraScript);

        void OnApply(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* target = GetTarget())
            if (target->IsInWater())
                target->AddAura(SPELL_RAFTLING, target);
        }
        ...
        void OnPeriodic(constAuraEffectPtr aurEff)
        {
            if (Unit* target = GetTarget())
            {
                ...
                ZLiquidStatus water = target->GetMap()->getLiquidStatus(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), MAP_ALL_LIQUIDS, &liquid_status);
                if (water && (water == LIQUID_MAP_WATER_WALK || water == LIQUID_MAP_ABOVE_WATER))
                    target->AddAura(SPELL_RAFTLING, target);
                else
                    target->RemoveAura(SPELL_RAFTLING);
            }
        }
        ...
        {
            if (Unit* target = GetTarget())
            if (target->HasAura(SPELL_RAFTLING))
                target->RemoveAura(SPELL_RAFTLING);
        }

        void Register()
        {
            AfterEffectApply += AuraEffectApplyFn(spell_item_anglers_fishing_raft_AuraScript::OnApply, EFFECT_0, SPELL_AURA_WATER_WALK, AuraEffectHandleModes(AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY));
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_item_anglers_fishing_raft_AuraScript::OnPeriodic, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
            AfterEffectRemove += AuraEffectRemoveFn(spell_item_anglers_fishing_raft_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_WATER_WALK, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_item_anglers_fishing_raft_AuraScript();
    }
};

//Called by Shear Wings 105759
enum eKniveIncisive
{
    NPC_GLITTERING_AMBERFLY = 54559,
    SPELL_SHEAR_WINGS = 105759,
    QUEST_HUE = 29893,
    ITEM_AMBERWING = 76115,
};

class spell_item_knive_incisive : public SpellScriptLoader
{
public:
    spell_item_knive_incisive() : SpellScriptLoader("spell_item_knive_incisive") { }

    class spell_item_knive_incisive_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_item_knive_incisive_SpellScript);
        ...
        SpellCastResult CheckRequirement()
        {
            if (GetExplTargetUnit())
            {
                ...
                if (GetExplTargetUnit()->GetEntry() != NPC_GLITTERING_AMBERFLY)
        ...
        void HandleOnHit()
        {
            ...
            if (Unit* caster = GetCaster())
            if (Unit* target = GetHitUnit())
            if (caster->ToPlayer()->GetQuestStatus(QUEST_HUE) == QUEST_STATUS_INCOMPLETE)
            {
                caster->ToPlayer()->AddItem(ITEM_AMBERWING, 1);
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_item_knive_incisive_SpellScript::CheckRequirement);
            OnHit += SpellHitFn(spell_item_knive_incisive_SpellScript::HandleOnHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_item_knive_incisive_SpellScript();
    }
};
...
public:
    ...
    class spell_item_mind_control_cap_SpellScript : public SpellScript
    {
        ...
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_GNOMISH_MIND_CONTROL_CAP))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_DULLARD))
                return false;
            return true;
        }
...
public:
    ...
    class spell_item_gnomish_universal_remote_SpellScript : public SpellScript
    {
        ...
        bool Validate(SpellInfo const* /*spellInfo*/) override
        {
            if (!sSpellMgr->GetSpellInfo(SPELL_CONTROL_MACHINE))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_MOBILITY_MALFUNCTION))
                return false;
            if (!sSpellMgr->GetSpellInfo(SPELL_TARGET_LOCK))
                return false;
            return true;
        }
...
class spell_item_zen_alchemist_stone : public SpellScriptLoader
{
public:
    spell_item_zen_alchemist_stone() : SpellScriptLoader("spell_item_zen_alchemist_stone") { }

    enum SpellAbility
    {
        SPELL_AGILITY = 60233,
        SPELL_STRENGHT = 60229,
        SPELL_INTELLECT = 60234,
    };

    class spell_item_zen_alchemist_stone_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_item_zen_alchemist_stone_AuraScript);
        ...
        void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            auto highestStat = Caster->ToPlayer()->GetHighestStat();

            switch (highestStat)
            {
            case STAT_AGILITY:
                trigger_spell_id = SPELL_AGILITY;
                break;
            case STAT_STRENGTH:
                trigger_spell_id = SPELL_STRENGHT;
                break;
            case STAT_INTELLECT:
                trigger_spell_id = SPELL_INTELLECT;
                break;
            }
            if (!trigger_spell_id)
                return;

            if (AuraPtr aura = Caster->AddAura(trigger_spell_id, Caster))
                aura->GetEffect(EFFECT_0)->ChangeAmount(4353);
            
            Caster->ToPlayer()->AddSpellCooldown(GetSpellInfo()->Id, 0, time(NULL) + 60);
        }

        void Register()
        {
            OnEffectProc += AuraEffectProcFn(spell_item_zen_alchemist_stone_AuraScript::OnProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_item_zen_alchemist_stone_AuraScript();
    }
};

void AddSC_item_spell_scripts()
{
    new spell_item_jagged_red_crystal();
    ...
    new spell_item_fandral_flamescythe();
    new spell_item_fortune_cookie();
    new spell_item_release_puppies();
    new spell_item_anglers_fishing_raft();
    new spell_item_knive_incisive();
