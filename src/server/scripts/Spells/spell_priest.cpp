...
enum PriestSpells
{
    ...
    PRIEST_LEAP_OF_FAITH_VISUAL                     = 92572,
    ...
    PRIEST_SPELL_DIVINE_STAR_HOLY                   = 110744,
    PRIEST_SPELL_DIVINE_STAR_SHADOW                 = 122121,
    PRIEST_SPELL_DIVINE_STAR_HEAL                   = 110745,
    PRIEST_SPELL_DIVINE_STAR_DAMAGE                 = 122128,
    PRIEST_SPELL_DIVINE_STAR_VISUAL_HOLY            = 58880,
    PRIEST_SPELL_DIVINE_STAR_VISUAL_SHADOW          = 122127,
    ...
    PRIEST_SURGE_OF_LIGHT_VISUAL                    = 128654,
    PRIEST_SURGE_OF_DARKNESS                        = 87160,
    PRIEST_SURGE_OF_DARKNESS_VISUAL                 = 126083,
    PRIEST_DEVOURING_PLAGUE_VISUAL                  = 124495,
    ...
    PRIEST_SPELL_GLYPH_OF_THE_HEAVENS               = 120581,
    PRIEST_SPELL_GLYPH_OF_THE_HEAVENS_EFFECT        = 124433,
    ...
    PRIEST_SPELL_PRAYER_OF_MENDING_AURA             = 41635,
    PRIEST_SPELL_PRAYER_OF_MENDING_VISUAL           = 41637,
    PRIEST_SPELL_ANGELIC_BULWARK                    = 108945,
    PRIEST_SPELL_ANGELIC_BULWARK_TRIGGERED          = 114214
};
...
public:
    ...
    class spell_pri_angelic_bulwark_AuraScript : public AuraScript
    {
        ...
        {
            if (!sSpellMgr->GetSpellInfo(PRIEST_SPELL_ANGELIC_BULWARK_TRIGGERED))
                return false;
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            if (DamageInfo* dmgInfo = eventInfo.GetDamageInfo())
            {
                ...
                {
                    if ((caster->GetHealth() - dmgInfo->GetDamage()) < caster->CountPctFromMaxHealth(GetSpellInfo()->Effects[EFFECT_0].CalcValue()))
                    {
                        if (!caster->ToPlayer()->HasSpellCooldown(PRIEST_SPELL_ANGELIC_BULWARK))
                            return true;
                    }
                }
            }
        ...
        void OnProc(constAuraEffectPtr /*aurEff*/, ProcEventInfo& procInfo)
        {
            ...
            Player* player = procInfo.GetActionTarget()->ToPlayer();
            int32 bp = int32(player->CountPctFromMaxHealth(GetSpellInfo()->Effects[EFFECT_1].CalcValue()));

            player->AddSpellCooldown(PRIEST_SPELL_ANGELIC_BULWARK, 0, time(nullptr) + GetSpellInfo()->Effects[EFFECT_2].CalcValue()); //Add 90 sec cooldow  
            player->CastCustomSpell(player, PRIEST_SPELL_ANGELIC_BULWARK_TRIGGERED, &bp, nullptr, nullptr, true);
        }

        void Register()
        {
            DoCheckProc += AuraCheckProcFn(spell_pri_angelic_bulwark_AuraScript::CheckProc);
           OnEffectProc += AuraEffectProcFn(spell_pri_angelic_bulwark_AuraScript::OnProc, EFFECT_0, SPELL_AURA_PROC_TRIGGER_SPELL);
        }
    };
            ...
            void OnRemove(constAuraEffectPtr aurEff, AuraEffectHandleModes /*mode*/)
            {
                if (Unit* caster = GetCaster())
                {
                    if (Unit* owner = caster->GetOwner())
                        owner->RemoveAura(119032); // Spectral Guise
            ...
            void HandleOnHit()
            {
                if (Unit* caster = GetCaster())
                {
                    if (Player* player = caster->ToPlayer())
                    {
                        if (Unit* target = GetHitUnit())
                        {
                            if (player->HasAura(PRIEST_SPELL_2P_S12_HEAL))
                                player->CastSpell(target, PRIEST_SPELL_HOLY_SPARK, true);
                        }
                    }
                }
            }
...
int32 PrayerOfMendingBonus;
/*######
## spell_pri_prayer_of_mending_proc - 41635
######*/
class spell_pri_prayer_of_mending_proc : public SpellScriptLoader
{
public:
	spell_pri_prayer_of_mending_proc() : SpellScriptLoader("spell_pri_prayer_of_mending_proc") { }

	class spell_pri_prayer_of_mending_proc_AuraScript : public AuraScript
    {
		PrepareAuraScript(spell_pri_prayer_of_mending_proc_AuraScript);

        void OnProc(constAuraEffectPtr aurEff, ProcEventInfo& procInfo)
        {
            PreventDefaultAction();

            AuraPtr PrayerOfMending = aurEff->GetBase();
            if (!PrayerOfMending)
                return;
            ...
            if (Unit* caster = GetCaster())
            {
                if (Player* player = caster->ToPlayer())
                {
                    ...
                    {
                        int32 charges = PrayerOfMending->GetCharges();
                        target->RemoveAura(PrayerOfMending);

                        PrayerOfMendingBonus = 0;

                        if (charges == 4)
                            if (AuraEffectPtr glyphEffect = player->GetAuraEffect(PRIEST_GLYPH_OF_PRAYER_OF_MENDING, EFFECT_0))
                                PrayerOfMendingBonus += glyphEffect->GetAmount();

                        player->CastSpell(target, PRIEST_PRAYER_OF_MENDING_HEAL, true);

                        JumpToNextTarget(player, target, charges, PrayerOfMending);
                    }
                }
            }
        }

        void JumpToNextTarget(Player* player, Unit* target, int32 charges, AuraPtr PrayerOfMending)
        {
            std::list<Unit*> nextTarget;
            float range = 20.f;
            uint32 filterMask = (EXCLUDE_ENEMIES | ONLY_LOS | ONLY_RAID_MEMBERS);

            target->FilterTargets(nextTarget, range, filterMask, 1, target, HealthPctOrderPred);

            for (auto itr : nextTarget)
            {
                if (PrayerOfMending->GetCharges() > 1)
                {
                    player->CastSpell(itr, PRIEST_SPELL_PRAYER_OF_MENDING_AURA, true);
                    target->CastSpell(itr, PRIEST_SPELL_PRAYER_OF_MENDING_VISUAL, true);

                    if (AuraPtr aura = itr->GetAura(PRIEST_SPELL_PRAYER_OF_MENDING_AURA, player->GetGUID()))
        ...
        void Register()
        {
			OnEffectProc += AuraEffectProcFn(spell_pri_prayer_of_mending_proc_AuraScript::OnProc, EFFECT_0, SPELL_AURA_RAID_PROC_FROM_CHARGE_WITH_VALUE);
        }
    };

    AuraScript* GetAuraScript() const
    {
		return new spell_pri_prayer_of_mending_proc_AuraScript();
    }
};

/*######
## spell_pri_prayer_of_mending_heal - 33110
######*/
...
public:
    ...
    class spell_pri_prayer_of_mending_heal_SpellScript : public SpellScript
    {
        ...
        void HandleOnHit()
        {
            ...
            AddPct(Heal, PrayerOfMendingBonus);
            SetHitHeal(Heal);
        ...
        void Register()
        {
            OnHit += SpellHitFn(spell_pri_prayer_of_mending_heal_SpellScript::HandleOnHit);
        }
    };
...
/*######
## spell_pri_prayer_of_mending_divine_insight - 123259
######*/
        ...
        void HandleOnHit()
        {
            if (Unit* caster = GetCaster())
            {
                if (Player* player = caster->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        if (player->HasAura(PRIEST_SPELL_DIVINE_INSIGHT_HOLY))
                            player->RemoveAura(PRIEST_SPELL_DIVINE_INSIGHT_HOLY);

                        float radius = 20.f;
                        int32 targetsAmount = player->HasGlyph(PRIEST_GLYPH_OF_PRAYER_OF_MENDING) ? 4 : 5;
                        std::list<Unit*> nextTarget;
                        Unit* lastTarget = NULL;
                        uint32 filterMask = (EXCLUDE_ENEMIES | ONLY_LOS | ONLY_RAID_MEMBERS);

                        for (uint8 i = 1; i <= targetsAmount; ++i)
                        {
                            if (i == 1)
                            {
                                PrayerOfMendingBonus = 0;
                            ...
                            {
                                target->FilterTargets(nextTarget, radius, filterMask, 1, lastTarget, HealthPctOrderPred);

                                for (auto itr : nextTarget)
                                {
                                    lastTarget = itr;
                                    player->CastSpell(itr, PRIEST_PRAYER_OF_MENDING_HEAL, true);
                                }
                            }
                        }
                    }
...
public:
	...
	class spell_pri_shadowy_apparition_AuraScript : public AuraScript
	{
		...
		void OnProc(constAuraEffectPtr aurEff, ProcEventInfo& procInfo)
		{
			...
			if (Player* player = procInfo.GetActor()->ToPlayer())
			{
				if (Unit* target = procInfo.GetActionTarget())
				{
					...
					player->SendPlaySpellVisual(33584, target, 6.f);
					player->CastSpell(target, 147193, true);
				}
			}
		}
		...
		void Register()
		{
			OnEffectProc += AuraEffectProcFn(spell_pri_shadowy_apparition_AuraScript::OnProc, EFFECT_0, SPELL_AURA_DUMMY);
		}
	};
        ...
        class spell_pri_surge_of_light_SpellScript : public SpellScript
        {
            ...
            void HandleOnCast()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    {
                        _player->RemoveAura(PRIEST_SURGE_OF_LIGHT_VISUAL);
...
class spell_pri_from_darkness_comes_light : public SpellScriptLoader
{
    public:
        ...
        class spell_pri_from_darkness_comes_light_AuraScript : public AuraScript
        {
            ...
            bool CheckProc(ProcEventInfo& eventInfo)
            {
                SpellInfo const* triggeredBySpell = eventInfo.GetDamageInfo()->GetSpellInfo();

                //This proc don´t need to be executed by this spells
                switch (triggeredBySpell->Id)
                {
                    case eSpells::SHADOW_WORD_PAIN:
                    case eSpells::HOLY_FIRE:
                    case eSpells::DIVINE_STAR:
                    case eSpells::SHADOW_WORD_DEATH:
                    case eSpells::HOLY_WORD_CHASTISE:
                        return false;
                        break;
                    default:
                        break;
            ...
            void OnProc(constAuraEffectPtr aurEff, ProcEventInfo& procInfo)
            {
                ...
                if (Player* player = procInfo.GetActor()->ToPlayer())
                {
                    ...
                    if (Unit* target = procInfo.GetActionTarget())
                    {
                        //15% - BasePoints of EFFECT_0
                        if (roll_chance_i(GetSpellInfo()->Effects[EFFECT_0].BasePoints))
                        {
                            player->CastSpell(player, PRIEST_SURGE_OF_LIGHT, true);

                            if (AuraPtr surgeOfLight = player->GetAura(PRIEST_SURGE_OF_LIGHT))
                            {
                                int32 stacks = surgeOfLight->GetStackAmount();

                                if (stacks == 2)
                                {
                                    //If proc again, update duration ( Instead of do nothing :P )
                                    surgeOfLight->SetDuration(surgeOfLight->GetMaxDuration());
                                    player->CastSpell(player, PRIEST_SURGE_OF_LIGHT_VISUAL, true);
                                }
                            }
                        }
                    }
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        if (GetSpellInfo()->Id == PRIEST_LEAP_OF_FAITH)
                            _player->AddAura(PRIEST_LEAP_OF_FAITH_VISUAL, target);
                    }
                }
            }
            ...
            if (Player* const _player = GetCaster()->ToPlayer())
            {
                if (Unit* const target = GetHitUnit())
                {
                    if (_player->HasAura(PRIEST_SPIRIT_SHELL_AURA))
                    {
                        ...
                        if (constAuraEffectPtr const shell = target->GetAuraEffect(PRIEST_SPIRIT_SHELL_ABSORPTION, EFFECT_0, _player->GetGUID()))
                        ...
                        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                        {
                            ...
                            uint32 dispelMask = GetSpellInfo()->GetDispelMask(DispelType(dispel_type));
                             ...
                             if (GetSpellInfo()->Id == 527)
                                 dispelMask = ((1 << DISPEL_MAGIC) | (1 << DISPEL_DISEASE));
        ...
        class spell_pri_devouring_plague_SpellScript : public SpellScript
        {
            ...
            void HandleOnHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    {
                        if (_player->HasAura(PRIEST_DEVOURING_PLAGUE_VISUAL))
                            _player->RemoveAura(PRIEST_DEVOURING_PLAGUE_VISUAL);
                    ...
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        else if (AuraPtr surgeOfDarkness = _player->GetAura(PRIEST_SURGE_OF_DARKNESS))
                        {
                            ...
                            else
                            {
                                ...
                                _player->RemoveAura(PRIEST_SURGE_OF_DARKNESS_VISUAL);
                            }
                        }
                    }
                }
        ...
        class spell_pri_halo_damage_SpellScript : public SpellScript
        {
            ...
            void HandleOnCast()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    ...
                    if (Creature* creature = _player->SummonCreature(WORLD_TRIGGER, x, y, z, 0.0f, TEMPSUMMON_TIMED_DESPAWN, 1000))
                    {
                        if (GetSpellInfo()->Id == 120517)
                            creature->SendPlaySpellVisual(24816, creature, 0.4f);
                        else
                            creature->SendPlaySpellVisual(24818, creature, 0.4f);
                    }
                }
            }
            ...
            void Register()
            {
                OnCast += SpellCastFn(spell_pri_halo_damage_SpellScript::HandleOnCast);
            ...
            void HandleOnHit()
            {
                if (Unit* caster = GetCaster())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        {
                            if (caster->HasAura(PRIEST_DEVOURING_PLAGUE_VISUAL))
                                caster->RemoveAura(PRIEST_DEVOURING_PLAGUE_VISUAL);
                ...
                if (GetCaster())
                {
                    ...
                    if (GetCaster()->HasAura(PRIEST_FROM_DARKNESS_COMES_LIGHT_AURA))
                        if (roll_chance_i(20))
                        {
                            ...
                            if (AuraPtr surgeOfDarkness = GetCaster()->GetAura(PRIEST_SURGE_OF_DARKNESS))
                            {
                                int32 stacks = surgeOfDarkness->GetStackAmount();

                                if (stacks == 2)
                                    GetCaster()->CastSpell(GetCaster(), PRIEST_SURGE_OF_DARKNESS_VISUAL, true);
                            }
                        }
                }
            }
...
public:
    ...
    class spell_pri_glyph_of_levitate_AuraScript : public AuraScript
    {
        ...
        void OnApply(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                if (caster->HasAura(PRIEST_SPELL_LEVITATE))
                    caster->RemoveAura(PRIEST_SPELL_LEVITATE);
            }
        }
...
public:
    ...
    class spell_pri_path_of_devout_AuraScript : public AuraScript
    {
        ...
        void OnApply(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (target->HasAura(PRIEST_SPELL_LEVITATE))
            {
                if (caster->HasAura(PRIEST_SPELL_GLYPH_OF_LEVITATE))
                    caster->CastSpell(target, PRIEST_SPELL_PATH_OF_DEVOUT, true);
            }
        }
        ...
        void OnRemove(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            {
                if (AuraPtr pathOfDevout = target->GetAura(PRIEST_SPELL_PATH_OF_DEVOUT))
                {
                    pathOfDevout->SetMaxDuration(10 * IN_MILLISECONDS);
                    pathOfDevout->SetDuration(pathOfDevout->GetMaxDuration());
                }
            }
        }
...
public:
    ...
    class spell_pri_glyph_of_the_heavens_AuraScript : public AuraScript
    {
        ...
        void OnApply(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (target->HasAura(PRIEST_SPELL_LEVITATE))
            {
                if (caster->HasAura(PRIEST_SPELL_GLYPH_OF_THE_HEAVENS))
                {
                    target->CastSpell(target, PRIEST_SPELL_GLYPH_OF_THE_HEAVENS_EFFECT, true);
                }
            }
        }
        ...
        void OnRemove(constAuraEffectPtr /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (target->HasAura(PRIEST_SPELL_GLYPH_OF_THE_HEAVENS_EFFECT))
            {
                target->RemoveAura(PRIEST_SPELL_GLYPH_OF_THE_HEAVENS_EFFECT);
            }
        }

        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_pri_glyph_of_the_heavens_AuraScript::OnApply, EFFECT_0, SPELL_AURA_ANY, AURA_EFFECT_HANDLE_REAL);
            AfterEffectRemove += AuraEffectRemoveFn(spell_pri_glyph_of_the_heavens_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_ANY, AURA_EFFECT_HANDLE_REAL);
        }
    };
...
public:
    ...
    class spell_pri_holy_nova_heal_SpellScript : public SpellScript
    {
        ...
        void CorrectTargets(std::list<WorldObject*>& targets)
        {
            ...
            WoWSource::RandomResizeList(targets, sSpellMgr->GetSpellInfo(PRIEST_SPELL_HOLY_NOVA)->Effects[EFFECT_1].BasePoints);
        }
    ...
    public:
        ...
        class spell_pri_mind_blast_SpellScript : public SpellScript
        {
            ...
            void HandleAfterHit()
            {
                if (Player* _player = GetCaster()->ToPlayer())
                {
                    if (_player->GetPower(POWER_SHADOW_ORBS) == 3)
                    {
                        _player->AddAura(PRIEST_DEVOURING_PLAGUE_VISUAL, _player);
                    }
                }
            }
...
/*######
## spell_pri_divine_star (holy) - 110744
## spell_pri_divine_star (shadow) - 122121
######*/
    ...
    class spell_pri_divine_star_SpellScript : public SpellScript
    {
        ...
        void HandleOnCast()
        {
            if (Unit* caster = GetCaster())
            {
                if (Player* player = caster->ToPlayer())
                {
                    float range = GetSpellInfo()->Effects[0].CalcRadius(player) * GetSpellValue()->RadiusMod;

                    uint32 spellId = PRIEST_SPELL_DIVINE_STAR_HOLY;
                    uint32 visualSpellId = PRIEST_SPELL_DIVINE_STAR_VISUAL_HOLY;

                    if (player->HasAura(PRIEST_SHADOWFORM_STANCE))
                    {
                        spellId = PRIEST_SPELL_DIVINE_STAR_SHADOW;
                        visualSpellId = PRIEST_SPELL_DIVINE_STAR_VISUAL_SHADOW;
                    }
                    float x, y;

                    player->GetNearPoint2D(x, y, range, player->GetOrientation());

                    if (Creature* endPoint = player->SummonCreature(WORLD_TRIGGER, x - 5 * cos(player->GetOrientation()),
                        y - 5 * sin(player->GetOrientation()), player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 2000))
                    {
                        if (Creature* firstPoint = player->SummonCreature(WORLD_TRIGGER, player->GetPositionX(), 
                            player->GetPositionY(), player->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN, 2000))
                        {
                            endPoint->SetFacingToObject(firstPoint);
                            firstPoint->SetFacingToObject(endPoint);

                            firstPoint->CastSpell(endPoint, visualSpellId, true, NULL, NULLAURA_EFFECT, player->GetGUID());

                            endPoint->CastWithDelay(1000, firstPoint, spellId, true, false, NULL, NULLAURA_EFFECT, player->GetGUID());
                            endPoint->CastWithDelay(1000, firstPoint, visualSpellId, true, false, NULL, NULLAURA_EFFECT, player->GetGUID());
                        }
                    }
                }
            }
            ...
            if (Unit* caster = GetCaster())
            {
                if (Unit* originalCaster = GetOriginalCaster())
                {
                    if (Unit* target = GetHitUnit())
                    {
                        ...
                        if (originalCaster->IsValidAssistTarget(target))
                            spellId = PRIEST_SPELL_DIVINE_STAR_HEAL;
                        else if (originalCaster->IsValidAttackTarget(target))
                            spellId = PRIEST_SPELL_DIVINE_STAR_DAMAGE;

                        if (spellId != NULL)
                            caster->CastSpell(target, spellId, true, NULL, NULLAURA_EFFECT, originalCaster->GetGUID());
                    }
                }
            }
        }
        ...
        void Register()
        {
            ...
            OnHit += SpellHitFn(spell_pri_divine_star_SpellScript::HandleOnHit);
        }
    };
...
class spell_pri_4p_holy_spark : public SpellScriptLoader
{
    public:
        ...
        class spell_pri_4p_holy_spark_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_pri_4p_holy_spark_SpellScript);
            ...
            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_pri_4p_holy_spark_SpellScript::HandleSpark, EFFECT_1, SPELL_EFFECT_TRIGGER_SPELL);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_pri_4p_holy_spark_SpellScript();
        }
};
...
class spell_pri_mass_dispel : public SpellScriptLoader
{
    class spell_impl : public SpellScript
    {
        ...
        enum Spells
        {
            GLYPH_OF_MASS_DISPELL = 55691,
            MASS_DISPELL_GLYPHED  = 39897,
        };
        ...
        void HandleOnCast()
        {
            ...
            {
                if (player->HasAura(GLYPH_OF_MASS_DISPELL))
                {
                    const WorldLocation* loc = GetExplTargetDest();

                    player->CastSpell(loc->GetPositionX(), loc->GetPositionY(), loc->GetPositionZ(), MASS_DISPELL_GLYPHED, true);
                }
            }
        }
...
//Fear Ward
class spell_pri_fear_ward : public SpellScriptLoader
{
public:
	spell_pri_fear_ward() : SpellScriptLoader("spell_pri_fear_ward") { }

	class spell_pri_fear_ward_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_pri_fear_ward_SpellScript);
		...
		void Register()
		{
			OnCheckCast += SpellCheckCastFn(spell_pri_fear_ward_SpellScript::CheckTarget);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_pri_fear_ward_SpellScript();
	}
};
...
void AddSC_priest_spell_scripts()
{
	...
	new spell_pri_prayer_of_mending_proc();
