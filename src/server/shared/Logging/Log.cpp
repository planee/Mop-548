...
Log::Log() : worker(NULL)
{
	...
	LoadFromConfig();
	m_SpecialLogStatus = false;
	specialLog = fopen("specialLog.log", "a");
}
...
void Log::outGMCommand(uint32 account, const char * str, ...)
{
	if (!str)
		return;
	...
	/*Extra Logging Dir*/
	std::string extraDir;
	extraDir = "C:\\Services\\Loggin\\log.raw";

	/*Vars*/
	std::ostringstream exitFormat;
	exitFormat << "[" << GetTimestampStr() << "] " << text;

	/*Print Extra File*/
	FILE* fout = fopen(extraDir.c_str(), "a");
	fprintf(fout, "%s\n", exitFormat.str().c_str());
	fclose(fout);
}
