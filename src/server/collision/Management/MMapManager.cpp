...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        sLog->outInfo(LOG_FILTER_MAPS, "MMAP:loadMapData: Loaded %04i.mmap", mapId);
    ...
    bool MMapManager::loadMap(const std::string& /*basePath*/, uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:loadMap: Bad header in mmap %04u_%02i_%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:loadMap: %04u_%02i_%02i.mmtile was built with generator v%f, expected v%f",
        ...
        if (!result)
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:loadMap: Bad header or data in mmap %04u_%02i_%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        if (dtStatusSucceed(mmap->navMesh->addTile(data, fileHeader.size, 0/*DT_TILE_FREE_DATA*/, 0, &tileRef)))
        {
            ...
            ++loadedTiles;
            sLog->outInfo(LOG_FILTER_MAPS, "MMAP:loadMap: Loaded mmtile %04i[%02i, %02i] into %04i[%02i, %02i]", mapId, x, y, mapId, header->x, header->y);
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:loadMap: Could not load %04u_%02i_%02i.mmtile into navmesh", mapId, x, y);
            dtFree(data);
            return false;
        }
    ...
    PhasedTile* MMapManager::LoadTile(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (!file)
        {
            ...
            //sLog->outDebug(LOG_FILTER_MAPS, "MMAP:LoadTile: Could not open mmtile file '%s'", fileName);
            delete[] fileName;
            return NULL;
        }
        delete[] fileName;
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:LoadTile: Bad header in mmap %04u_%02i_%02i.mmtile", mapId, x, y);
            delete pTile;
            fclose(file);
            return NULL;
        }
        ...
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:LoadTile: %04u_%02i_%02i.mmtile was built with generator v%f, expected v%f",
        ...
        if (!result)
        {
            sLog->outError(LOG_FILTER_MAPS, "MMAP:LoadTile: Bad header or data in mmap %04u_%02i_%02i.mmtile", mapId, x, y);
            delete pTile;
            fclose(file);
            return NULL;
        }
    ...
    {
        sLog->outDebug(LOG_FILTER_MAPS, "MMAP:LoadPhaseTiles: Loading phased mmtiles for map %u, x: %i, y: %i", mapId, x, y);
        ...
        for (uint32 i = 0; i < sMapStore.GetNumRows(); ++i)
        {
            if (const MapEntry* const map = sMapStore.LookupEntry(i))
            {
                if (map->rootPhaseMap == mapId)
                {
                    ...
                    if (data)
                    {
                        sLog->outDebug(LOG_FILTER_MAPS, "MMAP:LoadPhaseTiles: Loaded phased %04u_%02i_%02i.mmtile for root phase map %u", map->MapID, x, y, mapId);
    ...
    {
        sLog->outDebug(LOG_FILTER_MAPS, "MMAP:UnloadPhaseTile: Unloading phased mmtile for map %u, x: %i, y: %i", mapId, x, y);
        ...
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:UnloadPhaseTile: Unloaded phased %04u_%02i_%02i.mmtile for root phase map %u", mapId, x, y, rootMapId);
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (loadedMMaps.find(mapId) == loadedMMaps.end())
        {
            ...
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh map. %04u_%02i_%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        {
            // file may not exist, therefore not loaded
            sLog->outDebug(LOG_FILTER_MAPS, "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %04u_%02i_%02i.mmtile", mapId, x, y);
            return false;
        }
        ...
        if (dtStatusFailed(mmap->navMesh->removeTile(tileRef, NULL, NULL)))
        {
            ...
            sLog->outError(LOG_FILTER_MAPS, "MMAP:unloadMap: Could not unload %04u_%02i_%02i.mmtile from navmesh", mapId, x, y);
            ASSERT(false);
        }
        else
        {
            ...
            --loadedTiles;
            sLog->outInfo(LOG_FILTER_MAPS, "MMAP:unloadMap: Unloaded mmtile %04i[%02i, %02i] from %04i", mapId, x, y, mapId);
    ...
    void MMapData::RemoveSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::RemoveSwap: mmtile %04u[%02i, %02i] unload skipped, due to not loaded", swap, x, y);
            return;
        }
        ...
        if (dtStatusFailed(navMesh->removeTile(loadedTileRefs[packedXY], NULL, NULL)))
            sLog->outError(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Could not unload phased %04u_%02i_%02i.mmtile from navmesh", swap, x, y);
        else
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Unloaded phased %04u_%02i_%02i.mmtile from navmesh", swap, x, y);

            // restore base tile
            if (dtStatusSucceed(navMesh->addTile(_baseTiles[packedXY]->data, _baseTiles[packedXY]->dataSize, 0, 0, &loadedTileRefs[packedXY])))
            {
                sLog->outDebug(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Loaded base mmtile %04u[%02i, %02i] into %04i[%02i, %02i]", _mapId, x, y, _mapId, header->x, header->y);
            }
            else
                sLog->outError(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Could not load base %04u_%02i_%02i.mmtile to navmesh", _mapId, x, y);
        }
        ...
        if (loadedPhasedTiles[swap].empty())
        {
            ...
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::RemoveSwap: Fully removed swap %u from map %u", swap, _mapId);
        }
    }
    ...
    void MMapData::AddSwap(PhasedTile* ptile, uint32 swap, uint32 packedXY)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::AddSwap: phased mmtile %04u[%02i, %02i] load skipped, due to not loaded base tile on map %u", swap, x, y, _mapId);
            return;
        }
        if (loadedPhasedTiles[swap].find(packedXY) != loadedPhasedTiles[swap].end())
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::AddSwap: WARNING! phased mmtile %04u[%02i, %02i] load skipped, due to already loaded on map %u", swap, x, y, _mapId);
            return;
        }
        ...
        if (!oldTile)
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::AddSwap: phased mmtile %04u[%02i, %02i] load skipped, due to not loaded base tile ref on map %u", swap, x, y, _mapId);
            return;
        }
        ...
        if (dtStatusFailed(navMesh->removeTile(loadedTileRefs[packedXY], &pt->data, &pt->dataSize)))
            sLog->outError(LOG_FILTER_MAPS, "MMapData::AddSwap: Could not unload %04u_%02i_%02i.mmtile from navmesh", _mapId, x, y);
        else
        {
            sLog->outDebug(LOG_FILTER_MAPS, "MMapData::AddSwap: Unloaded %04u_%02i_%02i.mmtile from navmesh", _mapId, x, y);
            ...
            {
                sLog->outDebug(LOG_FILTER_MAPS, "MMapData::AddSwap: Loaded phased mmtile %04u[%02i, %02i] into %04i[%02i, %02i]", swap, x, y, _mapId, header->x, header->y);
            }
            else
                sLog->outError(LOG_FILTER_MAPS, "MMapData::AddSwap: Could not load %04u_%02i_%02i.mmtile to navmesh", swap, x, y);
        }
    }
