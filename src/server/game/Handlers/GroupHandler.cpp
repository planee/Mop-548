...
void WorldSession::SendPartyResult(PartyOperation operation, const std::string& member, PartyResult res, uint32 val /* = 0 */)
{
    ...
    data << uint64(0);                // player who caused the error (in some cases).
...
void WorldSession::HandleGroupUninviteGuidOpcode(WorldPacket& recvData)
{
    ...
    if (Player* player = grp->GetInvitedByGuid(guid))
    {
...
void WorldSession::HandleGroupUninviteOpcode(WorldPacket& recvData)
{
    ...
    if (Player* player = grp->GetInvitedByName(membername))
    {
        player->UninviteFromGroup();
        return;
    }
...
void WorldSession::HandleGroupRaidConvertOpcode(WorldPacket& recvData)
{
    ...
    bool unk = recvData.ReadBit(); // ToRaid bool - signals the change needed.
...
void WorldSession::HandleGroupSwapSubGroupOpcode(WorldPacket& recvData)
{
    ...
    recvData >> targetGroup;

    // The move should be made here!
}
