...
void WorldSession::HandleBattlegroundPortOpcode(WorldPacket& recvData)
{
    ...
    if (!bracketEntry)
    {
		sLog->outError(LOG_FILTER_NETWORKIO, "BattlegroundHandler: No bracket entry for Port for mapId: (%u) at Level (%u)!", bg->GetMapId(), _player->getLevel());
        return;
    }
...
void WorldSession::HandleRequestRatedBgStats(WorldPacket& recvData)
{
    ...
    WorldPacket data(Opcodes::SMSG_BATTLEFIELD_RATED_INFO, (8 * 4) * ArenaSlots::MAX_PVP_SLOT);

    for (uint8 l_I = 0; l_I < ArenaSlots::MAX_PVP_SLOT; l_I++)
    {
        data << uint32(_player->GetBestRatingOfSeason(l_I));  ///< BestSeasonRating
        data << uint32(_player->GetWeekGames(l_I));           ///< WeeklyPlayed
        data << uint32(_player->GetBestRatingOfWeek(l_I));    ///< BestWeeklyRating
        data << uint32(_player->GetSeasonGames(l_I));         ///< SeasonPlayed
        data << uint32(_player->GetArenaPersonalRating(l_I)); ///< PersonalRating

        /// SeasonWon or Ranking
        data << uint32(0);

        data << uint32(_player->GetWeekWins(l_I));            ///< WeeklyWon
