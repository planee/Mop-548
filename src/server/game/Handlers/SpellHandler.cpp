...
void WorldSession::HandleGameobjectReportUse(WorldPacket& recvPacket)
{
    ...
    if (!go->IsWithinDistInMap(_player, 2*INTERACTION_DISTANCE))
        return;
...
void WorldSession::HandleCastSpellOpcode(WorldPacket& recvPacket)
{
	...
	else if (spellInfo->Id == 15407 && _player->HasSpell(139139))
	{
    ...
    else if (spellInfo->Id == 116694 && _player->GetCurrentSpell(CURRENT_CHANNELED_SPELL) && _player->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->GetSpellInfo()->Id == 115175)
    {
		if (targets.GetUnitTarget())
		{
			if (_player->IsWithinLOS(targets.GetUnitTarget()->GetPositionX(), targets.GetUnitTarget()->GetPositionY(), targets.GetUnitTarget()->GetPositionZ()))
			{
				recvPacket.rfinish();
    ...
    // Surging Mist (Glyphed) - 123273 and Surging Mist - 116995
    // Surging Mist (Glyphed) is instantly casted if player is channeling Soothing Mist
    else if (spellInfo->Id == 123273 && _player->GetCurrentSpell(CURRENT_CHANNELED_SPELL) && _player->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->GetSpellInfo()->Id == 115175)
    {
        ...
        if (Target)
        {
            if (_player->IsWithinLOS(Target->GetPositionX(), Target->GetPositionY(), Target->GetPositionZ()))
            {
                recvPacket.rfinish();
