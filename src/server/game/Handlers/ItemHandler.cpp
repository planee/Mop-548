...
void WorldSession::HandleSplitItemOpcode(WorldPacket& recvData)
{
    ...
    recvData >> count >> srcslot >> dstslot >> dstbag >> srcbag;
...
void WorldSession::HandleSellItemOpcode(WorldPacket& recvData)
{
    ...
    if (pItem)
    {
		...
		if (_player->GetUInt32Value(ITEM_FIELD_EXPIRATION))
		{
			_player->SendSellError(SELL_ERR_ONLY_EMPTY_BAG, creature, pItem->GetGUID());
			return;
		}
...
void WorldSession::HandleBuybackItem(WorldPacket& recvData)
{
    ...
    uint8 bitsOrder[8] = { 3, 5, 0, 7, 2, 6, 1, 4 };
    recvData.ReadBitInOrder(vendorguid, bitsOrder);

    recvData.FlushBits();

    uint8 bytesOrder[8] = { 1, 7, 6, 0, 5, 3, 4, 2 };
...
void WorldSession::HandleListInventoryOpcode(WorldPacket& recvData)
{
    ...
    uint8 bitsOrder[8] = { 1, 0, 6, 3, 5, 4, 7, 2 };
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket& recvPacket)
{
    ...
    uint8 bitsOrder[8] = { 7, 2, 3, 1, 5, 4, 0, 6 };
    recvPacket.ReadBitInOrder(guid, bitsOrder);

    recvPacket.FlushBits();

    uint8 bytesOrder[8] = { 7, 0, 3, 1, 6, 2, 4, 5 };
...
void WorldSession::HandleSocketOpcode(WorldPacket& recvData)
{
    ...
    for (int i = 0; i < MAX_GEM_SOCKETS; ++i)
    {
        ...
        if (GemProps[i]->requiredILvl > itemProto->ItemLevel)
            return;
        ...
        if (!itemProto->Socket[i].Color)
        {
            ...
            // Not first not-colored (prismatic) socket
            if (i != firstPrismatic)
                return;
        }
    ...
    data.WriteByteSeq(item_guid[0]);
    data.WriteByteSeq(item_guid[5]);
    data.WriteByteSeq(item_guid[7]);
    data.WriteByteSeq(item_guid[1]);
...
void WorldSession::HandleItemRefundInfoRequest(WorldPacket& recvData)
{
    ...
    uint8 bitsOrder[8] = { 0, 4, 6, 3, 2, 1, 7, 5 };
    recvData.ReadBitInOrder(guid, bitsOrder);

    recvData.FlushBits();

    uint8 bytesOrder[8] = { 5, 3, 7, 2, 1, 6, 0, 4 };
    recvData.ReadBytesSeq(guid, bytesOrder);
...
void WorldSession::HandleItemRefund(WorldPacket& recvData)
{
    ...
    uint8 bitsOrder[8] = { 3, 2, 1, 4, 7, 6, 0, 5 };
    recvData.ReadBitInOrder(itemGuid, bitsOrder);

    recvData.FlushBits();

    uint8 bytesOrder[8] = { 3, 1, 5, 4, 2, 7, 0, 6 };
	...
	if (_player->GetLootGUID() == itemGuid)
		return;
...
void WorldSession::HandleTransmogrifyItems(WorldPacket& recvData)
{
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        if (hasItemGuid1[i])
        {
            uint8 bitsOrder[8] = { 5, 6, 4, 0, 7, 3, 1, 2 };
            recvData.ReadBitInOrder(originalItemGuid[i], bitsOrder);
        }

        if (hasItemGuid2[i])
        {
            uint8 bitsOrder[8] = { 3, 6, 4, 0, 1, 7, 5, 2 };
            recvData.ReadBitInOrder(targetItemGuid[i], bitsOrder);
        }
    }
    ...
    uint8 bytesNpcOrder[8] = { 5, 4, 1, 0, 2, 7, 6, 3 };
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        if (hasItemGuid2[i])
        {
            uint8 bytesOrder[8] = { 4, 0, 5, 6, 2, 7, 1, 3 };
            recvData.ReadBytesSeq(targetItemGuid[i], bytesOrder);
        }

        if (hasItemGuid1[i])
        {
            uint8 bytesOrder[8] = { 3, 6, 2, 7, 4, 5, 0, 1 };
            recvData.ReadBytesSeq(originalItemGuid[i], bytesOrder);
        }
    }
    ...
    for (uint8 i = 0; i < count; ++i)
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify an item (lowguid: %u) with a wrong slot (%u) when transmogrifying items.", player->GetGUIDLow(), player->GetName(), GUID_LOPART(originalItemGuid[i]), slots[i]);
            return;
        }
        ...
        if (originalItemGuid[i])
        {
            itemTransmogrifier = player->GetItemByGuid(originalItemGuid[i]);
            if (!itemTransmogrifier)
            {
                sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - Player (GUID: %u, name: %s) tried to transmogrify with an invalid item (lowguid: %u).", player->GetGUIDLow(), player->GetName(), GUID_LOPART(originalItemGuid[i]));
                return;
            }
        }
        ...
        else
        {
            if (!itemTransmogrifier)
            {
                sLog->outDebug(LOG_FILTER_NETWORKIO, "WORLD: HandleTransmogrifyItems - No item to transmogrify to!");
                return;
            }
...
void WorldSession::HandleReforgeItemOpcode(WorldPacket& recvData)
{
    ...
    uint8 bitOrder[8] = { 3, 5, 4, 6, 1, 0, 7, 2 };
    recvData.ReadBitInOrder(guid, bitOrder);

    recvData.FlushBits();

    uint8 byteOrder[8] = { 2, 0, 6, 4, 3, 5, 1, 7 };
...
void WorldSession::HandleUpgradeItemOpcode(WorldPacket& recvData)
{
    ...
    uint32 item_bag, item_slot, upgradeEntry;
    ObjectGuid npcGuid;
    ObjectGuid itemGuid;
    ...
    if (!upgradeEntry)
    {
        // Upgraded items cannot be reset.
        SendItemUpgradeResult(false);
        return;
    }
...
void WorldSession::HandleSetLootSpecialization(WorldPacket& recvData)
{
    ...
    GetPlayer()->SetUInt32Value(PLAYER_FIELD_LOOT_SPEC_ID, specializationId);
}
