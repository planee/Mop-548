...
bool Vehicle::CheckCustomCanEnter()
{
    switch (GetCreatureEntry())
    {
		...
		case 41620: // Pinza de Magmaw
		case 41789: // Pinza de Magmaw
        case 46185: // Sanitron
            return true;
    }
...
bool Vehicle::AddPassenger(Unit* unit, int8 seatId)
{
    ...
    if (seatId < 0) // no specific seat requirement
    {
        for (seat = Seats.begin(); seat != Seats.end(); ++seat)
			...
			{
				sLog->outDebug(LOG_FILTER_VEHICLES, "No hay sitio disponible: %u, Entry: %u",
					seatId, unit->GetEntry());
				return false;
			}            
    }
    else
    {
		...
		if (seat == Seats.end())
		{
			sLog->outDebug(LOG_FILTER_VEHICLES, "El id del vehiculo no es valido SeatId: %u, Entry: %u",
				seatId, unit->GetEntry());	
            return false;
		}
	...
	//if (GetBase()->GetTypeId() == TYPEID_UNIT)
	//	sScriptMgr->BeforeAddPassenger(this, unit);
    ...
    switch (veSeat->m_ID)
    {
        case 10882:
            ...
            break;
        // Galleon
        case 11227: // 0 galleon
            unit->m_movementInfo.t_pos.m_positionX = 0.4266f;
            unit->m_movementInfo.t_pos.m_positionY = -10.8f;
            unit->m_movementInfo.t_pos.m_positionZ = 34.4763f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;          
        case 11228: // 1 galleon 
            unit->m_movementInfo.t_pos.m_positionX = 0.4266f;
            unit->m_movementInfo.t_pos.m_positionY = 10.8f;
            unit->m_movementInfo.t_pos.m_positionZ = 34.4763f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;             
        case 11229: // 2 galleon
            unit->m_movementInfo.t_pos.m_positionX = 21.2562f;
            unit->m_movementInfo.t_pos.m_positionY = -0.007199988f;
            unit->m_movementInfo.t_pos.m_positionZ = 15.3918f;
            unit->m_movementInfo.t_pos.SetOrientation(0);
            break;            
        case 11233: // 4 galleon 
            unit->m_movementInfo.t_pos.m_positionX = -8.748f;
            unit->m_movementInfo.t_pos.m_positionY = 0.4662f;
            unit->m_movementInfo.t_pos.m_positionZ = 36.0963f;
            unit->m_movementInfo.t_pos.SetOrientation(0);
            break;              
        case 11234: // 5 galleon 
            unit->m_movementInfo.t_pos.m_positionX = -32.4513f;
            unit->m_movementInfo.t_pos.m_positionY = 0.4752f;
            unit->m_movementInfo.t_pos.m_positionZ = 28.7037f;
            unit->m_movementInfo.t_pos.SetOrientation(0);
            break;              
                // Galleon Skirmisher .  
        case 11230: // 3 galleon 
            unit->m_movementInfo.t_pos.m_positionX = 12.0654f;
            unit->m_movementInfo.t_pos.m_positionY = -3.3f;
            unit->m_movementInfo.t_pos.m_positionZ = 19.9944f;
            unit->m_movementInfo.t_pos.SetOrientation(5.497787f);
            break;            
        case 11235: // 6 galleon 
            unit->m_movementInfo.t_pos.m_positionX = 11.1654f;
            unit->m_movementInfo.t_pos.m_positionY = 2.97f;
            unit->m_movementInfo.t_pos.m_positionZ = 20.1744f;
            unit->m_movementInfo.t_pos.SetOrientation(0.7853982f);
            break;       
        case 11237: // 0 mid back
            unit->m_movementInfo.t_pos.m_positionX = -9.0f;
            unit->m_movementInfo.t_pos.m_positionY = -4.0f;
            unit->m_movementInfo.t_pos.m_positionZ = -9.7f;
            unit->m_movementInfo.t_pos.SetOrientation(4.712389f);
            break;             
        case 11238: // 1 mid back
            unit->m_movementInfo.t_pos.m_positionX = -9.0f;
            unit->m_movementInfo.t_pos.m_positionY = 3.0f;
            unit->m_movementInfo.t_pos.m_positionZ = -9.7f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;              
        case 11239: // 0 far back
            unit->m_movementInfo.t_pos.m_positionX = -2.0f;
            unit->m_movementInfo.t_pos.m_positionY = -4.0f;
            unit->m_movementInfo.t_pos.m_positionZ = -5.8f;
            unit->m_movementInfo.t_pos.SetOrientation(4.712389f);
            break;               
        case 11240: // 1 far back
            unit->m_movementInfo.t_pos.m_positionX = -2.0f;
            unit->m_movementInfo.t_pos.m_positionY = 3.0f;
            unit->m_movementInfo.t_pos.m_positionZ = -6.0f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;               
        case 11241: // 2 far back
            unit->m_movementInfo.t_pos.m_positionX = 5.5f;
            unit->m_movementInfo.t_pos.m_positionY = -4.0f;
            unit->m_movementInfo.t_pos.m_positionZ = -3.30f;
            unit->m_movementInfo.t_pos.SetOrientation(4.712389f);
            break;               
        case 11242: // 3 far back
            unit->m_movementInfo.t_pos.m_positionX = 5.5f;
            unit->m_movementInfo.t_pos.m_positionY = 3.2f;
            unit->m_movementInfo.t_pos.m_positionZ = -3.3f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;               
                // Galleon Warmonger  .  
        case 11243: // 2 mid back
            unit->m_movementInfo.t_pos.m_positionX = -5.0f;
            unit->m_movementInfo.t_pos.m_positionY = -3.0f;
            unit->m_movementInfo.t_pos.m_positionZ = 13.0f;
            unit->m_movementInfo.t_pos.SetOrientation(4.712389f);
            break;               
        case 11244: // 3 mid back
            unit->m_movementInfo.t_pos.m_positionX = -5.0f;
            unit->m_movementInfo.t_pos.m_positionY = 2.0f;
            unit->m_movementInfo.t_pos.m_positionZ = 12.8f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;               
        case 11245: // 4 mid back
            unit->m_movementInfo.t_pos.m_positionX = -13.0f;
            unit->m_movementInfo.t_pos.m_positionY = -3.0f;
            unit->m_movementInfo.t_pos.m_positionZ = 9.4f;
            unit->m_movementInfo.t_pos.SetOrientation(4.712389f);
            break;               
        case 11246: // 5 mid back
            unit->m_movementInfo.t_pos.m_positionX = -13.0f;
            unit->m_movementInfo.t_pos.m_positionY = 1.5f;
            unit->m_movementInfo.t_pos.m_positionZ = 9.5f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;               
        case 11247: // 4 far back
            unit->m_movementInfo.t_pos.m_positionX = 2.0f;
            unit->m_movementInfo.t_pos.m_positionY = -6.0f;
            unit->m_movementInfo.t_pos.m_positionZ = 15.4f;
            unit->m_movementInfo.t_pos.SetOrientation(4.712389f);
            break;              
        case 11248: // 5 far back
            unit->m_movementInfo.t_pos.m_positionX = 2.0f;
            unit->m_movementInfo.t_pos.m_positionY = 4.0f;
            unit->m_movementInfo.t_pos.m_positionZ = 15.8f;
            unit->m_movementInfo.t_pos.SetOrientation(1.570796f);
            break;
        default:
            break;
    ...
    if (_me->IsInWorld())
    {
        ...
        init.SetFacing(unit->m_movementInfo.t_pos.GetOrientation());
...
void Vehicle::RemovePassenger(Unit* unit)
{
    ...
    if (_me->IsInWorld())
    {
        if (!_me->GetTransport())
            ...
            unit->m_movementInfo.t_pos = _me->m_movementInfo.t_pos;
    }
