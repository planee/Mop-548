...
bool Pet::LoadPetFromDB(Player* owner, uint32 petentry, uint32 petnumber, bool current, PetSlot slotID, bool stampeded, PetLoginQueryHolder* holder, bool login)
{
    ...
    switch (getPetType())
    {
        case SUMMON_PET:
			...
			{
				if (owner && owner->ToPlayer() && owner->HasAura(56246))
				{
...
void Pet::SavePetToDB(PetSlot mode, bool stampeded)
{
    ...
    if (mode >= PET_SLOT_HUNTER_FIRST)
    {
		...
		uint32 petNumber = m_charmInfo->GetPetNumber();
		// Create Action Bar Entry Data
		std::ostringstream sacbar;
		for (uint32 i = ACTION_BAR_INDEX_START; i < ACTION_BAR_INDEX_END; ++i)
		{
			sacbar << uint32(m_charmInfo->GetActionBarEntry(i)->GetType()) << ' '
				   << uint32(m_charmInfo->GetActionBarEntry(i)->GetAction()) << ' ';
		};
		// Check if alredy exist
		QueryResult petCheck = CharacterDatabase.PQuery("SELECT COUNT(slot) FROM character_pet WHERE owner = '%u' AND id = '%u'", ownerLowGUID, petNumber);
		//
		if (petCheck)
		{
			// Fetch
			Field* fields = petCheck->Fetch();
			// Begin Transaction
			trans = CharacterDatabase.BeginTransaction();
			// Get Pet count
			uint8 count = fields[0].GetUInt8();
			// Pet not exist yet, soul be INSERTED into DB
			if (count == 0)
			{
				// remove current data
				...
				ss  << "INSERT INTO character_pet (id, entry, owner, modelid, CreatedBySpell, PetType, level, exp, Reactstate, name, renamed, slot, curhealth, curmana, savetime, abdata, specialization) VALUES ('"
					<< petNumber << "', '"
					<< GetEntry() << "', '"
					<< ownerLowGUID << "', '"
					<< GetNativeDisplayId() << "', '"
			...
			else // Pet exist, soul be UPDATED into DB
			{
				// save pet
				std::ostringstream ss;
				ss	<< "UPDATE character_pet SET "
					<< "id='" << petNumber << "', "
					<< "entry='" << GetEntry() << "', "
					<< "owner='" << ownerLowGUID << "', "
					<< "modelid='" << GetNativeDisplayId() << "', "
					<< "CreatedBySpell='" << GetUInt32Value(UNIT_CREATED_BY_SPELL) << "', "
					<< "PetType='" << uint32(getPetType()) << "', "
					<< "level='" << uint32(getLevel()) << "', "
					<< "exp='" << GetUInt32Value(UNIT_FIELD_PETEXPERIENCE) << "', "
					<< "Reactstate='" << uint32(GetReactState()) << "', "
					<< "name='" << name.c_str() << "', "
					<< "renamed='" << uint32(HasByteFlag(UNIT_FIELD_BYTES_2, 2, UNIT_CAN_BE_RENAMED) ? 0 : 1) << "', "
					<< "slot='" << uint32(mode) << "', "
					<< "curhealth='" << curhealth << "', "
					<< "curmana='" << curmana << "', "
					<< "savetime='" << time(NULL) << "', "
			...
			CharacterDatabase.CommitTransaction(trans);
			// Update petslotused
			QueryResult petsCount = CharacterDatabase.PQuery("SELECT COUNT(slot) FROM character_pet WHERE owner = '%u'", ownerLowGUID);
			// Prepare to save
			if (petsCount)
			{
				// Fetch
				Field* fields = petsCount->Fetch();
				// Begin Transaction
				trans = CharacterDatabase.BeginTransaction();
				// Get Pet count
				uint8 count = fields[0].GetUInt8();
				// Create Query
				std::ostringstream ss;
				ss << "UPDATE characters SET petslotused='" << ((std::pow(2, count)) - 1) << "' WHERE (guid='" << ownerLowGUID << "')";
				//
				trans->Append(ss.str().c_str());
				// Commit Transaction
				CharacterDatabase.CommitTransaction(trans);
                ...
                else
                {
                    switch (getPowerType())
                    {
                        case POWER_FOCUS:
                            ...
                            break;
						case POWER_ENERGY:
							if (owner->getClass() == CLASS_DEATH_KNIGHT && owner->GetSpecializationId(owner->GetActiveSpec()) == SPEC_DK_UNHOLY &&
								GetEntry() == ENTRY_GHOUL)
								if (GetMaxPower(POWER_ENERGY) > 100)
...
bool Guardian::InitStatsForLevel(uint8 petlevel)
{
    ...
    // Apply auras from glyphs whit SPELL_EFFECT_APPLY_AURA_ON_PET effects
    if (Player* Owner = m_owner->ToPlayer())
    {
        for (uint8 i = 0; i < MAX_GLYPH_SLOT_INDEX; ++i)
        {
            if (uint32 glyph = Owner->GetGlyph(Owner->GetActiveSpec(), i))
            {
                if (GlyphPropertiesEntry const* old_gp = sGlyphPropertiesStore.LookupEntry(glyph))
                {
                    if (const SpellInfo* proto = sSpellMgr->GetSpellInfo(old_gp->SpellId))
                    {
                        if (proto->HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET))
                            AddAura(old_gp->SpellId, this);
                    }
                }
            }
        }
        ...
        default:
        {
            switch (GetEntry())
            {
                ...
                case ENTRY_SHADOWFIEND: // Shadowfiend
                {
                    ...
                    else if (m_owner->HasAura(107903)) // Shadowform visual
                        CastSpell(this, 107903, true);
                ...
                case ENTRY_GARGOYLE:
                {
					...
					HitSpell += m_owner->GetTotalAuraModifier(SPELL_AURA_MOD_SPELL_HIT_CHANCE);
					// Increase hit spell from spell hit ratings
					HitSpell += m_owner->ToPlayer()->GetRatingBonusValue(CR_HIT_SPELL);
					// Take expertise
					float Expertise = 0.0f;
					// Increase Expertise from Expertise ratings
					Expertise += m_owner->ToPlayer()->GetRatingBonusValue(CR_EXPERTISE);
					// mastery receive x2 expertise
					Expertise *= 2;

					// Convert Owner's spell hit chance into the Gargoyle spell hit chance
					m_modSpellHitChance = int32(Expertise + HitSpell);
					// Convert Owner's crit chance into the Gargoyle spell crit chance
					m_baseSpellCritChance = int32(((Player*)m_owner)->GetFloatValue(PLAYER_CRIT_PERCENTAGE) + ((Player*)m_owner)->GetRatingBonusValue(CR_HIT_SPELL));
