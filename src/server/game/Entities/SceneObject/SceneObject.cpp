...
SceneObject* SceneObject::CreateSceneObject(uint32 sceneId, WorldObject* spectator)
{
    ...
    sceneObject->_Create(sceneId, HighGuid::HIGHGUID_SCENEOBJET, spectator->GetPhaseMask());

    sceneObject->SetObjectScale(1);
    sceneObject->SetEntry(sceneId);
	sceneObject->SetUInt32Value(SCENE_SCRIPT_PACKAGE_ID, sceneId);
	sceneObject->SetUInt32Value(SCENE_RND_SEED_VAL, rand() % 0xFFFFFFFF);
	sceneObject->SetUInt64Value(SCENE_CREATED_BY, spectator->GetGUID());
