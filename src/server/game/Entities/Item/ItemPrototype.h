...
enum ItemProtoFlags
{
    ...
    ITEM_PROTO_FLAG_PLAYERCAST                  = 0x00000040, // Item's spells are castable by players
    ITEM_PROTO_FLAG_NO_EQUIP_COOLDOWN           = 0x00000080, // No default 30 seconds cooldown when equipped
    ITEM_PROTO_FLAG_MULTI_LOOT_QUEST            = 0x00000100, // ?
    ...
    ITEM_PROTO_FLAG_HAS_TEXT                    = 0x00004000, // Only readable items have this (but not all)
    ITEM_PROTO_FLAG_NO_DISENCHANT               = 0x00008000, // ?
    ITEM_PROTO_FLAG_REAL_DURATION               = 0x00010000, // ?
    ITEM_PROTO_FLAG_NO_CREATOR                  = 0x00020000, // ?
    ...
    ITEM_PROTO_FLAG_IGNORE_FOR_AURAS            = 0x00100000, // ?
...
enum ItemFlagsExtra2
{
    ITEM_FLAGS_EXTRA2_UPGRADEABLE = 0x00000002,
};
...
struct ItemTemplate
{
    ...
    bool IsUpgradeable() const { return Flags3 & ITEM_FLAGS_EXTRA2_UPGRADEABLE; }

    bool CanUpgrade(uint32 upgradeId = 0) const
    {
        if (upgradeId && !sItemUpgradeStore.LookupEntry(upgradeId))
            return false;

        return IsUpgradeable();
    }

    uint32 GetBaseUpgradeId() const
    {
        if (!CanUpgrade())
            return 0;

        auto itr = sItemRulesetUpgradeMap.find(ItemId);
        if (itr == sItemRulesetUpgradeMap.end())
            return 0;

        return (*itr->second.begin())->itemUpgradeId;
    }
    ...
    static bool CanTransmogrifyItemWithItem(const ItemTemplate* transmogrifier, const ItemTemplate* transmogrified)
    {
        ...
        if (transmogrifier->SubClass != transmogrified->SubClass && !transmogrifier->IsRangedWeapon() && !transmogrified->IsRangedWeapon() &&
            transmogrifier->ItemId != 95475 && transmogrifier->ItemId != 95474 && transmogrifier->ItemId != 97213 && //  // Custom condition for Cosmetic items
            transmogrifier->ItemId < 90049 && transmogrifier->ItemId > 90154) // Challenges sets
            return false;
        ...
        if (transmogrifier->InventoryType != transmogrified->InventoryType && !transmogrifier->IsRangedWeapon() && !transmogrified->IsRangedWeapon())
        {
            ...
            else if (transmogrifier->Class == ITEM_CLASS_ARMOR && transmogrified->Class == ITEM_CLASS_ARMOR)
            {
                if (!((transmogrifier->InventoryType == INVTYPE_CHEST || transmogrifier->InventoryType == INVTYPE_ROBE) &&
                    (transmogrified->InventoryType == INVTYPE_CHEST || transmogrified->InventoryType == INVTYPE_ROBE)))
                    return false;
            }
        }
        ...
        if (transmogrifier->Class == ITEM_CLASS_ARMOR || transmogrified->Class == ITEM_CLASS_ARMOR)
        {
            ...
            // Custom condition for Cosmetic items
            if (transmogrifier->ItemId == 95475 || transmogrifier->ItemId == 95474 || transmogrifier->ItemId == 97213 ||
                (transmogrifier->ItemId >= 90049 && transmogrifier->ItemId <= 90154)) // Challenges sets
                return true;
...
namespace ItemSpecialization
{
    enum ItemSpecStats
    {
        ...
        ITEM_SPEC_STAT_8 = 8,
        ITEM_SPEC_STAT_9 = 9,
        ITEM_SPEC_STAT_10 = 10,
        ITEM_SPEC_STAT_11 = 11,
        ITEM_SPEC_STAT_12 = 12,
        ITEM_SPEC_STAT_13 = 13,
        ITEM_SPEC_STAT_14 = 14,
        ITEM_SPEC_STAT_15 = 15,
        ITEM_SPEC_STAT_16 = 16,
        ITEM_SPEC_STAT_17 = 17,
        ITEM_SPEC_STAT_18 = 18,
        ITEM_SPEC_STAT_19 = 19,
        ITEM_SPEC_STAT_20 = 20,
        //ITEM_SPEC_STAT_21           = 21,
        ITEM_SPEC_STAT_22 = 22,
        ITEM_SPEC_STAT_23 = 23,
        ITEM_SPEC_STAT_CRIT = 24,
        ITEM_SPEC_STAT_HASTE = 25,
        ITEM_SPEC_STAT_26 = 26,
    };

    enum ItemSpecTypes
    {
        ITEM_SPEC_TYPE_UNKNOWN = 0, // cloak ?

        ITEM_SPEC_TYPE_CLOTH = 1,
        ITEM_SPEC_TYPE_LEATER = 2,
        ITEM_SPEC_TYPE_MAIL = 3,
        ITEM_SPEC_TYPE_PLATE = 4,
        ITEM_SPEC_TYPE_WEAPON = 5,
        ITEM_SPEC_TYPE_ADDITIONAL = 6,
    };

    static const ItemSpecStats s_ItemSubClassSpec[ITEM_SUBCLASS_WEAPON_WAND] = { ITEM_SPEC_STAT_7, ITEM_SPEC_STAT_8, ITEM_SPEC_STAT_16, ITEM_SPEC_STAT_15, ITEM_SPEC_STAT_11, ITEM_SPEC_STAT_12, ITEM_SPEC_STAT_19, ITEM_SPEC_STAT_9, ITEM_SPEC_STAT_10, ITEM_SPEC_STAT_26, ITEM_SPEC_STAT_18, ITEM_SPEC_STAT_26, ITEM_SPEC_STAT_26, ITEM_SPEC_STAT_14, ITEM_SPEC_STAT_26, ITEM_SPEC_STAT_13, ITEM_SPEC_STAT_20, ITEM_SPEC_STAT_26, ITEM_SPEC_STAT_17 };
    static const ItemSpecStats s_ItemSubClassSpecEx[6] = { ITEM_SPEC_STAT_22, ITEM_SPEC_STAT_23, ITEM_SPEC_STAT_23, ITEM_SPEC_STAT_23, ITEM_SPEC_STAT_23, ITEM_SPEC_STAT_23 };

    static ItemSpecTypes GetItemType(ItemTemplate const* p_ItemTemplate)
    {
        ItemSpecTypes l_ItemType = ITEM_SPEC_TYPE_UNKNOWN;

        if (p_ItemTemplate->Class == ITEM_CLASS_WEAPON)
        {
            l_ItemType = ITEM_SPEC_TYPE_WEAPON;
        }
        else if (p_ItemTemplate->Class == ITEM_CLASS_ARMOR)
        {
            ...
            case ITEM_SUBCLASS_ARMOR_RELIC:
                l_ItemType = ITEM_SPEC_TYPE_ADDITIONAL;
                break;
            case ITEM_SUBCLASS_ARMOR_CLOTH:
                l_ItemType = p_ItemTemplate->InventoryType != INVTYPE_CLOAK ? ITEM_SPEC_TYPE_CLOTH : ITEM_SPEC_TYPE_UNKNOWN;
                break;
            case ITEM_SUBCLASS_ARMOR_LEATHER:
                l_ItemType = ITEM_SPEC_TYPE_LEATER;
                break;
            case ITEM_SUBCLASS_ARMOR_MAIL:
                l_ItemType = ITEM_SPEC_TYPE_MAIL;
                break;
            case ITEM_SUBCLASS_ARMOR_PLATE:
                l_ItemType = ITEM_SPEC_TYPE_PLATE;
                break;
            default:
                l_ItemType = ITEM_SPEC_TYPE_UNKNOWN;
                break;
            }
        }
    ...
    static ItemSpecStats GetItemSpecStat(ItemModType p_Stat)
    {
        ItemSpecStats l_ItemSpecStat = ITEM_SPEC_STAT_UNKNOWN;
        ...
        case ITEM_MOD_CRIT_RATING:
            l_ItemSpecStat = ITEM_SPEC_STAT_CRIT;
            break;
        case ITEM_MOD_HASTE_MELEE_RATING:
        case ITEM_MOD_HASTE_RANGED_RATING:
        case ITEM_MOD_HASTE_SPELL_RATING:
        case ITEM_MOD_HASTE_RATING:
            l_ItemSpecStat = ITEM_SPEC_STAT_HASTE;
            break;
        case ITEM_MOD_AGILITY:
            l_ItemSpecStat = ITEM_SPEC_STAT_AGILITY;
            break;
        case ITEM_MOD_STRENGTH:
            l_ItemSpecStat = ITEM_SPEC_STAT_STRENGTH;
            break;
        case ITEM_MOD_INTELLECT:
            l_ItemSpecStat = ITEM_SPEC_STAT_INTELLECT;
            break;
        case ITEM_MOD_SPIRIT:
            l_ItemSpecStat = ITEM_SPEC_STAT_SPIRIT;
            break;
        case ITEM_MOD_DODGE_RATING:
            l_ItemSpecStat = ITEM_SPEC_STAT_DODGE;
            break;
        case ITEM_MOD_PARRY_RATING:
            l_ItemSpecStat = ITEM_SPEC_STAT_PARRY;
            break;
        case ITEM_MOD_HIT_RATING:
            l_ItemSpecStat = ITEM_SPEC_STAT_HIT;
            break;
        default:
            l_ItemSpecStat = ITEM_SPEC_STAT_UNKNOWN;
            break;
        }
    ...
    static std::list<ItemSpecStats> GetItemSpecStats(ItemTemplate const* p_ItemTemplate)
    {
        std::list<ItemSpecStats> l_ItemSpecStats;
        ...
        for (uint32 l_Idx = 0; l_Idx < MAX_ITEM_PROTO_STATS; l_Idx++)
        {
            ...
            {
                ItemSpecStats l_ItemSpecStat = GetItemSpecStat((ItemModType)p_ItemTemplate->ItemStat[l_Idx].ItemStatType);
                if (l_ItemSpecStat != ITEM_SPEC_STAT_UNKNOWN)
                    l_ItemSpecStats.push_back(l_ItemSpecStat);
            }
        }

        // Scaling stat
        if (ScalingStatDistributionEntry const* l_ScalingStatDistribution = sScalingStatDistributionStore.LookupEntry(p_ItemTemplate->ScalingStatDistribution))
        {
            for (uint32 l_Idx = 0; l_Idx < MAX_ITEM_PROTO_STATS; l_Idx++)
            {
                if (l_ScalingStatDistribution->StatMod[l_Idx] != -1)
                {
                    ItemSpecStats l_ItemSpecStat = GetItemSpecStat((ItemModType)l_ScalingStatDistribution->StatMod[l_Idx]);
                    if (l_ItemSpecStat != ITEM_SPEC_STAT_UNKNOWN)
                        l_ItemSpecStats.push_back(l_ItemSpecStat);
                }
            }
        ...
        if (p_ItemTemplate->Class == ITEM_CLASS_WEAPON)
        {
            if (p_ItemTemplate->SubClass < ITEM_SUBCLASS_WEAPON_WAND)
            {
                ...
                if (GetItemType(p_ItemTemplate) == ITEM_SPEC_TYPE_ADDITIONAL)
                {
    ...
    static bool HasItemSpecStat(ItemSpecStats p_ItemSpecStat, std::list<ItemSpecStats>& p_ItemStats)
    {
        for (ItemSpecStats l_ItemSpecStat : p_ItemStats)
        {
    ...
    static bool IsAvailableForSpec(ItemSpecTypes itemSpecType, ItemSpecEntry const* l_ItemSpec, std::list<ItemSpecStats>& p_ItemStats)
    {
        bool isItemHasPrimaryStat = HasItemSpecStat((ItemSpecialization::ItemSpecStats)l_ItemSpec->PrimaryStat, p_ItemStats);
        bool isItemHasSecondaryStat = HasItemSpecStat((ItemSpecialization::ItemSpecStats)l_ItemSpec->SecondaryStat, p_ItemStats);
        ...
        bool isHasIntellect = HasItemSpecStat(ITEM_SPEC_STAT_INTELLECT, p_ItemStats);

        if (itemSpecType == ITEM_SPEC_TYPE_UNKNOWN)
        {
            if (isHasIntellect)
            {
                switch (l_ItemSpec->SpecializationID)
                {
        ...
        if (itemSpecType == ITEM_SPEC_TYPE_PLATE)
        {
            if (isHasIntellect)
            {
                switch (l_ItemSpec->SpecializationID)
