...
void GameObject::SaveToDB(uint32 mapid, uint32 spawnMask, uint32 phaseMask)
{
	...
	stmt->setUInt8(index++, uint8(0)); // PhaseID
	...
	stmt->setUInt8(index++, uint8(0)); // protect_anti_doublet
    trans->Append(stmt);
    ...
    switch (GetGoType())
    {
        ...
        case GAMEOBJECT_TYPE_FISHINGHOLE:                   // 25
        {
            ...
            if (Guild* guild = player->GetGuild())
                guild->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CATCH_FROM_POOL, 1, 0, 0, nullptr, player);
            return;
        }
