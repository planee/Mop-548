...
void Creature::Update(uint32 diff)
{
	...
	if (scriptExecuteTime > 15)
		sLog->OutSpecialLog("CreatureScript [%s] creature entry [%u] take more than 15 ms to execute (%u ms)", GetScriptName().c_str(), GetEntry(), scriptExecuteTime);
}
...
{
	EquipmentInfo const* l_EquipInfos = sObjectMgr->GetEquipmentInfo(GetEntry());
	if (!l_EquipInfos)
		return;

	m_equipmentId = l_EquipInfos->ItemEntry[0];

	SetUInt32Value(EUnitFields::UNIT_VIRTUAL_ITEM_SLOT_ID, p_First);
	SetUInt32Value(EUnitFields::UNIT_VIRTUAL_ITEM_SLOT_ID + 1, p_Second);
	SetUInt32Value(EUnitFields::UNIT_VIRTUAL_ITEM_SLOT_ID + 2, p_Third);
...
bool Creature::CanAssistTo(const Unit* u, const Unit* enemy, bool checkfaction /*= true*/) const
{
	...
	// Check Evade mode  -- Helps a lot With Creatures Getting Stuck Eternaly in evade Mode (sync of 3.3.5a)
	if (IsInEvadeMode())
		return false;
