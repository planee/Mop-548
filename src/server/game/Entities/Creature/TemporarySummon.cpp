...
void TempSummon::InitStats(uint32 duration)
{
	...
	if (owner && owner->getClass() == CLASS_DRUID && owner->HasSpell(106737))
	{
		...
		case ENTRY_TREANT_GUARDIAN:
		{
			...
			crit_chance += owner->GetFloatValue(PLAYER_CRIT_PERCENTAGE);
			crit_chance += owner->GetTotalAuraModifierByMiscMask(SPELL_AURA_MOD_ATTACKER_MELEE_CRIT_CHANCE, SPELL_SCHOOL_MASK_NORMAL);
			m_baseSpellCritChance = crit_chance;

			SetMaxHealth(owner->CountPctFromMaxHealth(40));

			// (Attack power / 14 * 2 * 0.75) * 0.2f
			float damage = ((owner->GetTotalAttackPowerValue(BASE_ATTACK) / 14.0f) * 2.0f * 0.75f) * 0.2f;
		...
		case ENTRY_TREANT_FERAL:
		{
			...
			float damage = (owner->GetTotalAttackPowerValue(BASE_ATTACK) / 14.0f) * 2.0f * 0.75f;
			SetStatFloatValue(UNIT_FIELD_MINDAMAGE, damage);
			SetStatFloatValue(UNIT_FIELD_MAXDAMAGE, damage);
			break;
		}
    ...
    if (owner)
    {
        if (uint32 slot = m_Properties->Slot)
        {
            ...
            if (AuraEffectPtr totemicPersistence = owner->GetAuraEffect(108284, EFFECT_0)) // Totemic Persistence
            {
                if (isTotem())
                {
					PlayerTotemType totemType = PlayerTotemType(m_Properties->Id);
					if (totemType != SUMMON_TYPE_TOTEM_FIRE  && totemType != SUMMON_TYPE_TOTEM_FIRE2 &&
						totemType != SUMMON_TYPE_TOTEM_FIRE3 && totemType != SUMMON_TYPE_TOTEM_FIRE4)
					{
