...
void Player::UpdateMasteryPercentage()
{
	...
	if (CanMastery() && getLevel() >= 80)
	{
		...
		float bonus = GetRatingBonusValue(CR_MASTERY);
		value += bonus;
        ...
        if (spellInfo->AttributesEx8 & SPELL_ATTR8_MASTERY_SPECIALIZATION)
        {
            for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
            {
                if (AuraEffectPtr effect = aura->GetEffect(i))
                    aura->CallScriptEffectMasteryUpdateHandlers(effect);
            }
        }
    }
}
...
void Player::UpdateManaRegen()
{
    ...
    {
        float HastePct = 1.0f + GetSpellHastePct(false) / 100.0f;
    ...
    for (AuraEffectList::const_iterator i = ModRegenPct.begin(); i != ModRegenPct.end(); ++i)
    {
        ...
        {
            AddPct(combat_regen, (*i)->GetAmount());
            AddPct(base_regen, (*i)->GetAmount());
        }
    }

    // Increase mana from SPELL_AURA_INCREASE_MANA_REGEN
    ...
    {
        combat_regen += combat_regen * ((*i)->GetAmount() / 100.0f);
        base_regen += base_regen * ((*i)->GetAmount() / 100.0f);
    }
