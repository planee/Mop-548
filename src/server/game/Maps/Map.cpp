...
void InstanceMap::PermBindAllPlayers(Player* source)
{
    ...
    for (MapRefManager::iterator itr = m_mapRefManager.begin(); itr != m_mapRefManager.end(); ++itr)
    {
		...
		// This is to avoid GameMaster Binding to instances
		if (player->isGameMaster())
			break;
		...
		else
		{
			...
			{
				sLog->outError(LOG_FILTER_MAPS, "Player (GUID: %u, Name: %s) is in instance map (Name: %s, Entry: %u, Difficulty: %u, ID: %u) that is being bound, but already has a save for the map on ID %u!", player->GetGUID(), player->GetName(), GetMapName(), save->GetMapId(), save->GetDifficulty(), save->GetInstanceId(), bind->save->GetInstanceId());
			}
			else if (!bind->save)
			{
				sLog->outError(LOG_FILTER_MAPS, "Player (GUID: %u, Name: %s) is in instance map (Name: %s, Entry: %u, Difficulty: %u, ID: %u) that is being bound, but already has a bind (without associated save) for the map!", player->GetGUID(), player->GetName(), GetMapName(), save->GetMapId(), save->GetDifficulty(), save->GetInstanceId());
			}
		}
    }
}
