    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_QUESTTAKEN:
        {
            if (Player* player = object->ToPlayer())
            {
				...
				//condMeets = (status != QUEST_STATUS_NONE && player->IsActiveQuest(ConditionValue1));
				condMeets = player->GetQuestStatus(ConditionValue1);
            }
            break;
        }
		//case CONDITION_QUESTTAKEN:
...
bool ConditionMgr::isConditionTypeValid(Condition* cond)
{
    ...
    switch (cond->ConditionType)
    {
        ...
        case CONDITION_CREATURE_TYPE:
        {
            ...
            {
                sLog->outError(LOG_FILTER_SQL, "CreatureType condition has non existing CreatureType in value1 (%u), skipped", cond->ConditionValue1);
                return false;
            }
            break;
        }
