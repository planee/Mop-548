    ...
    do
    {
        ...
        switch (type)
        {
			...
			case DISABLE_TYPE_MMAP:
			{
				...
				if (!mapEntry)
				{
					sLog->outInfo(LOG_FILTER_GENERAL, "Map entry %u from `disables` doesn't exist in dbc, skipped.", entry);
					continue;
				}
				switch (mapEntry->map_type)
				{
...
bool IsDisabledFor(DisableType type, uint32 entry, Unit const* unit, uint8 flags)
{
	...
	DisableTypeMap::iterator itr;

#if defined(__WIN__)
	__try
	{
#endif
		itr = m_DisableMap[type].find(entry);
