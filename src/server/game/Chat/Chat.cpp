        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            if (!AccountMgr::IsPlayerAccount(table[i].SecurityLevel))
            {
                ...
                if (m_session)
                {
					...
					/*Extra*/
					AreaTableEntry const* areaEntry = GetAreaEntryByAreaID(p->GetAreaId());
					AreaTableEntry const* zoneEntry = GetAreaEntryByAreaID(p->GetZoneId());
					if (!sel_guid || sel_guid == 0)
					{
						toLogFile(m_session, fullcmd, p);
						//sLog->outGMCommand(m_session->GetAccountId(), "Command: %s [Player: %s (Account: %u) (Ip: %s) || (Area: %s) (Zone: %s) Position: X: %f Y: %f Z: %f Map: %u || Selected %s: %s (GUID: %u)]",
						//	fullcmd.c_str(), p->GetName(), m_session->GetAccountId(), m_session->GetRemoteAddress(), areaEntry->area_name, zoneEntry->area_name, p->GetPositionX(), p->GetPositionY(), p->GetPositionZ(), p->GetMapId(),
						//	"none", "", 0);
					}
					else
					{
...
void ChatHandler::toLogFile(WorldSession* aM_session, const std::string& aFullcmd, Player* aP)
{
#if defined(__WIN__)
	__try
	{
#endif
		/*Extra*/
		AreaTableEntry const* areaEntry = GetAreaEntryByAreaID(aP->GetAreaId());
		AreaTableEntry const* zoneEntry = GetAreaEntryByAreaID(aP->GetZoneId());
		sLog->outGMCommand(aM_session->GetAccountId(), "Command: %s [Player: %s (Account: %u) (Ip: %s) || (Area: %s) (Zone: %s) Position: X: %f Y: %f Z: %f Map: %u || Selected %s: %s (GUID: %u)]",
			aFullcmd.c_str(), aP->GetName(), aM_session->GetAccountId(), aM_session->GetRemoteAddress(), areaEntry->area_name, zoneEntry->area_name, aP->GetPositionX(), aP->GetPositionY(), aP->GetPositionZ(), aP->GetMapId(),
			"none", "", 0);
#if defined(__WIN__)
	}
	__except (EXCEPTION_ACCESS_VIOLATION)
	{
		...
		std::cout << "ExecuteCommandInTable has trown a exception" << std::endl;
	}
#endif
}
