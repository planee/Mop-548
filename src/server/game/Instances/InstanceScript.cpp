...
{
    //player->RemoveAura(eInstanceSpells::SpellDetermination); FIX IN FUTURE
    player->RemoveAurasByType(AuraType::SPELL_AURA_TRIGGER_BONUS_LOOT);
}
...
void InstanceScript::DoModifyPlayerCurrenciesIfLevel(uint32 id, int32 value, int8 level)
{
	Map::PlayerList const &plrList = instance->GetPlayers();
...
void InstanceScript::DoAddTitleIfLevel(uint32 title, int8 level)
{
	Map::PlayerList const &plrList = instance->GetPlayers();
