    ...
    m_achievementMgr(this), _level(1), _experience(0), _todayExperience(0), _newsLog(this)
{
	...
	m_ChallengeCount.resize(GuildChallengeType::CHALLENGE_MAX);

	for (uint8 l_Type = 0; l_Type < GuildChallengeType::CHALLENGE_MAX; ++l_Type)
...
void Guild::HandleRoster(WorldSession* session /*= NULL*/)
{
    ...
    for (Members::const_iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
       ...
       // for (2 professions) TODO:@ Implementar que muestre los valores de las profesiones
		for (int i = 0; i < 2; ++i)
		{
			uint32 id = player ? player->GetUInt32Value(PLAYER_PROFESSION_SKILL_LINE_1 + i) : 0;

			if (id)/*Muestra los datos de quien tiene profesion en el guild pero no muestra los valores*/
...
void Guild::HandleSetLeader(WorldSession* session, const std::string& name)
{
    ...
    else if (Member* pOldLeader = GetMember(player->GetGUID()))
    {
        ...
        if (Member* pNewLeader = GetMember(session, name))
        {
            ...
            SendGuildNewLeader(pNewLeader, pOldLeader);
        }
    }
    else
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint64 amount, bool repair)
{
    ...
    if (!repair)
        ...
        GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_SPENT_GOLD_GUILD_REPAIRS, amount, 0, 0, nullptr, player);
    return true;
}
...
bool Guild::LoadGuildChallengesFromDB(Field* p_Fields)
{
	...
	if (l_ChallengeType >= GuildChallengeType::CHALLENGE_MAX)
		return false;
...
void Guild::DeleteMember(uint64 guid, bool isDisbanding, bool isKicked, bool canDeleteGuild)
{
    ...
    if (m_leaderGuid == guid && !isDisbanding)
    {
        ...
        if (oldLeader)
        {
            SendGuildNewLeader(newLeader, oldLeader);
...
void Guild::SendGuildNewLeader(Member* newLeader, Member* oldLeader)
{
    ObjectGuid guid1 = newLeader->GetGUID();
    ObjectGuid guid2 = oldLeader->GetGUID();

    WorldPacket data(SMSG_GUILD_EVENT_NEW_LEADER, 32);
    ...
    data.WriteBit(guid2[7]);
    data.WriteBit(guid2[1]);
    data.WriteBit(guid1 == guid2); // self promoted
    data.WriteBit(guid2[6]);
    data.WriteBit(guid2[0]);
...
{
	if (p_ChallengeType >= GuildChallengeType::CHALLENGE_MAX)
		return;
	...
	int32 l_GoldReward = l_RewardDatas[p_ChallengeType].Gold2;
	int32 l_XPReward = l_RewardDatas[p_ChallengeType].Experience;
	...
	WorldPacket l_Data(Opcodes::SMSG_GUILD_CHALLENGE_COMPLETED, 5 * 4);
	l_Data << int32(l_XPReward);
...
void Guild::SendGuildChallengeUpdated(WorldSession* p_Session /*= nullptr*/)
{
	GuildChallengeRewardData const& l_Rewards = sObjectMgr->GetGuildChallengeRewardData();

	WorldPacket l_Data(Opcodes::SMSG_GUILD_CHALLENGE_UPDATED, 5 * 6 * 4);

	for (uint8 l_I = 0; l_I < GuildChallengeType::CHALLENGE_MAX; l_I++)
		l_Data << uint32(l_Rewards[l_I].Experience);

	for (uint8 l_I = 0; l_I < GuildChallengeType::CHALLENGE_MAX; l_I++)
		l_Data << uint32(l_Rewards[l_I].ChallengeCount);

	for (uint8 l_I = 0; l_I < GuildChallengeType::CHALLENGE_MAX; l_I++)
		l_Data << uint32(GetChallengeCount(l_I));

	for (uint8 l_I = 0; l_I < GuildChallengeType::CHALLENGE_MAX; l_I++)
		l_Data << uint32(l_Rewards[l_I].Gold2);

	for (uint8 l_I = 0; l_I < GuildChallengeType::CHALLENGE_MAX; l_I++)
		l_Data << uint32(l_Rewards[l_I].Gold);
...
void Guild::GuildNewsLog::AddNewEvent(GuildNews eventType, time_t date, uint64 playerGuid, uint32 flags, uint32 data)
{
	...
	// Deleting Previous Record before Inserting to avoid SQL error
	QueryResult result = CharacterDatabase.PQuery("SELECT guild, id FROM guild_news_log WHERE guild = %u AND id = %u LIMIT 1", GetGuild()->GetId(), id);
	if (result)
	{
		CharacterDatabase.PQuery("DELETE FROM guild_news_log WHERE guild = %u AND id = %u", GetGuild()->GetId(), id);
	}
