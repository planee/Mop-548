...
void LoadDB2Stores(const std::string& dataPath)
{
	...
	for (uint32 i = 0; i < sRulesetItemUpgradeStore.GetNumRows(); ++i)
	{
		if (RulesetItemUpgradeEntry const* ruleset = sRulesetItemUpgradeStore.LookupEntry(i))
		{
			...
			if (!sItemUpgradeStore.LookupEntry(ruleset->itemUpgradeId))
			{
				sLog->outError(LOG_FILTER_GENERAL, "Upgrade %u listed in sRulesetItemUpgradeStore id %u not exist", ruleset->itemUpgradeId, ruleset->Id);
				continue;
			}

			// Legendary back has 2\4 in db2
			if (ruleset->itemUpgradeId == 493)
				const_cast<RulesetItemUpgradeEntry*>(ruleset)->itemUpgradeId = 491;
