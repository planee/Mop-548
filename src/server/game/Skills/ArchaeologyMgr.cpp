...
uint16 ArchaeologyMgr::GetResearchSiteID()
{
	...
	for (auto const kvp : zoneMap)
	{
		...
		{
			if (WoWSource::IsPointInZone(test, entry.coords))
				return kvp.first;
		}
	}
	return 0;
...
void ArchaeologyMgr::ShowResearchSites()
{
	...
	for (auto const& set : _researchSites)
	{
		...
		{
			_player->SetDynamicUInt32Value(PLAYER_DYNAMIC_RESEARCH_SITES, idx, CanResearchWithLevel(id) == RS_RESULT_HIDE ? 0 : id);
...
void ArchaeologyMgr::ShowResearchProjects()
{
	...
	{
		_player->SetUInt16Value(PLAYER_FIELD_RESEARCHING + (count / 2), count % 2, project);
...
void ArchaeologyMgr::GenerateResearchSites()
{
	...
	for (auto& set : _researchSites)
		WoWSource::Containers::RandomResizeSet(set, RESEARCH_SITES_PER_MAP);
...
void ArchaeologyMgr::GenerateResearchProjects()
{
	...
	for (auto const& kvp : sResearchProjectMap)
	{
		...
		auto itr = WoWSource::Containers::SelectRandomWeightedContainerElement(kvp.second, [this](ResearchProjectEntry const* entry) -> double
		{
...
bool ArchaeologyMgr::SolveResearchProject(uint32 projectId)
{
	...
	for (ProjectCost const& cost : costData)
	{
		...
		/* This little hack disrupt conjuction of Researchs, Consuming ALL currency on current project*/
		   _player->SetCurrency(cost.itemId, 0);
		else
	...
	// Add new project
	auto itr = WoWSource::Containers::SelectRandomWeightedContainerElement(sResearchProjectMap[entry->branchId], [this](ResearchProjectEntry const* entry) -> double
	{
		if (entry->rare && IsCompletedProject(entry->ID))
			return 0.0;
...
uint16 ArchaeologyMgr::GetRandomActiveSiteInMap(uint32 mapId) const
{
	...
	WoWSource::Containers::RandomResizeList(sitesId, 1);
