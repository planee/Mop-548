...
// Alchemy Spells
// Elixir
const static uint32 _ElixirSpells[124]{ 105687, 114763, 105685, 114764, 105686, 114762, 105684, 114759, 105683, 114756, 105682, 114754, 105681, 114755, 105688, 114758, 79635, 80497, 80493,
80491, 79631, 79480, 80488, 79477, 80484, 79474, 80480, 79468, 80477, 60354, 60365, 60355, 60357, 60366, 60356, 56519, 54220, 62410, 53840, 60367, 53751, 53898, 54218, 53847, 53749, 53848, 33721, 53842, 53746, 28578, 28570, 28558, 28557, 28556, 38954 ,
38960, 39628, 39639, 28553, 28552, 39627, 39638, 28549, 39626, 39637, 33726, 33741, 24361, 24368, 28545, 39625, 39636, 28543, 28544, 17538, 17571, 37740, 54452, 33720, 33738, 17537, 17557, 24363, 24365, 17535, 17555, 17554, 11406, 11477, 11478, 26277, 11476,
11468, 11467, 11371, 11390, 11461, 11465, 11460, 22808, 12608, 12609, 3453, 11450, 21923, 11449, 80472, 3223, 3451, 3450, 3593, 3188, 7845, 2333, 63732, 3177, 3176, 3222, 3171, 7179, 3230, 3170, 2329, 7183};

// Potions
const static uint32 _PotionSpells[126]{ 2330, 2331, 4508, 2337, 6617, 2335, 7841, 7255, 439, 3173, 3174, 7256, 6624, 7181, 3452, 3448, 6618, 7258, 7257,   
7259, 11448, 11452, 11522, 4942, 11457, 11387, 11458, 15833, 11464, 17527, 17552, 3175, 17553, 17556, 33732, 17634, 22732, 24366, 17570, 17572, 24367,
33733, 17578, 17576, 17579, 17575, 17574, 17577, 28546, 17580, 28494, 28550, 45051, 45061, 28551, 28554, 28555, 38908, 28576, 28573, 28577, 28572, 28571,
28562, 28575, 28563, 28564, 28565, 38961, 28579, 28586, 28600, 53838, 53839, 53900, 58871, 53938, 53942, 53937, 53939, 53936, 58868, 53837, 53904, 54222, 54221,
53905, 53750, 53895, 93935, 80478, 79625, 80479, 80481, 80265, 80269, 80482, 78778, 80487, 80490, 80494, 80495, 80496, 80498, 80725, 80263, 80726, 11452, 105704,
114751, 105698, 15822, 114753, 105702, 114757, 105697, 114765, 105706, 114760, 105701, 114782, 105707, 114774, 114775, 105699, 135855};

// Transmutations
const static uint32 _TransmutationSpells[59]{ 130326, 114780, 11477, 114781, 114784, 114766, 114778, 114783, 114776, 114767, 80244, 80237, 80243, 80247, 80245, 80248,
80246, 80250, 80251, 78866, 66891, 66887, 66890, 66784, 66892, 66888, 54020, 57425, 60350, 53777, 53776, 32765, 32766, 28566, 28669, 28567, 28568, 28667, 28668, 28666, 29688,
28665, 25146, 17559, 17187, 17566, 17561, 17560, 17565, 17563, 17562, 11480, 11479};
...
pEffect SpellEffects[TOTAL_SPELL_EFFECTS] =
{
	  ...
	  &Spell::EffectUnused,                                   // 91 SPELL_EFFECT_THREAT_ALL               one spell: zzOLDBrainwash, handled by EffectThreatAll but crash the server for some unknown reason
      ...
      &Spell::EffectDespawnAreaTrigger,                       //177 SPELL_EFFECT_DESPAWN_AREA_TRIGGER
      ...
      &Spell::EffectDespawnAreaTrigger,                       //182 SPELL_EFFECT_DESPAWN_AREA_TRIGGER_2
	  ...
	  &Spell::EffectBattlePetUpgrade,                         //204 SPELL_EFFECT_UPGRADE_BATTLEPET
      &Spell::EffectNULL,                                     //205 SPELL_EFFECT_PVE_PVP_CHOICE
      &Spell::EffectNULL,                                     //206 SPELL_EFFECT_ETERNITY_EMBRACE
      &Spell::EffectNULL,                                     //207 SPELL_EFFECT_207
      &Spell::EffectNULL,                                     //208 SPELL_EFFECT_TEST_REPUTATION
};

enum AlchemySpec
{
	TRANSMUTE_MASTER = 28672,
	ELIXIR_MASTER = 28677,
	POTION_MASTER = 28675
};
      ...
      if (unitTarget && unitTarget->isAlive())
      {
            ...
            case SPELLFAMILY_WARRIOR:
            {
                                          ...
                                          case 46968: // Shockwave
                                          {
                                                            ...
                                                            damage = int32(CalculatePct(m_caster->GetTotalAttackPowerValue(BASE_ATTACK), 75) * bonus);
            ...
            case SPELLFAMILY_WARLOCK:
            {
										  ...
										  case 54049: // Shadow Bite
										  {
														  ...
														  {
															  // Bite the enemy, causing 405 (+38% SP) Shadow damage.
															  damage += int32(m_caster->GetOwner()->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_SHADOW) * 3.8f);
														  }
										...
										case 2098:  // Eviscerate
										{
                                                                  if (m_caster->GetTypeId() == TYPEID_PLAYER)
                                                                  {
                                                                        if (uint32 combo = ((Player*)m_caster)->GetComboPoints())
                                                                        {
                                                                              ...
                                                                              uint8 _playerLevel = m_caster->getLevel();
                                                                              uint16 _minConst = 0;
                                                                              uint16 _maxConst = 0;
                                                                              float _commConst = 0;
                                                                              // From level 3 to level 39
                                                                              if (_playerLevel >= 3 && _playerLevel <= 39)
                                                                              {
                                                                                    _minConst = uint16(-0.0006f * std::pow(_playerLevel, 3) + 0.048f * std::pow(_playerLevel, 2) - 0.1155f * _playerLevel + 10.26f);
                                                                                    _maxConst = uint16(-0.0019f * std::pow(_playerLevel, 3) + 0.1519f * std::pow(_playerLevel, 2) - 0.4479f * _playerLevel + 30.449f);
                                                                                    // From level 3 to level 12 And level 13 to level 39
                                                                                    if (_playerLevel >= 3 && _playerLevel <= 12)
                                                                                          _commConst = float(-0.012f * std::pow(_playerLevel, 3) + 0.3256f * std::pow(_playerLevel, 2) - 1.5933f * _playerLevel + 30.293f);
                                                                                    else
                                                                                          _commConst = float(0.0005f * std::pow(_playerLevel, 3) - 0.0459f * std::pow(_playerLevel, 2) + 4.3039f * _playerLevel - 10.949f);
                                                                              }
                                                                              // From level 40 to level 58
                                                                              if (_playerLevel >= 40 && _playerLevel <= 58)
                                                                              {
                                                                                    _minConst = uint16(0.0033f * std::pow(_playerLevel, 3) - 0.405f * std::pow(_playerLevel, 2) + 19.74f * _playerLevel - 307.61f);
                                                                                    _maxConst = uint16(0.0063f * std::pow(_playerLevel, 3) - 0.6935f * std::pow(_playerLevel, 2) + 34.847f * _playerLevel - 548.51f);
                                                                                    _commConst = float(-0.0005f * std::pow(_playerLevel, 3) + 0.2579f * std::pow(_playerLevel, 2) - 11.159f * _playerLevel + 184.22f);
                                                                              }
                                                                              // From level 59 to level 90
                                                                              if (_playerLevel >= 59)
                                                                              {
                                                                                    _minConst = uint16(-0.002f * std::pow(_playerLevel, 3) + 0.4744f * std::pow(_playerLevel, 2) - 29.99f * _playerLevel + 674.5f);
                                                                                    _maxConst = uint16(-0.0069f * std::pow(_playerLevel, 3) + 1.615f * std::pow(_playerLevel, 2) - 104.68f * _playerLevel + 2390.9f);
                                                                                    _commConst = float(-0.0066f * std::pow(_playerLevel, 3) + 1.5251f * std::pow(_playerLevel, 2) - 99.373f * _playerLevel + 2271.2f);
                                                                              }
                                                                              // Damage
                                                                              damage = int32(frand(_minConst + _commConst * combo + ap * 0.18f * combo, _maxConst + _commConst * combo + ap * 0.18f * combo));
            ...
            case SPELLFAMILY_HUNTER:
            {
                                         ...
                                         case 17253: // Bite
                                         {
                                                           if (m_caster->GetOwner())
                                                           {
                                                                 damage += int32(m_caster->GetOwner()->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.168f);
            ...
            case SPELLFAMILY_MONK:
            {
                                       ...
                                       case 115080:// Touch of Death
                                             if (Unit* caster = GetCaster())
                                             {
                                                   if (unitTarget)
                                                   {
                                                         uint32 damage = unitTarget->GetMaxHealth();
                                       ...
                                       case 123986:// Chi Burst
                                           damage = m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * float(1.4841711412f);
                                           break;
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
      ...
      case SPELLFAMILY_DEATHKNIGHT:
      {
                                        ...
                                        {
                                            if (m_caster != unitTarget && m_caster->IsFriendlyTo(unitTarget) && m_caster->HasAura(63333) &&
                                                unitTarget->GetCreatureType() != CREATURE_TYPE_UNDEAD && !unitTarget->HasAura(49039)) // death coil glyph
                                            {
                                                SpellInfo const* spell = sSpellMgr->GetSpellInfo(47541);
                                                ...
                                                int32 bp = m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * 0.75;

                                                m_caster->CastCustomSpell(unitTarget, 115635, &bp, NULL, NULL, true, NULL);
                                            }
                                            else if (m_caster->IsFriendlyTo(unitTarget))
                                            {
      ...
      case 106299:// Summon Living Air
      {
                        ...
                        //enne->AddThreat(m_caster, 2000.f); // this is handled by script.
                        break;
      }
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
      ...
      case 110726: // Leap of Faith (Jump)
...
void Spell::EffectApplyAura(SpellEffIndex effIndex)
{
      ...
      // Touch of Karma (absorb aura) - 122470, Divine Aegis - 47753,  Mastery: Illuminated Healing - 76669, Spirit Shell - 114908 must ignore Battle Fatigue
      if (m_spellAura->GetId() != 122470 && m_spellAura->GetId() != 47753 && m_spellAura->GetId() != 86273 && m_spellAura->GetId() != 114908)
      {
        AuraEffectPtr Eff = m_spellAura->GetEffect(effIndex);
        if (Eff && Eff->GetAuraType() == SPELL_AURA_SCHOOL_ABSORB)
        {
              float TakenTotalMod = 1.0f;

              // Battle Fatigue And Others Modifications
              if (m_spellAura->GetId() != 48707)
                    if (auto amount = unitTarget->GetTotalAuraModifier(SPELL_AURA_MOD_ABSORPTION_PCT))
                          AddPct(TakenTotalMod, amount);

              // Dampening
              if (auto auraEffect = unitTarget->GetAuraEffect(110310, EFFECT_0))
                    AddPct(TakenTotalMod, -auraEffect->GetAmount());

              // Apply PvP Power healing bonus
              if (GetCaster()->GetTypeId() == TYPEID_PLAYER && (unitTarget->GetTypeId() == TYPEID_PLAYER || (unitTarget->GetTypeId() == TYPEID_UNIT && unitTarget->GetOwner() && unitTarget->GetOwner()->ToPlayer())))
              {
                  if (unitTarget->HasAura(134735)) // Battle Fatigue
                  {
                      float PvPPower = GetCaster()->ToPlayer()->GetPvpPowerHealingPct();
                      AddPct(TakenTotalMod, PvPPower);
                  }
              }

              int currentValue = (int)(Eff->GetAmount() * TakenTotalMod);

              Eff->SetAmount(currentValue);
        }
      }
      ...
      if (unitTarget && unitTarget->isAlive() && damage >= 0)
      {
            ...
            case 147489:// Expel Harm with glyph of Targeted Expulsion
            {
                              if (caster && caster->getClass() == CLASS_MONK && addhealth && (m_spellInfo->Id == 115072 || m_spellInfo->Id == 147489))
                              {
                                    ...
                                    {
                                          if (SpellInfo const* ExpelHarmGlyph = sSpellMgr->GetSpellInfo(146950))
                                                addhealth -= CalculatePct(addhealth, ExpelHarmGlyph->Effects[EFFECT_1].BasePoints);
                                    }
                              }
            ...
            case 123986:// Chi Burst
            {
                addhealth = m_caster->GetTotalAttackPowerValue(BASE_ATTACK) * float(1.0266041449f);
                break;
            }
			...
			if (caster && caster->GetTypeId() == TYPEID_PLAYER && caster->getClass() == CLASS_DRUID && m_spellInfo->Id != 81269 && m_spellInfo->Id != 33778 && m_spellInfo->Id != 145109 && m_spellInfo->Id != 142424)
            {
...
void Spell::EffectHealPct(SpellEffIndex /*effIndex*/)
{
	  ...
	  auto pctFromHp = unitTarget->CountPctFromMaxHealth(damage);
	  if (m_spellInfo->Id == 63106)
		  pctFromHp /= 100; // convert 50% of max hp to 0.5%

	  uint32 heal = m_originalCaster->SpellHealingBonusDone(unitTarget, m_spellInfo, pctFromHp, HEAL);
...
void Spell::DoCreateItem(uint32 /*i*/, uint32 itemtype)
{
	  ...
	  // Alchemy Specializations [Creating more elements of ItemType] with 50 % chance
	  if (roll_chance_i(50) && player->GetBaseSkillValue(SKILL_ALCHEMY) > 524)
	  {
		   // Potion Master
		  if (player->HasSpell(POTION_MASTER))
		  {
			  if (IsAlchemyAvailable(pProto->Spells[0].SpellId, 1))
				  items_count = items_count + 1;
		  }
		  // Elixir Master
		  else if (player->HasSpell(ELIXIR_MASTER))
		  {
			  if (IsAlchemyAvailable(pProto->Spells[0].SpellId, 2))
				  items_count = items_count + 1;
		  }
		  // Transmutation Master
		  else if (player->HasSpell(TRANSMUTE_MASTER))
		  {
			  if (IsAlchemyAvailable(m_spellInfo->Id, 3))
				items_count = items_count + 1;
		  }
	  }
      ...
      if (num_to_add)
      {
            ...
            if (Guild* guild = sGuildMgr->GetGuildById(player->GetGuildId()))
            {
                ...
                guild->GetAchievementMgr().UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_CRAFT_ITEMS_GUILD, newitemid, num_to_add, 0, nullptr, player);
            }
...
/*
   This function finds out if a selected spell is a Potion, an elixir or Transmutation
   Returns True if the Spell matches the Specialization
*/
bool Spell::IsAlchemyAvailable(uint32 spellId, uint8 alchemySpec)
{
	switch (alchemySpec)
	{
		case 1: // Potion
			for (uint8 i = 0; i <= 126; ++i)
				if (_PotionSpells[i] == spellId)
					return true;
			break;
		case 2: // Elixir
			for (uint8 i = 0; i <= 124; ++i)
				if (_ElixirSpells[i] == spellId)
					return true;
			break;
		case 3: // Transmute
			for (uint8 i = 0; i <= 59; ++i)
				if (_TransmutationSpells[i] == spellId)
					return true;
			break;
	}
	return false;
...
void Spell::EffectPersistentAA(SpellEffIndex effIndex)
{
      ...
      if (!m_spellAura)
      {
            ...
            if (AuraPtr aura = Aura::TryCreate(m_spellInfo, MAX_EFFECT_MASK, dynObj, caster, m_spellPowerData, &m_spellValue->EffectBasePoints[0], nullptr, 0, m_castItemLevel))
            {
                  m_spellAura = aura;
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
      ...
      case SUMMON_CATEGORY_UNK:
            ...
            case SUMMON_TYPE_MINIPET:
            {
				...
				Minion* minion = static_cast<Minion*>(summon);
				if (!minion)
					return;
				...
				if (player)
				{
					...
					BattlePet::PetTemplate* proto = journal->GetBattlePet(journal->GetCurrentSummonId());
					if (!proto)
						return;

					journal->SetCurrentSummon(summon);
					minion->InitialBattlePet(proto, player->GetGUID());

					player->SetUInt64Value(PLAYER_FIELD_SUMMONED_BATTLE_PET_GUID, proto->GetId());
					player->SetUInt32Value(PLAYER_FIELD_CURRENT_BATTLE_PET_BREED_QUALITY, proto->GetQuality());
				}
				else
				{
					BattlePet::PetTemplate* proto = minion->CreateBattlePet();
					ASSERT(proto);

					minion->InitialBattlePet(proto, m_caster->GetGUID());
				}
            ...
            default:
            {
                         ...
                         for (uint32 count = 0; count < numSummons; ++count)
                         {
                               ...
                               if (properties->Type == SUMMON_TYPE_LIGHTWELL)
                               {
                                     uint32 health = m_caster->CountPctFromMaxHealth(50);
...
void Spell::EffectDispel(SpellEffIndex effIndex)
{
      ...
      case 77130: // Purify Spirit
            if (m_caster->HasAura(55445)) // Glyph of Cleansing Waters
...
void Spell::EffectApplyGlyph(SpellEffIndex effIndex)
{
      ...
      if (uint32 glyph = m_spellInfo->Effects[effIndex].MiscValue)
      {
            if (GlyphPropertiesEntry const* gp = sGlyphPropertiesStore.LookupEntry(glyph))
            {
                  ...
                  if (const SpellInfo* proto = sSpellMgr->GetSpellInfo(gp->SpellId))
                  {
                      if (proto->HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET))
                      {
                          if (auto pet = player->GetPet())
                          {
                              player->AddAura(gp->SpellId, pet);
                              if (proto->HasAura(SPELL_AURA_TRANSFORM))
                                  pet->CastSpell(pet, 24085, true); // Transform Visual
                          }
                      }
                      else
                      {
      ...
      else
      {
            ...
            if (uint32 oldglyph = player->GetGlyph(player->GetActiveSpec(), m_glyphIndex))
            {
                  if (GlyphPropertiesEntry const* old_gp = sGlyphPropertiesStore.LookupEntry(oldglyph))
                  {
                        ...
                        if (const SpellInfo* proto = sSpellMgr->GetSpellInfo(old_gp->SpellId))
                        {
                            if (proto->HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET))
                            {
                                if (auto pet = player->GetGuardianPet())
                                {
                                    if (proto->HasAura(SPELL_AURA_TRANSFORM))
                                        pet->CastSpell(pet, 24085, true); // Transform Visual

                                    pet->RemoveAurasDueToSpell(old_gp->SpellId);
                                }
                            }
                        }
                  }
...
void Spell::EffectForcePlayerInteraction(SpellEffIndex effIndex)
{
    ...
    m_caster->CastSpell(l_Target, m_spellInfo->Effects[effIndex].TriggerSpell);
}
...
void Spell::EffectActivateSpec(SpellEffIndex /*effIndex*/)
{
      ...
      {
          player->ResetGearSpecizalization();
          player->ActivateSpec(damage - 1);  // damage is 1 or 2, spec is 0 or 1
          player->UpdateSecondaryPower();
          player->UpdateGearSpecialization();
      }
}
...
void Spell::EffectGiveCurrency(SpellEffIndex effIndex)
{
	  ...
	  if (!sCurrencyTypesStore.LookupEntry(m_spellInfo->Effects[effIndex].MiscValue))
		  return;
...
void Spell::EffectDespawnAreaTrigger(SpellEffIndex p_EffIndex)
{
    ...
    else
    {
        ...
        CellCoord l_Pair(WoWSource::ComputeCellCoord(m_caster->GetPositionX(), m_caster->GetPositionY()));
        Cell l_Cell(l_Pair);
        l_Cell.SetNoCreate();

        WoWSource::AllAreaTriggersInRangeCheck l_Check(m_caster, l_Radius);
        WoWSource::AreaTriggerListSearcher<WoWSource::AllAreaTriggersInRangeCheck> l_Searcher(m_caster, l_AreaTriggers, l_Check);
        TypeContainerVisitor<WoWSource::AreaTriggerListSearcher<WoWSource::AllAreaTriggersInRangeCheck>, GridTypeMapContainer> l_Visitor(l_Searcher);
...
void Spell::EffectStampede(SpellEffIndex p_EffIndex)
{
    ...
    bool OnlyCurrentPet = _player->HasAuraType(SPELL_AURA_STAMPEDE_ONLY_CURRENT_PET);
    ...
    for (uint32 i = uint32(PET_SLOT_HUNTER_FIRST); i <= uint32(PET_SLOT_HUNTER_LAST); ++i)
    {
        if (i != currentSlot)
        {
            ...
            Pet* pet = _player->SummonPet(0, x, y, z, _player->GetOrientation(), SUMMON_PET, _player->CalcSpellDuration(GetSpellInfo()), OnlyCurrentPet ? PetSlot(currentSlot) : PetSlot(i), true);
            if (!pet)
                return;
            ...
            pet->CastSpell(pet, 130201, true); // Stampede Damage Reduction
            pet->AI()->AttackStart(target);
        }
    }
}

void Spell::EffectTeleportToDigsite(SpellEffIndex p_EffIndex)
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;
    ...
    //switch (m_spellInfo->Id) Cuando se emigre la arqueologia por Nekroz descomentar esto
    //{
    ...
    //        target->NearTeleportTo(itr->x, itr->y, itr->z, target->GetOrientation());
    //        break;
    //    }
    //    default:
    //        break;
...
void Spell::EffectLootBonus(SpellEffIndex effIndex)
{
    if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT_TARGET)
        return;

    if (!unitTarget || !unitTarget->IsInWorld() || effIndex != EFFECT_0)
        return;
    ...
    if (m_CastItem)
    {
        // TODO: enable loot for some items
        // need to check for all items and spells
        if (m_CastItem->GetEntry() == 98546 ||  // Bulging Heroic Cache of Treasures
            m_CastItem->GetEntry() == 98134 ||  // Heroic Cache of Treasures
            m_CastItem->GetEntry() == 104014)   // Pouch of Timeless Coins
        {
            bool isGuaranteedLoot = false;
            if (m_CastItem->GetEntry() == 98546)
                isGuaranteedLoot = true;

            SpellBonusLootProcessor processor(m_spellInfo->Id);
            processor.SetGuaranteedLoot(isGuaranteedLoot);
            processor.ProcessLootFor(l_Player);

            l_Player->DestroyItem(m_CastItem->GetBagSlot(), m_CastItem->GetSlot(), true);
    ...
    else
    {
        ...
        CreatureBonusLootProcessor bonusLoot(l_Caster->ToCreature());
        bonusLoot.ProcessLootFor(l_Player);
    }
}
...
void Spell::EffectBattlePetUnCage(SpellEffIndex effIndex)
{
	if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT)
		return;
	...
	if (Item* item = player->GetItemByGuid(m_castItemGUID))
	{
		uint32 speciesId = item->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 3);
		uint32 breedData = item->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 4);
		uint32 level = item->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 5);

		player->GetPetJournal()->Create(speciesId, breedData & 0xFF, (breedData >> 24) & 0xFF, level);
		player->DestroyItem(item->GetBagSlot(), item->GetSlot(), true);
	}
}
...
void Spell::EffectBattlePetsUnlock(SpellEffIndex effIndex)
{
	...
	if (player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_BATTLE_PET_ENABLED))
		return;

	player->SetFlag(PLAYER_FLAGS, PLAYER_FLAGS_BATTLE_PET_ENABLED);	
	...
	journal->UnlockLoadoutSlot(BATTLE_PET_LOADOUT_SLOT_1);
}

void Spell::EffectBattlePetHeal(SpellEffIndex effIndex)
{
	if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT)
		return;
	...
	for (uint8 i = 0; i < BATTLE_PET_MAX_LOADOUT_SLOTS; ++i)
	{
		...
		BattlePet::PetTemplate* proto = journal->GetBattlePet(id);
		if (!proto)
			continue;

		if (proto->GetCurrentHealth() == proto->GetMaxHealth())
			continue;

		proto->SetHp(proto->GetMaxHealth());
	}

	journal->SendPetsUpdateInLoadoutSlots(true);
}

void Spell::EffectBattlePetUpgrade(SpellEffIndex effIndex)
{
	if (effectHandleMode != SPELL_EFFECT_HANDLE_HIT)
		return;
	...
	auto summon = GetUnitTarget();
	if (!summon)
		return;

	Minion* minion = static_cast<Minion*>(summon->ToTempSummon());
	if (!minion)
		return;

	BattlePet::PetTemplate* proto = minion->GetBattlePetTemplate();
	if (!proto)
		return;

	if (!(GetSpellInfo()->Effects[effIndex].MiscValue & (1 << proto->GetFamily())))
		return;
	...
	{
	case 85:
		quality = ItemQualities::ITEM_QUALITY_RARE;
		break;
	case 75:
		quality = ItemQualities::ITEM_QUALITY_UNCOMMON;
		break;
	case 98:
		quality = ItemQualities::ITEM_QUALITY_EPIC;
		break;
	}
	...
	proto->SetQuality(quality);
	if (proto->GetLevel() >= 15)
	{
		proto->SetLevel(proto->GetLevel() - 2);
		summon->SetUInt32Value(UNIT_FIELD_WILD_BATTLE_PET_LEVEL, proto->GetLevel());
	}

	if (BattlePet::PetJournal* journal = player->GetPetJournal())
	{
		journal->SendBattlePetUpdate(proto, true);
	}
...
void Spell::EffectThreatAll(SpellEffIndex p_EffIndex)
{
    ...
    std::list<HostileReference*> ThreadList = m_caster->getThreatManager().getThreatList();
    for (HostileReference* itr : ThreadList)
    {
        Unit* target = itr->getTarget();

        target->getThreatManager().clearReferences();
        target->getThreatManager().doAddThreat(m_caster, m_spellInfo->Effects[p_EffIndex].BasePoints);
    }
}
