...
{
    Version = AppareanceData::GetCurrentVersion();
    ...
    if (Player* casterPlayer = unit->ToPlayer())
    {
        IsPlayer = 1;

        if (Guild* playerGuild = casterPlayer->GetGuild())
            GuildGuid = playerGuild->GetGUID();
        else
            GuildGuid = 0;

        FaceId = casterPlayer->GetByteValue(PLAYER_FIELD_BYTES, 1);
        FacialHair = casterPlayer->GetByteValue(PLAYER_BYTES_2, 0);
        HairColor = casterPlayer->GetByteValue(PLAYER_FIELD_BYTES, 3);
        HairStyle = casterPlayer->GetByteValue(PLAYER_FIELD_BYTES, 2);
        SkinColor = casterPlayer->GetByteValue(PLAYER_FIELD_BYTES, 0);

        for (uint32 l_It = EQUIPMENT_SLOT_START; l_It < EQUIPMENT_SLOT_END; ++l_It)
        {
            if (l_It == EQUIPMENT_SLOT_HEAD && casterPlayer->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_HELM))
                EquipmentDisplayId[l_It] = 0;
            else if (l_It == EQUIPMENT_SLOT_BACK && casterPlayer->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_CLOAK))
                EquipmentDisplayId[l_It] = 0;
            else if (Item const* item = casterPlayer->GetItemByPos(INVENTORY_SLOT_BAG_0, l_It))
                EquipmentDisplayId[l_It] = item->GetTemplate()->DisplayInfoID;
            else
                EquipmentDisplayId[l_It] = 0;
        }
    }
...
void Spell::SelectEffectImplicitTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32& processedEffectMask)
{
    ...
    switch (targetType.GetSelectionCategory())
    {
        ...
        case TARGET_SELECT_CATEGORY_AREA:
            ...
            break;
        case TARGET_SELECT_CATEGORY_PROYECTILE:
            SelectImplicitPoyectileTargets(effIndex, targetType, effectMask);
            break;
...
void Spell::SelectImplicitNearbyTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    switch (targetType.GetObjectType())
    {
        case TARGET_OBJECT_TYPE_AREATRIGGER:
            if (AreaTrigger* l_AreaTrigger = target->ToAreaTrigger())
                AddAreaTriggerTarget(l_AreaTrigger, effMask);
            break;
...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (uint32 containerTypeMask = GetSearcherTypeMask(objectType, condList))
    {
        ...
        if (!targets.empty())
        {
            ...
            for (std::list<AreaTrigger*>::iterator itr = l_AreaTriggerTargets.begin(); itr != l_AreaTriggerTargets.end(); ++itr)
                AddAreaTriggerTarget(*itr, effMask);
        }
    }
}
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
                ...
                if (m_spellInfo->Id == 114654 || m_spellInfo->Id == 108685)
                {
                    for (std::list<Unit*>::iterator itr = unitTargets.begin(); itr != unitTargets.end();++itr)
                    {
                        if (IsCritForTarget((*itr)))
                            m_caster->SetPower(POWER_BURNING_EMBERS, m_caster->GetPower(POWER_BURNING_EMBERS) + 2, true);
                        else
                            m_caster->SetPower(POWER_BURNING_EMBERS, m_caster->GetPower(POWER_BURNING_EMBERS) + 1, true);
    ...
    if (!l_AreaTriggerTargets.empty())
    {
        if (uint32 l_MaxTargets = m_spellValue->MaxAffectedTargets)
            WoWSource::Containers::RandomResizeList(l_AreaTriggerTargets, l_MaxTargets);
...
void Spell::SelectImplicitPoyectileTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    Unit* referer = m_caster;
    Unit* originalCaster = m_originalCaster;

    if (!referer || !originalCaster)
        return;

    std::list<Unit*> targetList;
    uint32 maxAffectedTargets = m_spellValue->MaxAffectedTargets;
    float radius = m_spellInfo->Effects[effIndex].CalcRadius(m_caster) * m_spellValue->RadiusMod;
    bool positiveEffect = m_spellInfo->_IsPositiveEffect(effIndex, true);
    bool neutralEffect = m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_DUMMY;
    Unit* exclusion = NULL;

    if (m_spellInfo->AttributesEx & SPELL_ATTR1_CANT_TARGET_SELF)
        exclusion = referer;

    if (m_spellInfo->AttributesEx5 & SPELL_ATTR5_SINGLE_TARGET_SPELL)
        maxAffectedTargets = 1;

    float x, y;

    referer->GetNearPoint2D(x, y, radius, referer->GetOrientation());

    referer->FilterTargets(targetList, radius, INCLUDE_CRITTERS, maxAffectedTargets, exclusion, DistanceOrderPred);

    if (Creature* pointOfReference = referer->SummonCreature(WORLD_TRIGGER, x, y, referer->GetPositionZ(), 0.f, TEMPSUMMON_TIMED_DESPAWN, 100))
    {
        for (auto itr : targetList)
        {
            if (!itr->IsInBetween(referer, pointOfReference, 4.f))
                continue;

            if (itr->GetPositionZ() > referer->GetPositionZ() + 4.f || itr->GetPositionZ() < referer->GetPositionZ() - 4.f)
                continue;

            if (!(m_spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS) && !referer->IsWithinLOSInMap(itr))
                continue;

            if (m_spellInfo->AttributesEx & SPELL_ATTR1_CANT_TARGET_IN_COMBAT && itr->isInCombat())
                continue;

            if (m_spellInfo->AttributesEx3 & SPELL_ATTR3_ONLY_TARGET_PLAYERS && itr->GetTypeId() != TYPEID_PLAYER)
                continue;

            if (m_spellInfo->AttributesEx6 & SPELL_ATTR6_CANT_TARGET_CROWD_CONTROLLED && itr->HasCrowdControlAura())
                continue;

            if (!neutralEffect && positiveEffect && !originalCaster->IsValidAssistTarget(itr))
                continue;

            if (!neutralEffect && !positiveEffect && !originalCaster->IsValidAttackTarget(itr))
                continue;

            if (neutralEffect && !originalCaster->IsValidAssistTarget(itr) && !originalCaster->IsValidAttackTarget(itr))
                continue;
...
void Spell::SelectImplicitCasterObjectTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        case TARGET_UNIT_CASTER:
            target = m_caster;
            if (m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AURA_ON_PET)
                target = m_caster->GetGuardianPet();
            checkIfValid = m_spellInfo->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AURA_ON_PET;
            break;
...
void Spell::SelectImplicitTargetObjectTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    if (target)
    {
        ...
        else if (AreaTrigger* l_AreaTrigger = target->ToAreaTrigger())
            AddAreaTriggerTarget(l_AreaTrigger, 1 << effIndex);
...
void Spell::SelectEffectTypeImplicitTargets(uint8 effIndex)
{
    ...
    if (target)
    {
        ...
        else if (target->ToAreaTrigger())
            AddAreaTriggerTarget(target->ToAreaTrigger(), 1 << effIndex);
    }
}
...
{
    for (uint32 l_EffectIndex = 0; l_EffectIndex < MAX_EFFECTS; ++l_EffectIndex)
    {
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
	try
	{
		...
		if (!unit)
		{
			...
			for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
				if (farMask & (1 << i))
					HandleEffects(unit, NULL, NULL, NULL, i, SPELL_EFFECT_HANDLE_HIT_TARGET);
			return;
		}
		...
		// Handler for Maelstrom Weapon with Ancestral Swiftness
		if ((m_spellInfo->CastTimeEntry && m_spellInfo->CastTimeEntry->CastTime > 0) && m_caster && m_spellInfo && !m_spellInfo->IsChanneled() && !m_caster->HasAura(16188) && (m_spellInfo->SchoolMask == SPELL_SCHOOL_MASK_NATURE || GetSpellInfo()->Id == 117014) && m_caster->HasAura(53817))
			m_caster->RemoveAura(53817);
		...
		if (m_healing > 0)
		{
			...
			addhealth = caster->CapForPVP(caster->getVictim(), addhealth, false, m_spellInfo);
		...
		else if (m_damage > 0)
		{
			...
			// Limit Damage, for < 90 levels
			damageInfo.damage = caster->CapForPVP(damageInfo.target, damageInfo.damage, true, m_spellInfo);
	...
	{
		sLog->outInfo(LOG_FILTER_WORLDSERVER, "Exception catch at Spell::DoAllEffectOnTarget crash info: %s", e.what());
	}
	catch (...)
	{
		sLog->outInfo(LOG_FILTER_WORLDSERVER, "Unknwon exception catch at Spell::DoAllEffectOnTarget");
	}
}
...
SpellMissInfo Spell::DoSpellHitOnUnit(Unit* unit, uint32 effectMask, bool scaleAura)
{
    ...
    // Hack fix for Cloak of Shadows (just Blood Plague & Positive Spells can hit to Cloak of Shadows)
    if ((m_spellInfo->GetSchoolMask() & SPELL_SCHOOL_MASK_MAGIC) && m_spellInfo->Id != 59879 && !m_spellInfo->IsPositive() && unit->HasAura(31224))
        return SPELL_MISS_MISS;
    ...
    if (aura_effmask)
    {
        ...
        if (m_originalCaster)
        {
            ...
            m_spellAura = Aura::TryRefreshStackOrCreate(aurSpellInfo, effectMask, unit,
                m_originalCaster, m_spellPowerData, (aurSpellInfo == m_spellInfo) ? &m_spellValue->EffectBasePoints[0] : &basePoints[0], m_CastItem, 0, &refresh, m_castItemLevel);
            if (m_spellAura)
            {
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS; ++effectNumber)
        if (effectMask & (1 << effectNumber))
            HandleEffects(unit, NULL, NULL, NULL, effectNumber, SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoAllEffectOnTarget(GOTargetInfo* target)
{
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS; ++effectNumber)
        if (effectMask & (1 << effectNumber))
            HandleEffects(NULL, NULL, go, NULL, effectNumber, SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoAllEffectOnTarget(ItemTargetInfo* target)
{
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_SPELL_EFFECTS; ++effectNumber)
        if (effectMask & (1 << effectNumber))
            HandleEffects(NULL, target->item, NULL, NULL, effectNumber, SPELL_EFFECT_HANDLE_HIT_TARGET);
...
void Spell::DoAllEffectOnTarget(AreaTriggerTargetInfo* p_Target)
{
    ...
    for (uint32 effectNumber = 0; effectNumber < MAX_EFFECTS; ++effectNumber)
    {
        if (effectMask & (1 << effectNumber))
...
void Spell::cast(bool skipCheck)
{
    ...
    // Handler for Pyroblast with Presence of Mind
    if ((m_spellInfo->CastTimeEntry && m_spellInfo->CastTimeEntry->CastTime > 0 || GetSpellInfo()->Id == 140376) && m_caster && m_spellInfo && !m_spellInfo->IsChanneled() && (!m_caster->HasAura(48108) || GetSpellInfo()->Id != 11366) && m_caster->HasAura(12043))
        m_caster->RemoveAura(12043);

	// Handler for Soul of the Forest
	if (m_caster && (m_spellInfo->CastTimeEntry && m_spellInfo->CastTimeEntry->CastTime > 0) || (m_spellInfo && m_spellInfo->IsChanneled()) && m_caster->HasAura(114108))
		m_caster->RemoveAura(114108);
...
void Spell::_handle_immediate_phase()
{
    ...
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
    {
        ...
        HandleEffects(NULL, NULL, NULL, NULL, j, SPELL_EFFECT_HANDLE_HIT);
    }
...
void Spell::HandleEffects(Unit* pUnitTarget, Item* pItemTarget, GameObject* pGOTarget, AreaTrigger* p_AreaTrigger, uint32 i, SpellEffectHandleMode mode)
{
        ...
        switch (m_spellInfo->Effects[i].Effect)
        {
            ...
            case SPELL_EFFECT_DUMMY:
            {
                    ...
                    m_spellInfo->Id == 122282)
                {
                    ...
                    if (!target ||  (target == m_caster && !target->HasAura(49039)) || (!m_caster->HasAura(63333) && (target->IsFriendlyTo(m_caster) && target->GetCreatureType() != CREATURE_TYPE_UNDEAD)))
                        return SPELL_FAILED_BAD_TARGETS;

                    if (target != m_caster && m_caster->HasAura(63333) && (target->IsFriendlyTo(m_caster))) // Glyph of Death Coil
                        return SPELL_CAST_OK;
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                if (m_caster->IsInDisallowedMountForm() && !m_caster->HasAura(114338)) //Glyph of the Stag
...
SpellCastResult Spell::CheckCasterAuras() const
{
    ...
    bool usableInSleep = m_spellInfo->AttributesCu & SPELL_ATTR0_CU_USABLE_WHILE_SLEEPED;
    ...
    if (unitflag & UNIT_FLAG_STUNNED)
    {
        if (usableInStun || usableInSleep)
        {
            ...
            if (usableInStun)
            {
                ...
                if (foundNotStun && m_spellInfo->Id != 22812 && m_spellInfo->Id != 108482)
                    prevented_reason = SPELL_FAILED_STUNNED;
            }

            // spell is usable while sleeped, check if caster has only mechanic sleep auras
            if (usableInSleep)
            {
                bool foundNotSleep = false;
                Unit::AuraEffectList const& stunAuras = m_caster->GetAuraEffectsByType(SPELL_AURA_MOD_STUN);
                for (Unit::AuraEffectList::const_iterator i = stunAuras.begin(); i != stunAuras.end(); ++i)
                {
                    if ((*i)->GetSpellInfo()->GetAllEffectsMechanicMask() && !((*i)->GetSpellInfo()->GetAllEffectsMechanicMask() & (1 << MECHANIC_SLEEP)))
                    {
                        foundNotSleep = true;
                        break;
                    }
                ...
                if (foundNotSleep)
                    prevented_reason = SPELL_FAILED_CANT_DO_THAT_RIGHT_NOW;
            }
        }
        else
            prevented_reason = SPELL_FAILED_STUNNED;
                    ...
                    if (AuraEffectPtr part = aura->GetEffect(i))
                    {
                        switch (part->GetAuraType())
                        {
                            case SPELL_AURA_MOD_STUN:
                                if (!(usableInStun && usableInSleep) || !(auraInfo->GetAllEffectsMechanicMask() & ((1<<MECHANIC_STUN) | (1<<MECHANIC_SLEEP))))
...
SpellCastResult Spell::CheckPower()
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        {
            if (m_powerCost != 0 && m_caster->HasAura(118864))
            {
                m_powerCost = 0;
                m_caster->RemoveAuraFromStack(118864);
            }
        ...
        {
            if (m_powerCost != 0 && m_caster->HasAura(116768))
            {
                m_powerCost = 0;
                m_caster->RemoveAuraFromStack(116768);
            }
        ...
        {
            if (m_powerCost != 0 && m_caster->HasAura(81340))
            {
                m_powerCost = 0;
                m_caster->RemoveAuraFromStack(81340);
            }
        ...
        case 130552:// World of Glory (damage)
        case 53385: // Divine Storm
        case 85256: // Templar's Verdict
        {
            if (m_powerCost != 0 && m_caster->HasAura(90174))
            {
                m_powerCost = 0;
                m_caster->RemoveAuraFromStack(90174);
            }
             break;
        }
        default:
...
{
	return m_spellInfo->Attributes & (SPELL_ATTR0_ON_NEXT_SWING | SPELL_ATTR0_ON_NEXT_SWING_2);
    return m_spellInfo->Attributes & (SPELL_ATTR0_ON_NEXT_SWING | SPELL_ATTR0_ON_NEXT_SWING_2);
}
...
void Spell::HandleLaunchPhase()
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        ...
        HandleEffects(NULL, NULL, NULL, NULL, i, SPELL_EFFECT_HANDLE_LAUNCH);
    }
...
void Spell::DoAllEffectOnLaunchTarget(TargetInfo& targetInfo, float* multiplier)
{
    ...
    for (uint32 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        if (targetInfo.effectMask & (1<<i))
        {
            ...
            HandleEffects(unit, NULL, NULL, NULL, i, SPELL_EFFECT_HANDLE_LAUNCH_TARGET);
...
void Spell::CallScriptPrepareCastHandlers() {
	uint32 scriptExecuteTime = getMSTime();
	for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
	{
		(*scritr)->_PrepareScriptCall(SPELL_SCRIPT_HOOK_PREPARE_CAST);
		std::list<SpellScript::CastHandler>::iterator hookItrEnd = (*scritr)->OnPrepareCast.end(), hookItr = (*scritr)->OnPrepareCast.begin();
		for (; hookItr != hookItrEnd; ++hookItr)
			(*hookItr).Call(*scritr);
...
void Spell::CallScriptOnHitHandlers()
{
    ...
    for (std::list<SpellScript*>::iterator scritr = m_loadedScripts.begin(); scritr != m_loadedScripts.end(); ++scritr)
    {
		...
		for (; hookItr != hookItrEnd; ++hookItr){	
			auto value = (*scritr)->_GetScriptName();			
			(*hookItr).Call(*scritr);			
		}  
        (*scritr)->_FinishScriptCall();
    }
