...
struct SpellGroupStack
{
    SpellGroupStackRule stackRule;
    uint32 auraType;
};

#define SPELL_GROUP_STACK_RULE_MAX 4

typedef std::map<SpellGroup, SpellGroupStack> SpellGroupStackMap;
...
enum BonusLootSources : int
{
    BONUS_LOOT_SOURCE_NONE = 0,
    BONUS_LOOT_SOURCE_CREATURE = 1,
    BONUS_LOOT_SOURCE_GAMEOBJECT = 2,
    BONUS_LOOT_SOURCE_MAX,
};
...
struct BonusLootData
{
    ...
    BonusLootSources lootSourceType;
    uint32 lootSourceEntry;
};

typedef std::map<uint32 /*creatureEntry*/, BonusLootData> BonusLootSpellsMap;
...
class SpellMgr
{
    ...
    public:
        ...
        bool AddSameEffectStackRuleSpellGroups(SpellInfo const* spellInfo, int32 amount, std::map<SpellGroup, int32>& groups, AuraType auraType = SPELL_AURA_NONE) const;
        ...
        BonusLootData const* GetSpellBonusLoot(uint32 creatureEntry) { return mBonusLootSpells.find(creatureEntry) != mBonusLootSpells.end() ? &mBonusLootSpells[creatureEntry] : NULL; }
    ...
    private:
        ...
        BonusLootSpellsMap         mBonusLootSpells;
};
