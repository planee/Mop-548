...
struct AppareanceData
{
    // /!\ Do not put any non-numeric type in this struct (it's stored in DB as-is) /!\ 
    // And no pointer of course
    uint8 IsPlayer; // Boolean (stored as uint8 to guarantee its size)
    uint8 Class;
    uint8 FaceId;
    uint8 FacialHair;
    uint8 HairStyle;
    ...
    uint8 SkinColor;
    uint32 DisplayId;
    uint32 EquipmentDisplayId[EQUIPMENT_SLOT_END];
    uint32 Version;
    uint64 GuildGuid;
    ...
    static uint32 GetCurrentVersion() { return 1; } // Increment this everytime you change this structure
};

static_assert(std::is_trivial<AppareanceData>::value, "This struct is stored as-is in database, it should stay trivial (no std::list, std::string, etc., only primitive types)");
...
class Spell
{
    ...
    public:
		...
		bool IsAlchemyAvailable(uint32 spellId, uint8 alchemySpec);
        ...
        void EffectLootBonus(SpellEffIndex effIndex);
		...
		void EffectBattlePetUnCage(SpellEffIndex effIndex);
		void EffectBattlePetHeal(SpellEffIndex effIndex);
		void EffectBattlePetUpgrade(SpellEffIndex effIndex);
        ...
        void SelectImplicitPoyectileTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask);
        ...
        int32 CalculateDamage(uint8 i, Unit const* target) const { return m_caster->CalculateSpellDamage(target, m_spellInfo, i, &m_spellValue->EffectBasePoints[i], m_castItemLevel); }
        ...
        void HandleEffects(Unit* pUnitTarget, Item* pItemTarget, GameObject* pGOTarget, AreaTrigger* p_AreaTrigger, uint32 i, SpellEffectHandleMode mode);
