...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //350 SPELL_AURA_MOD_GATHERING_ITEMS_GAINED_PERCENT implemented in Loot::AddItem
    ...
    &AuraEffect::HandleNULL,                                      //416 SPELL_AURA_MOD_COOLDOWN_BY_MELEE_HASTE
    &AuraEffect::HandleNULL,                                      //417 SPELL_AURA_MOD_GLOBAL_COOLDOWN_BY_MELEE_HASTE
    ...
    switch (GetAuraType())
    {
        ...
        case SPELL_AURA_MOD_INCREASE_HEALTH_PERCENT:
        {
            switch (GetId())
            {
                case 120954: // Fortifying Brew.
                {
					...
					if (caster->HasAura(124997) && caster)
                        amount = 10;
                    break;
                }
        ...
        case SPELL_AURA_OBS_MOD_HEALTH:
        {
            switch (GetId())
            {
                case 6262:  // Healthstone
					if (!caster->HasAura(56224) && caster) // Glyph of Healthstone
                        amount = 0;
                    break;
        ...
        case SPELL_AURA_MOD_INCREASE_SWIM_SPEED:
        {
            switch (GetId())
            {
			    ...
			    if (caster)
			        if (caster->HasAura(57856)) //Glyph of Aquatic Form
					{
						amount = 147;
					}
					else
				...
				case 102416: //Wild charge in Aquatic Form
				{
			     if (!caster)
				  break;
        ...
        case SPELL_AURA_ADD_PCT_MODIFIER:
            ...
            switch (GetId())
            {
                ...
                case 114232:// Sanctified Wrath
                {
                    ...
                    // Sanctity of Battle
                    float reducePct = (100 - _player->GetMeleeHastePct(false)) / 100.0f;
...
bool AuraEffect::IsAffectingSpell(SpellInfo const* spell) const
{
	...
	{
		sLog->outInfo(LOG_FILTER_WORLDSERVER, "bool AuraEffect::IsAffectingSpell");
		return false;
	}
}
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    switch (GetMiscValue())
    {
        case FORM_CAT:
			...
			target->RemoveAura(114282); //Treant Form
            break;
        case FORM_TRAVEL:
            spellId = 5419;     
			target->RemoveAura(114282); //Treant Form
        ...
        case FORM_FLIGHT_EPIC:
			...
			break;
		case FORM_TREE:
			target->RemoveAura(24858); //Moonkin Form
			break;
...
void AuraEffect::HandleModInvisibility(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply && (mode & AURA_EFFECT_HANDLE_REAL))
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
			target->ToPlayer()->HandleBGDropEvent();
		}
    }
    ...
    if (apply)
    {
        ...
        switch (form)
        {
            ...
            case FORM_MOONKIN:
            {
				...
				// Remove movement impairing effects when shifting in.
...
void AuraEffect::HandleAuraModSchoolImmunity(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    // call this method to correctly handle battleground thinks like remove obrs, flags etc.
    //81549 - Dont call this when current spell is Cloack of Shadow (81549)
    if ((target->GetTypeId() == TYPEID_PLAYER) && !(GetId() == 81549))
    {
        target->ToPlayer()->HandleBGDropEvent();
    }
...
void AuraEffect::HandleModTotalPercentStat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 spellGroupVal = target->GetHighestExclusiveSameEffectSpellGroupValue(shared_from_this(), SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE, NULL, GetMiscValueB());
...
void AuraEffect::HandleAuraModIncreaseHealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    {
        // From 5.2 patch, Nature's Guardian now preserves the player's health percentage when its maximum health boosting effect expires.
        ...
        else if (int32(target->GetHealth()) > GetAmount())
            target->ModifyHealth(-GetAmount());
        else
            target->SetHealth(1);
...
void AuraEffect::HandleModCastingSpeed(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (target->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        target->ToPlayer()->UpdateSpellHaste();
    }
...
void AuraEffect::HandleModMeleeRangedSpeedPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 spellGroupVal = target->GetHighestExclusiveSameEffectSpellGroupValue(shared_from_this(), SPELL_AURA_MOD_MELEE_RANGED_HASTE_2, GetMiscValue());
    if (abs(spellGroupVal) >= abs(GetAmount()))
        return;
...
void AuraEffect::HandleModRating(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    int32 spellGroupVal = target->GetHighestExclusiveSameEffectSpellGroupValue(shared_from_this(), SPELL_AURA_MOD_RATING, GetMiscValue());
    if (abs(spellGroupVal) >= abs(GetAmount()))
        return;
...
void AuraEffect::HandleIncreaseHasteFromItemsByPct(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    target->ToPlayer()->UpdateRangeHaste();
    target->ToPlayer()->UpdateSpellHaste();
    target->ToPlayer()->UpdateMeleeHaste();
}
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (GetSpellInfo()->Id)
            {
                ...
                case 51690:
                {
					...
					Unit* spellTarget = sObjectAccessor->GetUnit(*target, WoWSource::Containers::SelectRandomContainerElement(validTargets));
...
void AuraEffect::HandlePeriodicDamageAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (GetAuraType() == SPELL_AURA_PERIODIC_DAMAGE)
    {
        ...
        if (GetSpellInfo()->Id == 88954)
        {
            uint32 dmg = urand(3900, 4100);
            damage += (dmg * m_tickNumber);
        }

        //Wrack (Sinestra)
        if (GetSpellInfo()->Id == 89421)
            damage += (1500 * m_tickNumber);

        //Electrocute (Alakir)
        if (GetSpellInfo()->Id == 88427)
        {
            uint32 dmg = urand(103950, 106050);
            damage += dmg / (11 - (m_tickNumber / 2));
        }

        // Corrupting Parasite Dmg (Madness of Deathwing)   
        if (GetSpellInfo()->Id == 108601)
        {
        ...
        {
			damage = (damage) / 10; // Prevent insane damage with 10 stacks  
        }
        // Malefic Grasp
	...
	//Cap damage para PVP con nivel menor de 80
	try{
		Unit* victimCAP = caster->getVictim();
		int32 totalTick = GetTotalTicks();
		uint32 totalDamage = totalTick > 0 ? (damage * totalTick) : damage;
		uint32 capdamage = totalTick > 0 ? ((caster->CapForPVP(victimCAP, totalDamage, true, m_spellInfo)) / totalTick) : (caster->CapForPVP(victimCAP, totalDamage, true, m_spellInfo));		
		damage = capdamage;
	}
	catch (std::exception& e)
	{
		printf("Exception ""%s"" when calling CapForPVP in Damage Tick", e.what());
	}
...
void AuraEffect::HandlePeriodicHealAurasTick(Unit* target, Unit* caster) const
{
    ...
    if (m_spellInfo->Id == 16491)
    {
        // PVP Power Only Works On PVP Combats
	...
	//Cap Heal para PVP con nivel menor de 80
	try{
		Unit* victimCAP = caster->getVictim();
		int32 totalTick = GetTotalTicks();
		uint32 totalHeal = totalTick > 0 ? (heal * totalTick) : heal;
		uint32 capheal = totalTick > 0 ? ((caster->CapForPVP(victimCAP, totalHeal, false, m_spellInfo)) / totalTick) : (caster->CapForPVP(victimCAP, totalHeal, false, m_spellInfo));		
		heal = capheal;
	}
	catch (std::exception& e)
	{
...
bool AuraEffect::HasSpellClassMaskSafe() const
{
	try
	{
		return m_spellInfo->Effects[m_effIndex].SpellClassMask != flag128(0) ? true : false;
	}
	catch (...)
	{
		sLog->outInfo(LOG_FILTER_WORLDSERVER, "Unknwon exception catch at void Unit::HasSpellClassMaskSafe");
		return false;
	}
}
