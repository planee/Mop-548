...
{
    return GetSelectionCategory() == TARGET_SELECT_CATEGORY_AREA || GetSelectionCategory() == TARGET_SELECT_CATEGORY_CONE || GetSelectionCategory() == TARGET_SELECT_CATEGORY_PROYECTILE;
}
...
SpellImplicitTargetInfo::StaticData  SpellImplicitTargetInfo::_data[TOTAL_SPELL_TARGETS] =
{
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_PROYECTILE, TARGET_CHECK_DEFAULT, TARGET_DIR_FRONT},     // 62 TARGET_UNIT_PROYECTILE
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_CONE,    TARGET_CHECK_ENEMY,    TARGET_DIR_FRONT },      // 130 TARGET_UNIT_CONE_ENEMY_130
};
...
SpellEffectInfo::SpellEffectInfo(SpellEntry const* spellEntry, SpellInfo const* spellInfo, uint8 effIndex, uint32 difficulty)
{
    ...
    if (itr != sSpellEnchantScalingData.end())
    {
        ...
        for (uint8 j = 0; j < MAX_ITEM_ENCHANTMENT_EFFECTS; ++j)
        {
            if (ench->spellid[j] != spellInfo->Id)
                continue;

            ScalingMultiplier = ench->ScalingPoints[j];
            break;
}
    }
}
...
bool SpellEffectInfo::IsPersistentAreaAura() const
{
...
int32 SpellEffectInfo::CalcValue(Unit const* caster, int32 const* bp, Unit const* target, Item const* itemTarget, int32 itemLevel /*= -1*/) const
{
    ...
    if (ScalingMultiplier != 0.0f)
    {
        if (caster && !_spellInfo->IsCustomCalculated())
        {
            ...
            if (target && _spellInfo->IsPositiveEffect(_effIndex) && (Effect == SPELL_EFFECT_APPLY_AURA) && _spellInfo->Id != 774) // Hack Fix Rejuvenation, doesn't use the target level for basepoints
                ...
                if (!_spellInfo->HasAttribute(SPELL_ATTR11_CAST_WITH_ITEM) && _spellInfo->HasAttribute(SPELL_ATTR10_UNK12))
                    level = _spellInfo->BaseLevel;
                if (level == 0 || _spellInfo->MaxScalingLevel && level > _spellInfo->MaxScalingLevel)
                    level = _spellInfo->MaxScalingLevel;

                if (GtSpellScalingEntry const* gtScaling = sGtSpellScalingStore.LookupEntry(((_spellInfo->ScalingClass > 0 ? _spellInfo->ScalingClass : ((MAX_CLASSES - 1) - _spellInfo->ScalingClass)) - 1) * 100 + (level - 1)))
            {
                    float multiplier = 0.0f;

					if (Effect != SPELL_EFFECT_ENCHANT_ITEM)
					{
						if (!_spellInfo->ScalesFromItemLevel)
						{
							if (!_spellInfo->HasAttribute(SPELL_ATTR11_CAST_WITH_ITEM))
							{
								if (GtSpellScalingEntry const* gtScaling = sGtSpellScalingStore.LookupEntry(((_spellInfo->ScalingClass > 0 ? _spellInfo->ScalingClass : ((MAX_CLASSES - 1) - _spellInfo->ScalingClass)) - 1) * 100 + (level - 1)))
									multiplier = gtScaling->value;
							}
							else
								multiplier = GetRandomPropertyPoints(itemLevel != -1 ? itemLevel : 1, ITEM_QUALITY_RARE, INVTYPE_CHEST, 0);
						}
						else
							multiplier = GetRandomPropertyPoints(_spellInfo->ScalesFromItemLevel, ITEM_QUALITY_RARE, INVTYPE_CHEST, 0);
					}
                ...
                if (DeltaScalingMultiplier)
                {
                    ...
                    if (preciseBasePoints != 0.0f && std::abs(preciseBasePoints) < 1.0f)
                        preciseBasePoints = std::copysign(1.0f, preciseBasePoints);
                    else
                        preciseBasePoints = std::round(preciseBasePoints);
            ...
            else // Hack fix for 2hand weapon pvp power scaling with ilvl
            {
                uint32 mod = 0;
                for (uint32 position = EQUIPMENT_SLOT_MAINHAND; position <= EQUIPMENT_SLOT_OFFHAND; ++position)
                {
                    if (auto weapon = caster->ToPlayer()->GetItemByPos(INVENTORY_SLOT_BAG_0, position))
                    {
                        if (!weapon->IsEquipped())
                            continue;

                        if (auto proto = weapon->GetTemplate())
                        {
                            for (uint8 z = 0; z < MAX_ITEM_PROTO_SPELLS; ++z)
                            {
                                if (proto->Spells[z].SpellId == _spellInfo->Id)
                                {
                                    auto ilvl = proto->ItemLevel;
                                    ItemUpgradeEntry const* itemUpgrade = sItemUpgradeStore.LookupEntry(weapon->GetDynamicUInt32Value(ITEM_DYNAMIC_MODIFIERS, 2));
                                    if (itemUpgrade && itemUpgrade->itemLevelUpgrade)
                                        ilvl += itemUpgrade->itemLevelUpgrade;
    ...
    // Hack fix for all incorrect bonuses
    value = _spellInfo->GetBonusValueForOldBonus(value);

    if (itemTarget && caster && caster->IsPlayer())
    {
        ...
        if (Player->GetMap() && Player->GetMap()->IsBattlegroundOrArena())
        {
            ...
            {
                int32 reduction = itemLevel >= 502 && !itemTarget->IsPvPItem() ? 50 : 0;
                if (_spellInfo->HasCustomAttribute(SPELL_ATTR0_CU_REDUCE_33))
                    reduction = 33;
                else if (_spellInfo->HasCustomAttribute(SPELL_ATTR0_CU_REDUCE_75))
                    reduction = 75;
    // Hack fix for Inner Brilliance
                if (reduction)
                    value -= CalculatePct(value, reduction);
            }
        }
    }

    // Hack fix for 522 pve trinkets
    if (_spellInfo->Id == 108300 && caster)
        value = caster->GetStat(STAT_INTELLECT);
    ...
    if (caster)
    {
        ...
        if (!_spellInfo->GetSpellScaling() && !basePointsPerLevel && (_spellInfo->Attributes & SPELL_ATTR0_LEVEL_DAMAGE_CALCULATION) && _spellInfo->SpellLevel && _spellInfo->SpellLevel != caster->getLevel())
        {
            ...
            if (canEffectScale && (caster->GetMapId() == 870 || caster->GetMapId() == 1007) && Effect == SPELL_EFFECT_SCHOOL_DAMAGE)
                value *= 1.25f * exp(caster->getLevel() * (70 - _spellInfo->SpellLevel) / 1000.0f); // ToDo: Fix this.            
            else if (canEffectScale)
                value *= 0.25f * exp(caster->getLevel() * (70 - _spellInfo->SpellLevel) / 1000.0f); // ToDo: Fix this.     
        }
...
SpellEffectInfo::StaticData  SpellEffectInfo::_data[TOTAL_SPELL_EFFECTS] =
{
    ...
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 187 SPELL_EFFECT_RANDOM_DIGSITE
    ...
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_DEST}, // 198 SPELL_EFFECT_PLAY_SCENE
    ...
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 201 SPELL_EFFECT_BATTLE_PET_UNLOCK
    ...
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 204 SPELL_EFFECT_UPGRADE_BATTLEPET
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 205 SPELL_EFFECT_PVE_PVP_CHOICE
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 206 SPELL_EFFECT_ETERNITY_EMBRACE
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 207 SPELL_EFFECT_207
    {EFFECT_IMPLICIT_TARGET_EXPLICIT, TARGET_OBJECT_TYPE_UNIT}, // 208 SPELL_EFFECT_TEST_REPUTATION
};
...
{
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        if (Effects[i].IsPersistentAreaAura())
            return true;
    return false;
}
...
void SpellInfo::SetTriggeredByHavoc(bool const value) const{	
	m_havoctrigged->isTrigged = value;
}
...
{
    return IsPassive() && !HasEffect(SPELL_EFFECT_APPLY_AURA) && !HasEffect(SPELL_EFFECT_APPLY_AURA_ON_PET);
}
...
//void SpellInfo::SetTriggeredByHavoc(bool value) const
//{ 
//	bool const pass = value;
//	SpellInfo::m_havoctrigged = 10;
//}
...
SpellCastResult SpellInfo::CheckLocation(uint32 map_id, uint32 zone_id, uint32 area_id, Player* player) const
{
    ...
    switch (Id)
    {
        ...
        case 106213: // Blood of Neltharion, Spine of Deathwing, Dragon Soul
            return (map_id == 967 && (area_id == 5928 || area_id == 5960)) ? SPELL_CAST_OK : SPELL_FAILED_REQUIRES_AREA;
        case 105896:
        case 105900:
        case 105903:
        case 105984:
...
SpellCastResult SpellInfo::CheckTarget(Unit const* caster, WorldObject const* target, bool implicit) const
{
    ...
    // Custom MoP Script - Glyph of Guided Stars
    if (unitTarget && Id == 50286 && !unitTarget->HasAura(8921) && !unitTarget->HasAura(93402) && caster->HasAura(146655))
        return SPELL_FAILED_BAD_TARGETS;

     // Custom MoP Script - Hack fix for Halo (Holy), Halo (Shadow), Piercing Howl, Multi-Shot, Psychic Terror, Earthgrab Totem - it doesn't break stealth.
     if ((Id == 120517 || Id == 120644 || Id == 12323 || Id == 2643 || Id == 113792 || Id == 3600 || Id == 64695) && (unitTarget->HasAuraType(SPELL_AURA_MOD_STEALTH) || HasAura(SPELL_AURA_MOD_INVISIBILITY)) && unitTarget)
         return SPELL_FAILED_BAD_TARGETS;
        ...
        if (neededTargets & (TARGET_FLAG_UNIT_ENEMY | TARGET_FLAG_UNIT_ALLY | TARGET_FLAG_UNIT_RAID | TARGET_FLAG_UNIT_PARTY | TARGET_FLAG_UNIT_MINIPET | TARGET_FLAG_UNIT_PASSENGER))
        {
            if (neededTargets & TARGET_FLAG_UNIT_ENEMY)
                 if (caster->_IsValidAttackTarget(unitTarget, this) ||
                    // Custom exception for Wrath spell treant from Force of Nature spell
                    Id == 113769)
                    return SPELL_CAST_OK;
...
SpellCastResult SpellInfo::CheckVehicle(Unit const* caster) const
{
    ...
    if (vehicle)
    {
        ...
        if (!(AttributesEx6 & SPELL_ATTR6_CASTABLE_WHILE_ON_VEHICLE) && !(Attributes & SPELL_ATTR0_CASTABLE_WHILE_MOUNTED)
            && (vehicleSeat && vehicleSeat->m_flags & checkMask) != checkMask)
...
uint32 SpellInfo::CalcCastTime(Unit* caster, Spell* spell) const
{
	...
	*/
    ...
    //Trinket Kiroptyric Sigil 
    if (Id == 107947 && caster)
        castTime = 0;

    //Trinket Steadfast Talisman of the Shado-Pan Assault 
    if (Id == 138728 && caster)
        castTime = 0;
    ...
    switch (SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch (Id)
            {
				...
				case 75322: // Reverberating Hymn (Temple Guardian Anhuur)
		...
		case SPELLFAMILY_WARRIOR:
			// Bladestorm
			if (SpellFamilyFlags[1] == 0x00004000)
				return true;
			// Mortal Strike
			if (SpellFamilyFlags[0] == 0x02000000)
				return true;
			break;
        case SPELLFAMILY_PRIEST:
            switch (Id)
		...
		case SPELLFAMILY_DRUID:
			// Starfall
			if (SpellFamilyFlags[1] == 0x00800000)
				return true;
            // Dispersion (Symbiosis)
            if (Id == 110715)
                return true;
			break;
        case SPELLFAMILY_HUNTER:
		...
		case SPELLFAMILY_DEATHKNIGHT:
			// Unholy Blight
			if (SpellFamilyFlags[1] == 0x00000100)
				return true;
			// Unholy Frenzy
		...
		case SPELLFAMILY_MONK:
            // Rushing Jade Wind
            if (SpellFamilyFlags[0] == 0x00000020)
                return true;
			// Spinning Crane Kick
...
float SpellInfo::GetGiftOfTheSerpentScaling(Unit* caster) const
{
    ...
    switch (Id)
    {
        case 135914:// Healing Sphere (Detonation)
        case 135920:// Healing Sphere From Gift of the Serpent (Detonation)
            scale = 0.0f;
            break;
...
float SpellInfo::GetBonusValueForOldBonus(float defaultValue) const
{
    switch (Id)
    {
    case 138724:
    ...
    case 144205:  // Typhoon
        return 765.0f;
    case 126697:  // 550 trinkets for hp
        return 95875.0f;
    case 92174:   // Item - Proc Mastery Rating
        return 1710.0f;
    case 64763:   // Heart of Iron
        return 432.0f;
    case 11351:   // Oil of Immolation
        return 31.0f;
    case 126577:  // Inner Brilliance
        return 3653.0f;
    case 127230:  //  Visions of Insanity
        return 500.0f;
        // 522 pve trinkets
    case 138703:  // Acceleration
    case 138702:  // Surge of Strength
    case 138699:  // Superluminal
        return 8800.0f;
    case 105698:
        return 12000.0f;
    case 105694:
    ...
    case 130348:
        return 60.0f;
    case 89344:   // Scroll of Protection IX
        return 400.0f;
        // Food:
        // "Well Fed" effect
    case 124218:
    ...
    if (HasAura(SPELL_AURA_MOD_STEALTH))
    {
        if (procSpell)
        {
            if (procSpell->Id == 93435  ||  // Roar of Courage
                ...
                procSpell->Id == 5277   ||  // Evasion
                procSpell->Id == 91023  ||  // Find Weakness
                procSpell->Id == 91021  ||  // Find Weakness (trigger)
                procSpell->Id == 57934  ||  // Tricks of the Trade
                procSpell->Id == 36554  ||  // Shadowstep
                procSpell->Id == 36563  ||  // Shadowstep (teleport effect)
                procSpell->Id == 54861  ||  // Nitro Boosts
                //procSpell->Id == 96228  ||  // Synapse Springs (Agility, it's just for rogue/feral)
                procSpell->Id == 79140  ||  // Vendetta
                procSpell->Id == 6770   ||  // Sap
                procSpell->Id == 14185  ||  // Preparation
                ...
                procSpell->Id == 32375  ||  // Mass Dispel
                procSpell->Id == 32592  ||  // Mass Dispel (triggered effect)
                procSpell->Id == 72734  ||  // Mass Dispel (second triggered effect)
                procSpell->Id == 97229  ||  // Bellowing Roal
                procSpell->Id == 24604  ||  // Furious Howl
                procSpell->Id == 90309  ||  // Terrifying Roar
                procSpell->Id == 126373)    // Fearless Roar
                return true;
        }
    }
