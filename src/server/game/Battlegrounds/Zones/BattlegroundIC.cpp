...
void BattlegroundIC::StartingEventOpenDoors()
{
    ...
    for (uint8 i = 0; i < MAX_FORTRESS_TELEPORTERS_SPAWNS; i++)
    {
        if (!AddObject(BG_IC_Teleporters[i].type, BG_IC_Teleporters[i].entry,
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle of Conquest | Starting Event Open Doors: There was an error spawning gameobject %u", BG_IC_Teleporters[i].entry);
    }
}
...
void BattlegroundIC::UpdateArea(Player* player, uint32 newarea)
{
    ...
    if (player->GetBGTeam() == ALLIANCE)
    {
        if (newarea == HORDE_KEEP)
            if (GateStatus[BG_IC_H_FRONT] != BG_IC_GATE_DESTROYED && GateStatus[BG_IC_H_WEST] != BG_IC_GATE_DESTROYED && GateStatus[BG_IC_H_EAST] != BG_IC_GATE_DESTROYED)
                // an area is too big, correct coordinates
                if (player->GetPositionX() >= 1161.0f && player->GetPositionX() <= 1281.0f && player->GetPositionY() >= -842.0f && player->GetPositionY() <= -683.0f)
                    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_BACK_DOOR_JOB);
    }
    else
    {
        if (newarea == ALLIANCE_KEEP)
            if (GateStatus[BG_IC_A_FRONT] != BG_IC_GATE_DESTROYED && GateStatus[BG_IC_A_WEST] != BG_IC_GATE_DESTROYED && GateStatus[BG_IC_A_EAST] != BG_IC_GATE_DESTROYED)
                // an area is too big, correct coordinates
                if (player->GetPositionX() >= 391.0f && player->GetPositionX() <= 399.0f && player->GetPositionY() >= -889.0f && player->GetPositionY() <= -775.0f)
                    player->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_BE_SPELL_TARGET, SPELL_BACK_DOOR_JOB);
    }
}
...
bool BattlegroundIC::SetupBattleground()
{
    for (uint8 i = 0; i < MAX_NORMAL_GAMEOBJECTS_SPAWNS; i++)
    {
        if (!AddObject(BG_IC_ObjSpawnlocs[i].type, BG_IC_ObjSpawnlocs[i].entry,
            ...
            0, 0, 0, 0, RESPAWN_ONE_DAY))
        {
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle of Conquest: There was an error spawning gameobject %u", BG_IC_ObjSpawnlocs[i].entry);
            return false;
        }
    }
    ...
    for (uint8 i = 2; i < MAX_NORMAL_NPCS_SPAWNS; i++)
    {
        if (!AddCreature(BG_IC_NpcSpawnlocs[i].entry, BG_IC_NpcSpawnlocs[i].type, BG_IC_NpcSpawnlocs[i].team,
            ...
            RESPAWN_ONE_DAY))
        {
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle of Conquest: There was an error spawning creature %u", BG_IC_NpcSpawnlocs[i].entry);
            return false;
        }
    }
    ...
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle of Conquest: Failed to spawn initial spirit guide!");
        return false;
    }
    ...
    if (!gunshipAlliance || !gunshipHorde)
    {
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle of Conquest: There was an error creating gunships!");
        return false;
    }
...
void BattlegroundIC::EndBattleground(uint32 winner)
{
	// Win reward.
	if (winner == ALLIANCE)
		RewardHonorToTeam(GetBonusHonorFromKill(1) * 5.5, ALLIANCE);
	if (winner == HORDE)
		RewardHonorToTeam(GetBonusHonorFromKill(1) * 5.5, HORDE);
...
uint32 BattlegroundIC::GetNextBanner(ICNodePoint* nodePoint, uint32 team, bool returnDefinitve)
{
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle Of Conquest: Unexpected return in GetNextBanner function");
    return 0;
}
...
void BattlegroundIC::HandleCapturedNodes(ICNodePoint* nodePoint, bool recapture)
{
    if (nodePoint->nodeType != NODE_TYPE_REFINERY && nodePoint->nodeType != NODE_TYPE_QUARRY)
    {
        if (!AddSpiritGuide(BG_IC_NPC_SPIRIT_GUIDE_1+nodePoint->nodeType-2,
            ...
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "Isle of Conquest: Failed to spawn spirit guide! point: %u, team: %u, ", nodePoint->nodeType, nodePoint->faction);
    }
    ...
    switch (nodePoint->gameobject_type)
    {
        case BG_IC_GO_HANGAR_BANNER:
            ...
            //sLog->outInfo(LOG_FILTER_SERVER_LOADING, "BG_IC_GO_HANGAR_BANNER CAPTURED Faction: %u", nodePoint->faction);
