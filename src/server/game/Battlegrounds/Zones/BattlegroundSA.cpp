...
void BattlegroundSA::RelocatePlayerPosition() 
{		
	TeleportPlayers();
}
...
void BattlegroundSA::GetTeamStartLoc(uint32 p_TeamID, float &p_PositionX, float &p_PositionY, float &p_PositionZ, float &p_Orientation) const 
{
	...
	if (GetStatus() == STATUS_WAIT_JOIN || Status == BG_SA_SECOND_WARMUP || Status == BG_SA_WARMUP)
    {
...
void BattlegroundSA::PostUpdateImpl(uint32 diff)
{
    ...
    if (Status == BG_SA_WARMUP)
    {
        ...
        if (TotalTime >= BG_SA_WARMUPLENGTH)
        {
            ...
            m_EndTimestamp = time(nullptr) + BG_SA_Timers::BG_SA_ROUNDLENGTH / TimeConstants::IN_MILLISECONDS;

            UpdateWorldState(BG_SA_WorldStates::BG_SA_ENABLE_TIMER, 1);
            UpdateWorldState(BG_SA_WorldStates::BG_SA_TIMER, m_EndTimestamp);
        }
    ...
    else if (Status == BG_SA_SECOND_WARMUP)
    {
        ...
        if (TotalTime >= 60000)
        {
            ...
            m_EndTimestamp = time(nullptr) + EndRoundTimer / TimeConstants::IN_MILLISECONDS;
            ...
            for (BattlegroundPlayerMap::const_iterator l_Itr = GetPlayers().begin(); l_Itr != GetPlayers().end(); ++l_Itr)
            {
                if (Player* l_Player = ObjectAccessor::FindPlayer(l_Itr->first))
                {
                    ...
                    l_Player->RemoveAurasDueToSpell(SPELL_PREPARATION);
                }
            }
        }
        if (TotalTime >= 30000)
...
void BattlegroundSA::FillInitialWorldStates(ByteBuffer &data)
{
	...
	{
		data << uint32(1);
		data << uint32(uint32(BG_SA_WorldStates::BG_SA_ENABLE_TIMER));
		data << uint32(m_EndTimestamp);
		data << uint32(uint32(BG_SA_WorldStates::BG_SA_TIMER));
	}
}
...
void BattlegroundSA::TeleportPlayers()
{
		for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
		{
			if (Player* player = ObjectAccessor::FindPlayer(itr->first))
			{
				...
				//Distancia permitida para el defensor, esta distancia esta dada desde el cementerio central hasta casi llegar a la playa, si sobrepasa
				//la distancia permitida se teletrasporta para atras al defensor;
				float maxDist = 120000.0f;
				uint32 l_AttackersTeam = (Attackers == TEAM_ALLIANCE) ? ALLIANCE : HORDE;
				if (player->GetBGTeam() == l_AttackersTeam){
					//Distancia permitida para que el atacante pueda caminar sobre el barco. 
					maxDist = 2000.0f;
				}
    ...
    {		
        if (Source->GetTeamId() == Attackers)
        {
            ...
            else if (Status == BG_SA_ROUND_TWO)
            {
                ...
                UpdateWorldState(BG_SA_WorldStates::BG_SA_ENABLE_TIMER, 0);
                UpdateWorldState(BG_SA_WorldStates::BG_SA_TIMER, 0);
