...
void BattlegroundKT::ClearOrbAuras(Player* player)
{
	...
	player->RemoveAura(121177); // Orb 4
	player->RemoveAura(121176); // Orb 3
	player->RemoveAura(121175); // Orb 2
	player->RemoveAura(121164); // Orb 1
}
...
void BattlegroundKT::EventPlayerDroppedOrb(Player* source, uint64 guid, uint32 team)
{
    ...
    if (source)
    {
		...
		source->RemoveAura(121177); // Orb 4
		source->RemoveAura(121176); // Orb 3
		source->RemoveAura(121175); // Orb 2
		source->RemoveAura(121164); // Orb 1
    }
...
void BattlegroundKT::HandleAreaTrigger(Player* source, uint32 trigger)
{
    ...
    switch(trigger)
    {
        ...
        case 7735: // Middle-In trigger
        {
			...
			if (m_playersZone[sourceGuid] == KT_KILLING_BLOW){				
				m_playersZone[sourceGuid] = KT_ZONE_MIDDLE;
			}               
			else{
				m_playersZone[sourceGuid] = KT_KILLING_BLOW;				
			}
