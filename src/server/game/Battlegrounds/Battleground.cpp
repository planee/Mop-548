    ...
    switch (GetStatus())
    {
        ...
        case STATUS_IN_PROGRESS:
            ...
            if (isArena())
            {
				...
				{
					uint32 time = (GetArenaType() == 2 ? 5 : 10) * MINUTE * IN_MILLISECONDS;
				...
				{
					UpdateBattleFatigue();
					m_BattleFatigueUpdateTimer = 3000;
				}
				else
...
inline void Battleground::_ProcessJoin(uint32 diff)
{
	...
	if (m_ResetStatTimer > 5000)
	{
		...
		for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
		{
			if (Player* player = ObjectAccessor::FindPlayer(itr->first))
			{
				...
				// Better Use The Same Method as Arenas Do to Refresh Combat Spells of Class when entering BG
				// Refreshing Spell Cooldowns that have Less than 10 Minutes
				...
				if (player->getClass() == CLASS_WARLOCK)
				{
					...
					case SPEC_WARLOCK_DEMONOLOGY:
						player->SetPower(POWER_DEMONIC_FURY, 200);
						break;
					case SPEC_WARLOCK_DESTRUCTION:
						player->SetPower(POWER_BURNING_EMBERS, 10);
						break;
					case SPEC_WARLOCK_AFFLICTION:
						player->SetPower(POWER_SOUL_SHARDS, 400);
						break;
					default:
						break;
					}
    ...
    else if (GetStartDelayTime() <= 0 && !(m_Events & BG_STARTING_EVENT_4))
    {
        ...
        for (BattlegroundPlayerMap::const_iterator Itr = m_Players.begin(); Itr != m_Players.end(); ++Itr)
        {
            Player* player = _GetPlayer(Itr, "RemoveAura");
            if (player)
            {
                player->RemoveAura(115867); // Mana Tea
                player->RemoveAura(125195); // Tigereye Brew
                player->RemoveAura(128939); // Elusive Brew

                if (Pet* pet = player->GetPet())
                {
                    pet->RemoveAura(91342); // Shadow Infusion
                    pet->RemoveAura(63560); // Dark Transformation
                }
            }
        }
...
void Battleground::EndBattleground(uint32 winner)
{
	...
	else if (IsRatedBG())
	{
		try
		{
			// Get the alliance group
			Group* alliance = GetBgRaid(ALLIANCE);
			// Get the horde group
			Group* horde = GetBgRaid(HORDE);
			// Check the groups
			if (!alliance)
			{
				sLog->outError(LOG_FILTER_BATTLEGROUND, "Error at Battleground::EndBattleground, cant get Alliance group");
				sLog->OutSpecialLog("Error at Battleground::EndBattleground, cant get Alliance group");
				SetWinner(WINNER_UNDECIDED);
				return;
			}
			if (!horde)
			{
				sLog->outError(LOG_FILTER_BATTLEGROUND, "Error at Battleground::EndBattleground, cant get Horde group");
				sLog->OutSpecialLog("Error at Battleground::EndBattleground, cant get Horde group");
				SetWinner(WINNER_UNDECIDED);
				return;
			}
			// 
			if (alliance->GetMembersCount() > horde->GetMembersCount() && horde->GetMembersCount() == 0)
			{
				SetWinner(WINNER_ALLIANCE);
				winner = ALLIANCE;
			}
			else if (horde->GetMembersCount() > alliance->GetMembersCount() && alliance->GetMembersCount() == 0)
			{
				SetWinner(WINNER_HORDE);
				winner = HORDE;
			}
		...
		{
			sLog->outError(LOG_FILTER_BATTLEGROUND, "Error at Battleground::EndBattleground, Exception Catched");
			sLog->OutSpecialLog("Error at Battleground::EndBattleground, Exception Catched");
			SetWinner(WINNER_UNDECIDED);
		}
	}
	else
        ...
        if (winner_team && loser_team && winner_team != loser_team && GetWinner() != WINNER_UNDECIDED)
        {
			...
			for (BattlegroundPlayerMap::iterator l_Itr = m_Players.begin(); l_Itr != m_Players.end(); l_Itr++)
			{
				...
				if (l_Player->GetGroup() && l_Player->GetGroup()->IsGuildGroup(0, true, true))
				{
					...
					{
						l_Guild->CompleteGuildChallenge(GuildChallengeType::CHALLENGE_RATED_BG);
						break;
					}
				}
			}
    ...
    for (BattlegroundPlayerMap::iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
    {
		...
		if (team == winner && isArena() && isRated() && !IsWargame() && GetArenaType() == ARENA_TYPE_5v5 && aliveWinners == 1 && player->isAlive())
        ...
        if (isArena() && isRated() && winner_team && loser_team && winner_team != loser_team && GetWinner() != WINNER_UNDECIDED)
        {
            ...
            if (team == winner)
            {
				...
				player->ModifyCurrency(CURRENCY_TYPE_CONQUEST_META_ARENA, sWorld->getIntConfig(CONFIG_CURRENCY_CONQUEST_POINTS_ARENA_REWARD), true, true, false, true);
            }
            else
            {
		...
		/*	uint32 mapId = GetMapId();
			uint32 char_id = player->GetGUIDLow();

			// Delete Before Insert If Duplicate
			PreparedStatement* del_duplicate = CharacterDatabase.GetPreparedStatement(CHAR_DEL_PVPSTATS_PLAYER);
			del_duplicate->setUInt32(0, mapId);
			del_duplicate->setUInt32(1, char_id);
			CharacterDatabase.Execute(del_duplicate);
			...
			stmt->setUInt32(1, char_id);
			stmt->setBool(2, team == winner);
			stmt->setUInt32(3, score->second->KillingBlows);
			stmt->setUInt32(4, score->second->Deaths);
			stmt->setUInt32(5, score->second->HonorableKills);
			stmt->setUInt32(6, score->second->BonusHonor);
			stmt->setUInt32(7, score->second->DamageDone);
			stmt->setUInt32(8, score->second->HealingDone);
			stmt->setUInt32(9, 0);
			stmt->setUInt32(10, 0);
			stmt->setUInt32(11, 0);
			stmt->setUInt32(12, 0);
...
void Battleground::RemovePlayerAtLeave(uint64 guid, bool Transport, bool SendPacket)
{
	...
	// Remove Player from Any kind of Team&Group to avoid Mismatchs
	// player->RemoveFromGroup(GROUP_REMOVEMETHOD_DEFAULT); Crash
...
void Battleground::UpdateBattleFatigue()
{
	for (BattlegroundPlayerMap::const_iterator itr = m_Players.begin(); itr != m_Players.end(); ++itr)
	{
		if (Player* player = _GetPlayer(itr, "UpdateBattleFatigue"))
		{
			...
			auto aura = player->GetAura(c_BattleFatigueEffect);
			if (aura){
				auto effect = aura->GetEffect(EFFECT_0);
				effect->SetAmount(-60);
