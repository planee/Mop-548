...
{
    protected:
        TargetedMovementGeneratorMedium(Unit* owner, float offset, float angle, bool useExactTargetLocation = false) : TargetedMovementGeneratorBase(owner), i_recheckDistance(0), i_offset(offset), i_angle(angle), i_recalculateTravel(false), i_targetReached(false), m_UseExactTargetLocation(useExactTargetLocation) { }
