...
void StartEluna(bool restart)
{
    if (restart)
    {
        ...
        sLog->outInfo(LOG_FILTER_SERVER_LOADING, "[Eluna]: Restarting Lua Engine");
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "[Eluna]: Lua Engine loaded.");
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "");
    ...
    for (std::set<std::string>::const_iterator itr = loadedScripts.begin(); itr !=  loadedScripts.end(); ++itr)
    {
        ...
        {
            sLog->outInfo(LOG_FILTER_SERVER_LOADING, "[Eluna]: Error loading file `%s`.", itr->c_str());
        ...
        else
        {
            ...
            {
                sLog->outInfo(LOG_FILTER_SERVER_LOADING, "server.loading", "[Eluna]: Error loading file `%s`.", itr->c_str());
                sEluna->report(sEluna->L);
            }
        }
        ++count;
    ...
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "[Eluna]: Loaded %u Lua scripts..", count);
    sLog->outInfo(LOG_FILTER_SERVER_LOADING, "");
}
